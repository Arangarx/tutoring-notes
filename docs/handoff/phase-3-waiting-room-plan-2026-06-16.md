# Phase 3 ΓÇö Mutual waiting room + retire consent click: Detailed Executable Plan

> **Branch:** `phase3/wb-waiting-room` (fork off `v1-redesign` **after** P2 smoke PASS + `merge --no-ff`)  
> **Program:** Live-session floor ΓÇö [`live_session_floor`](../../../.cursor/plans/live_session_floor_2e662852.plan.md) P3 (largest / highest-risk floor phase)  
> **Authored:** 2026-06-16 (planning pass ΓÇö **no P3 production code in this commit**)  
...
> **Status:** Planning complete ΓÇö **5-axis review follows separately**; **EXECUTION GATED** on (a) P2 smoke PASS + merge and (b) explicit Andrew greenlight. **Destructive Neon test-account migration must NOT run unattended.**

---

## Γ¢ö EXECUTION GATES (read first)

| Gate | Requirement |
|---|---|
| **G1 ΓÇö P2 landed** | `phase2/wb-student-new-shell` smokebook PASS + `merge --no-ff` into `v1-redesign`. P3 builds on P2's unified shell (`WhiteboardSessionShell` student union, `StudentLiveWorkspaceClient`, `LiveBoardChrome`). |
| **G2 ΓÇö Andrew greenlight** | Explicit go before **any** P3 code, **any** shared Neon destructive migration, or deleting `CONSENT_ENFORCEMENT`. |
| **G3 ΓÇö 5-axis review** | Independent adversarial review of **this plan** completes; BLOCKERs folded into ┬º8 acceptance before executor dispatch. |
| **G4 ΓÇö Destructive DB** | Test-account forward-migration (┬º6) runs only after dry-run + Andrew confirms inventory row-by-row. Never on production Sarah data paths without explicit keep-list. |

---

## Γ¢ö HARD CONSTRAINT ΓÇö extend don't rewrite

**Tutor live engine (`WhiteboardWorkspaceClient.tsx`) is ADDITIVE-ONLY for P3 waitingΓåÆlive.**
...
Implement the **mutual waiting room** (tutor + student both present before live board mounts), backed by a real **session lifecycle state machine** (`pending` ΓåÆ `active`), **unconditional consent enforcement** (retire tutor consent click + `CONSENT_ENFORCEMENT` flag), and **timer realignment** (billable clock starts at synchronized waitingΓåÆlive, not student link-open).

### Baked-in decisions (from [`live_session_floor`](../../../.cursor/plans/live_session_floor_2e662852.plan.md) ΓÇö do not re-litigate)

| ID | Decision |
|---|---|
| **P3-a** | Add `sessionPhase` (`pending` \| `active`) + `sessionMode` (`live` \| `in_person`); server `start`/admit endpoint is **source of truth** for waitingΓåÆlive. |
...
| **P3-d** | **Synchronized start** ΓÇö both sides transition to live only after server admits; `[wtr]` / `[slc]` logging; timer realigned to admit moment. |
| **P3-e** | **Delete `CONSENT_ENFORCEMENT`** ΓÇö enforcement unconditional; feature branch is rollback. Wire parent-edit POST + whiteboard-recording gate. |
| **P3-f** | **No backwards-compat / no unclaimed fallback** ΓÇö require `Student.learnerProfileId` + `ConsentRecord` to run a session. One-time destructive forward-migration of shared Neon **test** accounts (┬º6). |
...
| **(c)** | Keep `/w/[joinToken]` hybrid backup path (Andrew 2026-06-14). P3 changes student entry from straight-to-live ΓåÆ waiting-first, not URL retirement. |
...
- Timer: stop stamping `bothConnectedAt` on student link-open; start `activeMs` accumulation only after `sessionPhase ΓåÆ active`.
...
- Destructive test-account migration script with dry-run (┬º6).
- Register `wtr` + `slc` in `AGENTS.md` (same commit as first log line).
- `docs/WHITEBOARD-P3-STATUS.md` + smokebook at Step 9.
- Unit + `test:wb-sync` scenarios + two-device smoke matrix (┬º7).

### Out of scope (P4+)

- Mid-session learner swap UI + `/api/sessions/swap` (design in [`session-lifecycle-consent-design-2026-05-31.md`](session-lifecycle-consent-design-2026-05-31.md) ┬º3; BLOCKERs listed in [`v1-redesign-STATUS.md`](v1-redesign-STATUS.md) ΓÇö **not P3**).
...
- Retiring `NEXT_PUBLIC_WB_STUDENT_NEW_SHELL` / legacy `StudentWhiteboardClient` (P2 Step 9 ΓÇö separate gate).
- Backend spine: A6-1 multi-segment replay, `phase1/wb-reliability-floor` 1b clock merge, note-quality map/reduce (parked until floor P1ΓÇôP3 smokeable).
- Waiting room 10-minute learner timeout (design ┬º2.6 ΓÇö optional P3.1 if time; default defer unless 5-axis mandates).
- Parent mid-session consent poll / banner (design ┬º5 Q-CGC-5 ΓÇö defer).

---

## 2. Current state

> **Branch note:** Line refs below are `v1-redesign` @ planning time. P2 shell files (`StudentLiveWorkspaceClient`, student union on `WhiteboardSessionShell`) live on `phase2/wb-student-new-shell` @ `93d71ca` until merged ΓÇö P3 executor rebases onto post-P2 `v1-redesign`.

### Session lifecycle (no waiting room today)

| Piece | Location | Behavior today |
|---|---|---|
| Session create | [`createWhiteboardSession`](../../src/app/admin/students/[id]/whiteboard/actions.ts) L73ΓÇô246 | Creates row with `startedAt @default(now())` ΓåÆ **immediately "active"**; `consentAcknowledged` required; freezes `SessionConsentSnapshot` in same transaction; redirects straight to workspace **live**. |
| Tutor consent click | [`StartWhiteboardSession.tsx`](../../src/app/admin/students/[id]/whiteboard/StartWhiteboardSession.tsx) L31ΓÇô198 | Modal checkbox ΓåÆ `consentAcknowledged` form field. |
| Workspace re-check | [`workspace/page.tsx`](../../src/app/admin/students/[id]/whiteboard/[whiteboardSessionId]/workspace/page.tsx) L71ΓÇô73 | `notFound()` if `!consentAcknowledged`. |
| Shell modes | [`WhiteboardSessionShell.tsx`](../../src/app/admin/students/[id]/whiteboard/[whiteboardSessionId]/workspace/WhiteboardSessionShell.tsx) L8ΓÇô10, L34 | `live` \| `review` only; comment: `waiting` reserved. |
| Student join | [`w/[joinToken]/page.tsx`](../../src/app/w/[joinToken]/page.tsx) L88ΓÇô99 | Stamps `bothConnectedAt` on first link-open ΓåÆ **timer starts too early**. Renders straight to live board (legacy or P2 shell). |
| Timer API | [`join-timer/route.ts`](../../src/app/api/whiteboard/[sessionId]/join-timer/route.ts) L88ΓÇô108 | Returns `activeMs` / `lastActiveAt`; no `sessionPhase` awareness. |
| Active ping | [`active-ping/route.ts`](../../src/app/api/whiteboard/[sessionId]/active-ping/route.ts) L126ΓÇô132 | Stamps `bothConnectedAt` on first positive ping ΓÇö must align with new admit semantics. |
| Prisma session row | [`schema.prisma`](../../prisma/schema.prisma) L266ΓÇô344 | `startedAt DateTime @default(now())`; `consentAcknowledged Boolean`; `bothConnectedAt DateTime?`; **no** `sessionPhase` / `sessionMode`. |
| Visual stubs | [`LearnerWaitingRoom.tsx`](../../src/components/student/LearnerWaitingRoom.tsx) L23ΓÇô24 | `state` prop visual-only; no presence/admit. [`join/page.tsx`](../../src/app/join/page.tsx) L34ΓÇô36 ΓÇö `?preview=admitted` dev toggle only. |
...
| Piece | Location | Behavior today |
|---|---|---|
| Flag | [`consent-scope.ts`](../../src/lib/consent-scope.ts) L56ΓÇô59 | `isConsentEnforcementEnabled()` reads `CONSENT_ENFORCEMENT`; **default OFF**. |
| Session assert | [`consent-scope.ts`](../../src/lib/consent-scope.ts) L90ΓÇô159 | Flag OFF ΓåÆ void; no snapshot ΓåÆ **pass** (unclaimed fallback); self-learner auto-pass. |
| Snapshot freeze | [`consent-scope.ts`](../../src/lib/consent-scope.ts) L176ΓÇô256 | Always runs on create; skipped when `!learnerProfileId` or no `ConsentRecord`. |
| Live session block | [`actions.ts`](../../src/app/admin/students/[id]/whiteboard/actions.ts) L134ΓÇô156 | `allowLiveSession` check **only when flag ON**. |
| Audio register | [`actions.ts`](../../src/app/admin/students/[id]/whiteboard/actions.ts) L1273, L601 | `assertEffectiveConsent(..., allowAudioRecording)` ΓÇö flag-gated. |
| Notes generate | [`actions.ts`](../../src/app/admin/students/[id]/whiteboard/actions.ts) L977 | `assertEffectiveConsent(..., allowNoteSending)` ΓÇö flag-gated. |
| **Whiteboard replay gate** | ΓÇö | **`allowWhiteboardRecording` NOT wired** to events.json upload or FSM `shouldCaptureWB` (BLOCKER-D2 from lifecycle design). |
| Parent edit save | [`ParentConsentEditor.tsx`](../../src/app/account/children/[id]/consent/ParentConsentEditor.tsx) L137ΓÇô139, L269 | `handlePreviewSave` sets local state only; toast says visual-first. |
...
| B2 schema | [`schema.prisma`](../../prisma/schema.prisma) L1039ΓÇô1098 | `ConsentRecord`, `ConsentRestriction`, `SessionConsentSnapshot` **exist** (merged B2). |
| Participant stub | [`session-participant-scope.ts`](../../src/lib/session-participant-scope.ts) L36ΓÇô47 | Always `notFound()` ΓÇö `SessionParticipant` **not in schema**. |
...
|---|---|---|
| Student shell | `phase2` ΓåÆ `StudentLiveWorkspaceClient.tsx` | `LiveBoardChrome`, `useLiveAV`, `useStudentWhiteboardCanvas`, straight-to-**live**. |
| Shell union | `phase2` ΓåÆ `WhiteboardSessionShell.tsx` L46ΓÇô62 | `role: "student"` \| `"tutor"` discriminated union. |
...
createWhiteboardSession
  ...
  capture      = BLOCKED
        Γöé
        Γöé  both sides in waiting room (A/V preview, consent display)
        Γöé
        Γû╝
POST startWhiteboardSession / admit
  ...
  activeMs clock may begin (bothConnectedAt / active-ping policy ┬º5)
  capture      = per snapshot toggles
        Γöé
        Γû╝
  shell mode = live  ΓåÆ  WhiteboardWorkspaceClient / StudentLiveWorkspaceClient
```
...
//   waiting ΓåÆ WaitingRoomWorkspace (role=tutor|student)
//   live    ΓåÆ existing P2 paths
//   review  ΓåÆ SessionReviewMode (tutor only)
```
...
| `cns` | (existing) | Consent checks ΓÇö now unconditional |

Full event catalog: [`session-lifecycle-consent-design-2026-05-31.md`](session-lifecycle-consent-design-2026-05-31.md) ┬º7.5 (BLOCKER-O1).

---

## 4. Three-pillar implementation sequence

**Why this order:** Pillar 1 (schema + server state) is prerequisite for every other surface ΓÇö UI cannot honest-show "waiting" without `sessionPhase`, and capture gates must be server-enforced before deleting consent click. Pillar 2 (waiting room UI) validates the lifecycle on real devices before the irreversible Pillar 3 (flag deletion + destructive migration + consent click retirement).

```
Pillar 1 ΓöÇΓöÇΓû║ Pillar 2 ΓöÇΓöÇΓû║ Pillar 3
...
### Pillar 1 ΓÇö Session lifecycle state machine (Steps 0ΓÇô4)

#### Step 0 ΓÇö Pre-flight + design reconciliation spike

| Action | Detail |
|---|---|
| Rebase | Fork `phase3/wb-waiting-room` from post-P2-merge `v1-redesign`. |
| Read | [`RECORDER-LIFECYCLE.md`](../RECORDER-LIFECYCLE.md), [`LIVE-AV.md`](../LIVE-AV.md) ┬º workspace integration, shell mock waiting page. |
...
**Exit:** Spike note in `WHITEBOARD-P3-STATUS.md` ΓÇö preview-only A/V path confirmed or escalated.

---

#### Step 1 ΓÇö Additive migration + backfill
...
startedAt      DateTime?    // REMOVE @default(now()) ΓÇö set on admit only
activeSwapId   String?      // schema for future swap; no UX in P3
```

**New model `SessionParticipant`** (from [`authed-session-access-design-2026-06-10.md`](authed-session-access-design-2026-06-10.md) ┬º4.1):
...
-- 3. Partial unique index (BLOCKER-C2) ΓÇö one non-ended session per student
...
- `consentAcknowledged` column **retained** through Pillar 1ΓÇô2; dropped or deprecated in Pillar 3 only after click retirement.
...
#### Step 2 ΓÇö `createWhiteboardSession` ΓåÆ pending sessions
...
**Hard requirement (Pillar 1 partial):** Reject session create when `!student.learnerProfileId` **only after Pillar 3** ΓÇö OR gate behind interim check in Pillar 1 if Andrew prefers early fail. **Plan default:** enforce in Pillar 3 together with migration (avoid bricking unmigrated test accounts mid-branch).

**Exit:** New sessions land in waiting mode; existing sessions unchanged.

---

#### Step 3 ΓÇö Admit / start endpoint (source of truth)
...
| Presence (see ┬º5.2) | Policy: require student presence signal OR explicit tutor "Start without student" for solo warmup |
...
#### Step 4 ΓÇö Block capture while pending
...
| `active-ping` | Do not accumulate `activeMs` while `pending` (or ignore pings ΓÇö document choice in ┬º5.3). |

**Exit:** Unit tests: pending session ΓåÆ audio register 403; FSM `shouldCaptureWB === false`; after admit ΓåÆ capture follows consent snapshot.

---

### Pillar 2 ΓÇö Waiting room UI (Steps 5ΓÇô8)

#### Step 5 ΓÇö `WaitingRoomWorkspace` + shell wiring

**New files:**

| File | Role |
|---|---|
| `src/app/admin/.../workspace/WaitingRoomWorkspace.tsx` | Tutor waiting: `useLiveAV` preview, consent panel from snapshot, in-person declaration, student presence pill, **Start session** ΓåÆ calls admit action. |
...
| `src/components/whiteboard/chrome/WbWaitingRoomChrome.tsx` | Layout per shell mock `page-waiting` ΓÇö reuse `LiveBoardChrome` tokens without tool strip. |
...
```typescript
type ShellMode = "waiting" | "live" | "review";
// tutor: if mode==="waiting" ΓåÆ WaitingRoomWorkspace
// student (P2 union): if waiting ΓåÆ StudentWaitingRoomWorkspace
```

**`useLiveAV` reuse:** Request mic/cam in waiting room per [`useLiveAV.ts`](../../src/hooks/useLiveAV.ts) contract (inert until `requestMic`/`requestCam`). Do **not** mount peer mesh for board sync until `live` unless presence requires lightweight sync connection ΓÇö document in STATUS.

**In-person mode (`sessionMode === "in_person"`):**

- Tutor UI captures declaration: "Student is present in person."
- Display student's effective privacy toggles (from snapshot) on tutor side ΓÇö read-only projection.
...
#### Step 6 ΓÇö Presence + synchronized transition

| Mechanism | Detail |
|---|---|
| Student arrive | On `/w/[joinToken]` load in waiting: log `[wtr] action=student_arrived`; set presence flag (DB column `studentJoinedAt` **or** sync presence event **or** `SessionParticipant.joinedAt` ΓÇö pick one; prefer `SessionParticipant.joinedAt` update on first waiting mount). |
| Tutor UI | Show "Student has joined" when presence true. |
| Admit poll | Both clients poll `join-timer` / status every 2ΓÇô5s; on `sessionPhase === "active"`, atomically flip shell `mode` to `live`. |
...
**Remove** [`w/[joinToken]/page.tsx`](../../src/app/w/[joinToken]/page.tsx) L88ΓÇô99 `bothConnectedAt` stamp on link-open ΓÇö defer to post-admit path (┬º5.3).

**Exit:** Integration test: admit flips API response; student poll triggers mode transition mock.

---

#### Step 7 ΓÇö Timer realignment
...
| `join-timer` | Return `startedAt` / `sessionPhase`; student timer displays zero or "WaitingΓÇª" until active. |
| Tutor timer | `WhiteboardWorkspaceClient` / waiting chrome: same rule. |

**Open semantics (Andrew confirm ┬º9):** whether pre-admit waiting time is visible anywhere (recommend: hidden; timer shows `--:--` until active).

**Exit:** Unit test on `active-time.ts` + join-timer route with phase fixtures.

---

#### Step 8 ΓÇö Flag-gated waiting room rollout (interim)

| Env var | Semantics |
|---|---|
| `NEXT_PUBLIC_WB_WAITING_ROOM=1` | New pendingΓåÆwaitingΓåÆlive path. |
...
**Exit:** Flag off ΓåÆ legacy straight-to-live (post-P2 behavior); flag on ΓåÆ waiting room path.

---

### Pillar 3 ΓÇö Consent hook-in + click retirement + data migration (Steps 9ΓÇô12)

> **ΓÜá∩╕Å Destructive / irreversible on shared Neon.** Run ┬º6 only after Pillar 1ΓÇô2 smoke on Preview with **migrated test accounts**, then Andrew greenlight.

#### Step 9 ΓÇö Unconditional consent enforcement

| File | Change |
|---|---|
| [`consent-scope.ts`](../../src/lib/consent-scope.ts) | **Delete** `isConsentEnforcementEnabled`, `CONSENT_ENFORCEMENT` branches. `assertEffectiveConsent`: no snapshot ΓåÆ **throw** (claimed learner); unclaimed ΓåÆ **throw** at create time. Remove unclaimed pass paths in `assertConsentFromLiveRecord`. |
...
| Tests | Update [`consent-b2.test.ts`](../../src/__tests__/identity/consent-b2.test.ts) ΓÇö remove flag toggling; always enforce. |
...
#### Step 10 ΓÇö Parent consent POST
...
#### Step 11 ΓÇö Retire tutor consent click

| File | Change |
|---|---|
| [`StartWhiteboardSession.tsx`](../../src/app/admin/students/[id]/whiteboard/StartWhiteboardSession.tsx) | Remove consent modal ΓåÆ simple "Start session" (+ mode picker). Or inline button calling `createWhiteboardSession` without checkbox. |
| [`actions.ts`](../../src/app/admin/students/[id]/whiteboard/actions.ts) | Remove `consentAcknowledged` form validation; stop writing `consentAcknowledged: true` (column optional deprecate ΓÇö keep for audit history). |
...
| [`createWhiteboardSession.test.ts`](../../src/__tests__/whiteboard/createWhiteboardSession.test.ts) | Rewrite consent tests ΓåÆ learner-profile + `allowLiveSession` tests. |

**Exit:** No tutor checkbox; consent = parent toggles + waiting room assessment display.

---

#### Step 12 ΓÇö Destructive Neon test-account migration (┬º6)

Run **after** Step 9ΓÇô11 code is on Preview; **not** in same deploy as first prod cut without Andrew.

**Exit:** Dry-run log reviewed; Andrew signs inventory; execute; verify Sarah tutor account untouched.

---

## 5. Reliability ΓÇö design details

### 5.1 Synchronized start vs recorder pillars

| Invariant | P3 protection |
|---|---|
| Pillar 1 ΓÇö FSM purity | `sessionPhase` is a **precondition input** to `evaluateLifecycle`, not inside FSM side effects. |
| Pillar 2 ΓÇö Outbox | No `registerUpload` for session audio until `active`. Outbox `sessionId` unchanged across waitingΓåÆlive (same `wbsid`). |
| Pillar 3 ΓÇö Atomic end | Unchanged; pending cancel/end uses shortened path (no segments expected). |
| Admit race | Server transaction sets `sessionPhase`; clients poll ΓÇö no client-only admit. |
| Double Start click | Admit endpoint idempotent. |

### 5.2 Waiting ΓåÆ live race matrix

| Scenario | Expected behavior |
|---|---|
| Student polls active before tutor UI flips | Student mounts live board; tutor follows on next poll ΓÇö acceptable brief asymmetry |
| Tutor admit while student offline | Policy: allow (solo start) OR block with message ΓÇö **Andrew confirm ┬º9** |
...
- `endWhiteboardSession` with zero segments ΓÇö must succeed (extend if today assumes active).
- Log `[slc] action=session_ended phase=cancelled_from_pending`.

### 5.5 5-axis BLOCKERs to fold (TBD ΓÇö reviewer adds specifics)
...
| ID | Summary |
|---|---|
| (reviewer) | Note write without `allowNoteSending` ΓåÆ 403 |
| (reviewer) | Session close never wedged by consent failure |
| (reviewer) | No plaintext consent bypass via direct API |

**Swap BLOCKERs (D1, R1, C1, A1, A2):** **Out of P3 scope** ΓÇö note in 5-axis as N/A unless reviewer disagrees.
...
- **No backwards-compat** for unclaimed students ΓÇö after migration + P3, every `Student` must have `learnerProfileId` + effective `ConsentRecord`.
- Accounts with **no learners** ΓåÆ create **solo learner** (`isSelfLearner=true`) with all capture toggles **on**.
...
|---|---|---|
| **Sarah ΓÇö tutor (operator)** | **KEEP** | Production pilot; real data |
| **Sarah ΓÇö student standalone** | **DELETE** (disposable) | Master-era unclear model; student re-signs up as real learner if continuing |
...
1. **`--dry-run`** (default): print JSON plan ΓÇö no writes.
...
5. **Reversibility:** solo-learner migrate reversible manually; deletes **not** reversible ΓÇö dry-run is the control.
...
| **`npm run test:wb-sync`** with `NEXT_PUBLIC_WB_WAITING_ROOM=1` + `NEXT_PUBLIC_WB_STUDENT_NEW_SHELL=1` | **Mandatory** ΓÇö waiting + admit + live sync |
...
| 1 | Tutor create ΓåÆ waiting | Tutor lands waiting; no board tools; no recording arm |
| 2 | Student join ΓåÆ waiting | Student sees waiting chrome; self-view; disclosure; no Excalidraw |
| 3 | A/V preview both sides | Mic/cam preview works; no live board |
| 4 | Consent display | Tutor sees snapshot toggles; denied audio ΓåÆ recording toggle disabled post-admit |
...
| 7 | Synchronized start | Tutor Start ΓåÆ both flip live within 5s; `slc` + `wtr` logs |
...
| 15 | Themes | Repeat 1ΓÇô9 in **light** and **dark** |
| 16 | P2 tutor regression | Tutor live path unchanged with waiting flag off |

### 7.3 `test:wb-sync` scenarios (flag on)

- Student joins waiting ΓåÆ tutor admits ΓåÆ stroke sync (extend existing spec).
...
| P3-G6 | Timer aligned to admit ΓÇö not link-open |
...
| P3-A* | **TBD** ΓÇö 5-axis reviewer folds axis-specific BLOCKERs here |

**Sarah-trust framing:** If Sarah must keep Zoom open because the waiting room lies about readiness, capture starts early, or consent is unclear ΓÇö P3 is not done.
...
|---|---|---|
| **Q1** | **Destructive migration inventory:** confirm table ┬º6.2 row-by-row (especially `arangarx` / `shaltinelis` / `daniel` / `lilah` ΓÇö solo-migrate vs delete). | Solo-migrate all family test accounts; delete only Sarah standalone student |
...
| `src/app/admin/.../WaitingRoomWorkspace.tsx` | **New** ΓÇö tutor waiting |
| `src/app/w/[joinToken]/page.tsx` | Waiting entry; remove early `bothConnectedAt` |
| `src/app/w/[joinToken]/StudentWaitingRoomWorkspace.tsx` | **New** ΓÇö student waiting |
| `src/components/whiteboard/chrome/WbWaitingRoomChrome.tsx` | **New** ΓÇö layout |
| `src/lib/consent-scope.ts` | Delete flag; hard enforcement |
| `src/lib/session-participant-scope.ts` | Real implementation |
| `src/app/api/account/children/[id]/consent/route.ts` | **New** ΓÇö parent POST |
...
| `scripts/forward-migrate-p3-test-accounts.ts` | **New** ΓÇö dry-run migration |
| `AGENTS.md` | `slc`, `wtr` registry |
| `docs/PLATFORM-ASSUMPTIONS.md` | Waiting room flag |
| `docs/WHITEBOARD-P3-STATUS.md` | **New** ΓÇö STATUS handoff |
| `docs/handoff/phase-3-waiting-room-smokebook-2026-06-16.md` | **New** ΓÇö at Step 9 |
...
1. **Synchronized start ├ù recorder pillars** ΓÇö admit race must not arm FSM early or split outbox across pseudo-states.
2. **Timer realignment** ΓÇö `bothConnectedAt` / `activeMs` / `join-timer` triple consistency across tutor + student + server.
3. **Unconditional consent + destructive migration** ΓÇö wrong row in ┬º6.2 deletes Sarah data; dry-run discipline is the control.
4. **Block-capture-while-pending** ΓÇö every capture path (audio register, WB replay events, notes, end-session segments) must be enumerated; one leak = COPPA failure.
5. **P2 integration** ΓÇö waiting mode must not regress P2 student shell (`StudentLiveWorkspaceClient` mounts only after admit).

---

*End of plan ΓÇö execution gated on P2 smoke + merge and explicit Andrew greenlight.*
