...
Behavior:
1. **Inputs (env or CLI)**: `DATABASE_URL` (which DB to use for orphan detection — typically dev DB mirroring prod), `BLOB_READ_WRITE_TOKEN` (Vercel Blob token — typically prod since the orphans live there), `--dry-run` (default true), `--delete` (must be explicit), `--min-age-days N` (default 7 — never touch blobs younger than this; protects against race conditions where blob exists but DB row pending), `--max-deletions N` (default 50 — safety cap; explicit `--no-limit` to override), `--prefix <path>` (optional; restrict to blobs under a prefix).
2. **Logic**: (a) LIST all blobs in the configured Vercel Blob store; (b) for each blob URL, check if it's referenced in `SessionRecording.blobUrl`, `WhiteboardAsset.blobUrl`, or `WhiteboardSession.snapshotBlobUrl`; (c) if NOT referenced AND blob older than `--min-age-days`, mark as orphan candidate; (d) print full orphan list with size + age; (e) if `--delete`, delete each (with rate-limit aware backoff: 100ms between calls, 10s on 429); (f) if `--dry-run`, just print "WOULD DELETE: <url> (age=Xd, size=Y)".
3. **Safety refusals** (the script HARD-EXITs on these, no override):
   - Refuse to run if `DATABASE_URL` matches `BLOB_READ_WRITE_TOKEN`'s implied environment (e.g. both prod) AND `--delete` is set, UNLESS `--allow-prod-x-prod` is explicitly passed.
...
4. **Logging**: every action logged with `[blob-cleanup] blb=<scriptRunId> ...` prefix per AGENTS.md. Generate a `scriptRunId` UUID at script start. Print summary at end: `LISTED <n>, ORPHANS <m>, DELETED <k> (or WOULD-DELETE if dry-run)`.
...
### Blob cleanup
- [ ] Set up env: `DATABASE_URL` = local dev DB or Neon dev branch, `BLOB_READ_WRITE_TOKEN` = whichever Vercel Blob store you're inspecting (typically prod for real orphan detection).
- [ ] Run `node scripts/blob-cleanup.mjs` (dry-run). Verify output: lists blobs, identifies orphans, no actual deletes. No errors.
- [ ] Spot-check 2-3 of the listed orphans — query the DB for their URLs to confirm they really have no reference.
- [ ] Try `node scripts/blob-cleanup.mjs --delete --dry-run` → script refuses (contradictory flags).
- [ ] Try `node scripts/blob-cleanup.mjs --delete --max-deletions 0 --no-limit` (carefully against dev environment) — verify it's willing to delete but encounters 0 candidates if the env is clean.
- [ ] Do NOT run `--delete` against prod yet. That's a separate operational step Andrew runs during a maintenance window.
