...
2. **Phase-2 identity (HOLD):** schedule **AccountHolder auth/session pillar** design pass before merging `identity-p2-schema` @ `e305d0b` or `identity-p2-ownership-guard` @ `f74f164`.
...
4. **Session-lifecycle + consent implementation — DECISION-UNBLOCKED (2026-05-31):** dispatch Identity Phases 3–4 per design §8 **after** p2 schema + ownership guard merged.
5. **BLOCKERs (active):** umbrella deploy + OpenAI DPA + functioning deletion-request inbox; session-lifecycle Phase-3 BLOCKERs.
