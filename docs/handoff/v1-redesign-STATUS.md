...
| **`v1-redesign`** | `e466a26` → advances with identity Phase-2 auth/session design commit | Long-running V1 epic integration branch. **`identity-p1-2fa` MERGED `--no-ff` 2026-06-01** @ `b5ef4fe` (smoked @ `d782430`). **`component-b2-dashboard-students` MERGED `--no-ff` 2026-06-01** @ `0424206` (smoked as-scoped reskin floor). Also: design-integration + **Component Phase A** (`5aa3c7d`) + **Component Phase B1** (`b798494`) + notice-fix (`38a849a`) + legal facades (`29f5e88`) + whiteboard viewport layout principle (`0bf2ea0`) + overnight p2 ownership-guard spine (`df821d1`). **Epic corpus + Phase A + B1 + B2 reskin + mandatory tutor 2FA live ONLY here** until epic merges to `master`. **PRODUCTION:** apply **both** p1 migrations (`20260531180000` + `20260601120000`) at/after next deploy. **Phase-2 auth/session design added 2026-06-01** — see sub-pass tracker row above + [`docs/handoff/identity-phase2-auth-session-design-2026-06-01.md`](identity-phase2-auth-session-design-2026-06-01.md). |
...
| **`identity-p2-schema`** | `e305d0b` | **PUSHED-BUT-HELD — NOT merged.** Migration `20260531190000_identity_p2_principals`. **Identity Phase 2** — additive 3-principal schema only. **Gate:** deliberate **AccountHolder auth/session design step with Andrew** before merge (not autonomous overnight work). |
| **`identity-p2-ownership-guard`** | `f74f164` | **PUSHED-BUT-HELD — NOT merged.** `assertOwnsLearnerProfile` + 14 tests @ `f74f164`. **Gate:** same as p2-schema — merge only after AccountHolder session pillar is designed with Andrew, then `identity-p2-schema` merge first. |
...
> **ORCHESTRATOR NOTE — next identity step (DEFERRED, do NOT improvise):** **AccountHolder authentication / session infrastructure** so Phase-2 data routes can authenticate the cookie, extract `accountHolderId`, and call `assertOwnsLearnerProfile`. This is a **new auth pillar**, **high blast-radius** — requires a **deliberate design pass with Andrew**; not autonomous overnight work.

> **ORCHESTRATOR — migration merge-order (F12):** **p1 DONE on `v1-redesign`** (`20260531180000` + `20260601120000` applied on branch). **When p2 merges:** add `20260531190000_identity_p2_principals` — Prisma apply order on full epic will be `180000` (p1) → `190000` (p2) → `0601` (p1 pending-secret) because `0601` timestamp sorts after `190000`. All additive — order is safe. **PROD:** deploy both p1 migrations on next `v1-redesign` production deploy. **p2 + ownership-guard:** HOLD until AccountHolder session design with Andrew.

> **Overnight autonomous mode (2026-06-01):** **`identity-p1-2fa` + `component-b2` merged to `v1-redesign` after Andrew smoke PASS.** **NOT merged:** `identity-p2-schema`, `identity-p2-ownership-guard`. **NO merge to master/prod** for the epic yet.
...
| **`identity-p2-ownership-guard`** | `f74f164` | **PUSHED-BUT-HELD** — `assertOwnsLearnerProfile` + 14 tests; merge gated on AccountHolder session design with Andrew. |
| **`identity-p2-schema`** | `e305d0b` | **PUSHED-BUT-HELD** — additive 3-principal schema; same gate. |
...
**Headline Sonnet pass — LANDED 2026-05-31** (`23c65c0`) → [`docs/handoff/session-lifecycle-consent-design-2026-05-31.md`](session-lifecycle-consent-design-2026-05-31.md). **Implementation is downstream** (Identity Phases 2–4 per design §8); **decision-unblocked 2026-05-31** (essentials-vs-optional split RATIFIED; H-1/H-2 RESOLVED; §9 accepted). Remaining gates: legal/umbrella disclosure floor; session-lifecycle Phase-3 BLOCKERs; **AccountHolder session infra (design pass with Andrew — required before p2 merges).** **`assertOwnsLearnerProfile` SATISFIED on branch, not merged** (`identity-p2-ownership-guard` @ `f74f164`). **TOTP encrypt-at-rest SATISFIED+MERGED** (`identity-p1-2fa` → `v1-redesign` @ `b5ef4fe`, smoke 2026-06-01).
...
| Identity Phase 2 (additive schema foundation) | **DELIVERED** on sub-branch `identity-p2-schema` @ `e305d0b` (pushed, **NOT merged**) | Scope: 6 new Prisma models (`AccountHolder`, `AccountHolderEmailToken`, `LearnerProfile`, `LearnerCredential`, `LearnerDeviceSession`, `StudentClaimInvite`) + `AccountHolderEmailTokenPurpose` enum; `Student.learnerProfileId String? @unique` (nullable, `onDelete: SetNull`) + `claimInvites` back-relation; COPPA tombstone columns (`tombstonedAt` on AccountHolder + LearnerProfile; `Restrict` on AccountHolder→LearnerProfile delete so PII must be tombstoned first); migration `20260531190000_identity_p2_principals`. **Purely additive, ZERO route wiring** — all new tables empty, all `learnerProfileId` NULL. Deviation: `StudentClaimInvite` tutor FK is `adminUserId` (relation `"TutorClaimInvites"`), matching the repo's universal AdminUser FK convention. **Ownership assertion** delivered separately on `identity-p2-ownership-guard` @ `f74f164` (see row below). Verification: `npx next build` exit 0, `tsc` exit 0, jest 1383/1384 (same 1 pre-existing `auth.test.ts` fail). **Status: awaiting merge (after `identity-p1-2fa` per F12) + ultimately Andrew Phase-1 smoke gate.** |
| Identity Phase 2 ownership guard (`assertOwnsLearnerProfile`) | **DELIVERED** on sub-branch `identity-p2-ownership-guard` @ `f74f164` (pushed, **NOT merged**) | `assertOwnsLearnerProfile(accountHolderId, learnerProfileId)` in `src/lib/learner-profile-scope.ts` + 14 tests. **Ownership semantics enforced:** AccountHolder owns LearnerProfile via `accountHolderId`; cross-tenant mismatch and `tombstonedAt != null` deny (COPPA-deleted treated as non-existent even to owner); deny-by-default `notFound()` (mirrors `assertOwnsStudent`). **`lpr=`** logs on denial. Guard is **parameterized** (no session/cookie reads) — AccountHolder session infrastructure does not exist yet; Phase-2 data routes will authenticate, extract `accountHolderId`, then call this guard. Gates: `tsc` + `next build` exit 0; 14 ownership tests + 92 regression green. **Awaiting:** merge after p1 smoke + `identity-p2-schema` merge (backend, test-gated, low-risk). |
| **Identity Phase-2 AUTH + SESSION INFRASTRUCTURE design** | **DONE** @ `v1-redesign` (this commit) | Design-only pass covering: realm fork (SEPARATE realm recommended, AH-1), DB-backed `AccountHolderSession` table (AH-2), PIN soft-lockout policy (AH-4), sliding renewal (AH-5), session lifetime (AH-6), claim flow state machine (states/transitions/idempotency/multi-child), privilege-confusion prevention invariants (I-1..I-6), `assertIsSessionParticipant` + `assertEffectiveConsent` + `assertOwnsConsentRecord` guard catalog, schema additions required (new `AccountHolderSession` model, `expiresAt` gap on `LearnerDeviceSession`, `payload`+`targetLearnerProfileId` on `AccountHolderEmailToken`, `tokenHash` rename on `StudentClaimInvite`), P2a/P2b/P2c sub-batch plan, 5-axis adversarial review with 7 BLOCKERs folded into P2a acceptance. **7 Andrew-decisions (AH-1..AH-7) in §0 need ratification before P2a dispatch.** Detail: [`docs/handoff/identity-phase2-auth-session-design-2026-06-01.md`](identity-phase2-auth-session-design-2026-06-01.md) |
...
>
> **Phase 2 merges still HOLD** — require AccountHolder auth/session design step with Andrew before `identity-p2-schema` / `identity-p2-ownership-guard`.
...
- ~~`assertOwnsLearnerProfile` exists + negative-tested **before Phase 2 data routes**.~~ **SATISFIED on branch, NOT merged** (`identity-p2-ownership-guard` @ `f74f164` — 14 tests; closes on merge after `identity-p2-schema` + AccountHolder session design with Andrew).
...
> **2026-06-01:** Identity Phase 1 **MERGED** @ `b5ef4fe`; Component Phase B2 **MERGED** @ `0424206` (reskin floor, smoked as-scoped). Phase 2 branches **NOT merged** — gate on AccountHolder auth/session design with Andrew.

**Andrew's queue:**

1. **PROD deploy hygiene:** on next `v1-redesign` production deploy, **`prisma migrate deploy`** both p1 migrations (`20260531180000` + `20260601120000`). Confirm `TOTP_ENCRYPTION_KEY` on all Vercel envs.
2. **Phase-2 identity — RATIFY 7 DECISIONS (AH-1..AH-7) in [`docs/handoff/identity-phase2-auth-session-design-2026-06-01.md`](identity-phase2-auth-session-design-2026-06-01.md) §0 before P2a dispatch.** Recommendations are recommend-and-proceed; Andrew can accept all 7 in one pass. After ratification: merge `identity-p2-schema` → merge `identity-p2-ownership-guard` → dispatch P2a (see sub-batch plan in design doc §10.2). Key decisions: **AH-1** separate realm (HIGH blast radius if wrong); **AH-2** DB-backed session; **AH-4** never hard-lock a child.
3. **Component next:** Phase **B3** (session list / billing log UI) or **Phase D** gap-close design pass (landing/hero + `/about` — V1-required per 2026-06-01 decision; see component doc Phase D). Nav redesign stays bundled with B3–B6 surface work, not pulled forward.
4. **Session-lifecycle + consent implementation — DECISION-UNBLOCKED (2026-05-31):** dispatch Identity Phases 3–4 per design §8 **after** p2 schema + ownership guard merged AND p2 auth/session infrastructure (P2a) live.
5. **BLOCKERs (active):** umbrella deploy + OpenAI DPA + functioning deletion-request inbox; session-lifecycle Phase-3 BLOCKERs; identity Phase-2 auth/session design ratification (AH-1..AH-7).
6. **Andrew parallel:** interim `interim-capture-attestation` migrate → smoke → merge; umbrella `coppa-312-10-disclosure` review+deploy; Phase A + B1 real-hardware smoke if not done; B2 follow-ups in BACKLOG; optional `npm run test:visual:update` for B2 snapshots.
7. **Sarah-pending:** test-students audit.
8. **Solo in-person recording (V1 prod goal):** B-5 acceptance before `soloEnabled` in production.
9. Keep this spine + bootstrapper in sync at every handoff.
