...
| **`v1-redesign`** | `f41a445` (advances with P2a dispatch) | Long-running V1 epic integration branch. **`identity-p1-2fa` MERGED `--no-ff` 2026-06-01** @ `b5ef4fe` (smoked @ `d782430`). **`component-b2-dashboard-students` MERGED `--no-ff` 2026-06-01** @ `0424206` (smoked as-scoped reskin floor). **Identity Phase-2 auth/session design RATIFIED 2026-06-01** — AH-1..AH-7 + amendments in [`identity-phase2-auth-session-design-2026-06-01.md`](identity-phase2-auth-session-design-2026-06-01.md). **P2a next:** merge held p2 branches (see migration-ordering hazard) → session-infra build. **Epic corpus + Phase A + B1 + B2 reskin + mandatory tutor 2FA live ONLY here** until epic merges to `master`. **PRODUCTION:** apply **both** p1 migrations (`20260531180000` + `20260601120000`) at/after next deploy. |
...
| **`identity-p2-schema`** | `e305d0b` | **PUSHED-BUT-HELD — NOT merged.** Migration `20260531190000_identity_p2_principals`. **Gate cleared 2026-06-01** (design ratified). **Merge first** in P2a sequence; check migration-ordering hazard vs `20260601120000`. |
| **`identity-p2-ownership-guard`** | `f74f164` | **PUSHED-BUT-HELD — NOT merged.** `assertOwnsLearnerProfile` + 14 tests. **Merge second** after `identity-p2-schema`. |
...
> **ORCHESTRATOR — next identity step:** **P2a build** — design **RATIFIED 2026-06-01** ([`identity-phase2-auth-session-design-2026-06-01.md`](identity-phase2-auth-session-design-2026-06-01.md)). Merge `identity-p2-schema` → `identity-p2-ownership-guard` **as-is (serial)** → dispatch P2a session-infra (7 BLOCKERs + no-dup-code + connect interstitial in acceptance). **Do NOT** fold p2-schema into a mega-migration (AH-7).

> **ORCHESTRATOR — migration merge-order + HAZARD:** **p1 DONE on `v1-redesign`** (`20260531180000` + `20260601120000`; `0601` already applied on preview-dev). **HAZARD:** `identity-p2-schema` migration `20260531190000` timestamps **before** `20260601120000` (2FA). Prisma may treat `190000` as already "past" relative to applied history. **P2a merge executor MUST** inspect `_prisma_migrations` on preview-dev and decide whether to **bump** the p2-schema migration timestamp for monotonic ordering **before** merge — **not resolved in docs** (recorded only). All migrations additive. **PROD:** deploy p1 migrations on next `v1-redesign` production deploy if not yet applied.
...
| **`identity-p2-ownership-guard`** | `f74f164` | **PUSHED-BUT-HELD** — merge after p2-schema (design ratified 2026-06-01). |
| **`identity-p2-schema`** | `e305d0b` | **PUSHED-BUT-HELD** — merge first in P2a sequence (migration-ordering hazard — see orchestrator note). |
...
| **Identity Phase 2a (build — session infra + claim back-end)** | **PENDING** | **Sequence:** merge `identity-p2-schema` @ `e305d0b` → merge `identity-p2-ownership-guard` @ `f74f164` (**inspect migration-ordering hazard first** — `20260531190000` vs `20260601120000`) → P2a branch: `AccountHolderSession`, `LearnerDeviceSession.expiresAt`, `AccountHolderEmailToken.payload` + `.targetLearnerProfileId`, `StudentClaimInvite.token`→`tokenHash`, shared auth primitives + thin realm adapters, claim/connect back-end (tutor-initiated + existing-account path w/ interstitial), child login back-end, guards + middleware. **Acceptance:** 7 BLOCKERs (design §12) + no duplicated auth logic + connect identity interstitial. No UI (P2b). Design §10.2. |
...
- ~~`assertOwnsLearnerProfile` exists + negative-tested **before Phase 2 data routes**.~~ **SATISFIED on branch, NOT merged** (`identity-p2-ownership-guard` @ `f74f164` — 14 tests; closes on merge after `identity-p2-schema`; design ratified 2026-06-01).
...
> **2026-06-01:** Identity Phase 1 **MERGED** @ `b5ef4fe`; Component Phase B2 **MERGED** @ `0424206`; **Identity Phase-2 auth/session design RATIFIED** (AH-1..AH-7 + amendments @ design doc [RATIFIED + AMENDED](identity-phase2-auth-session-design-2026-06-01.md#ratified--amended-andrew-2026-06-01)).

**Orchestrator / executor queue:**

1. **P2a (identity build):** merge held branches **as-is, serial** — `identity-p2-schema` @ `e305d0b` → `identity-p2-ownership-guard` @ `f74f164`. **Before merge:** inspect `_prisma_migrations` on preview-dev for **migration-ordering hazard** (`20260531190000` timestamps before applied `20260601120000`); bump p2-schema timestamp if needed for monotonic timeline. Then P2a branch: AccountHolder/child session infra + claim/connect back-end with **7 BLOCKERs** (S1, S2, S3, R1, C1, A1, O1) + **no duplicated auth logic** + **connect-link identity interstitial** in acceptance.
2. **Schema gaps in P2a migration (additive):** `AccountHolderSession` (new table); `LearnerDeviceSession.expiresAt` (missing on held schema); `StudentClaimInvite.token` → `tokenHash`; `AccountHolderEmailToken.payload` + `.targetLearnerProfileId`; per-`LearnerProfile` child access mode field(s). See design doc §9 + [RATIFIED + AMENDED](identity-phase2-auth-session-design-2026-06-01.md#ratified--amended-andrew-2026-06-01).
3. **PROD deploy hygiene:** on next `v1-redesign` production deploy, **`prisma migrate deploy`** both p1 migrations (`20260531180000` + `20260601120000`) if not applied. Confirm `TOTP_ENCRYPTION_KEY` on all Vercel envs; reserve `AH_SESSION_HMAC_SECRET`, `LEARNER_SESSION_HMAC_SECRET`, `AH_TOTP_ENCRYPTION_KEY` for P2a.
