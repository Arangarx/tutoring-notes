...
| **`identity-p2-ownership-guard`** | `f74f164` | Branched off `identity-p2-schema` @ `e305d0b`. Pushed, **NOT merged.** **`assertOwnsLearnerProfile(accountHolderId, learnerProfileId)`** in `src/lib/learner-profile-scope.ts` + **14 tests** (`assertOwnsLearnerProfile.test.ts`). Deny-by-default via `notFound()` (anti-enumeration); denies not-found, cross-tenant `accountHolderId` mismatch, and `tombstonedAt != null`. **`lpr=`** denial logs. **Parameterized** (no session reads) — AccountHolder session infra not built yet. **HOLD** smoke-ready backend branch; **merge after** p1 smoke + `identity-p2-schema` merge. |
...
**Policy:** BUILD-DON'T-MERGE — branches pushed, not merged to `v1-redesign` or `master`.

**Smoke-ready branches delivered overnight:**

| Branch | SHA | Delivered |
|--------|-----|-----------|
| **`identity-p1-2fa`** | `45c01fa` | Full mandatory tutor/admin TOTP 2FA + `/admin/settings/2fa` management page (pending-secret migration, impersonation/sign-out fixes, reenroll-trap closed). |
| **`identity-p2-ownership-guard`** | `f74f164` | `assertOwnsLearnerProfile` ownership guard + 14 tests (branched off `identity-p2-schema`). |

**Docs / spine on `v1-redesign`:** consolidations through HEAD **`7f3745d`** (this wrap advances the spine). **`preview-dev`:** identity p1 migrations applied (verified on `45c01fa` Vercel build).

**Park:** Opus stopped after this wrap (safe-unattended limit reached). Remaining work is **visual-smoke-gated** (`component-b2`) or **design-gated** (AccountHolder session pillar — see ORCHESTRATOR NOTE above).
...
>
> **Consolidated smoke @ `identity-p1-2fa` `45c01fa` (latest preview):** fresh enroll (email-labeled QR) → Enter-submits → backup codes (Copy + Download) → login lands on dashboard → Settings → **2FA management page** (status, Rotate authenticator no-lockout, Regenerate backup codes) → admin Reset → exit-impersonation (no 2FA re-prompt) → sign-out-while-impersonating == exit. Ratify **F1–F4**. On pass: `merge --no-ff identity-p1-2fa` into `v1-redesign` (**apply both p1 migrations to PROD** at/after merge), then `merge --no-ff identity-p2-schema`, then `merge --no-ff identity-p2-ownership-guard` @ `f74f164`.
...
### BLOCKERs (1 active + 3 satisfied-pending-merge)
...
- ~~`assertOwnsLearnerProfile` exists + negative-tested **before Phase 2 data routes**.~~ **SATISFIED on `identity-p2-ownership-guard` @ `f74f164`** (`src/lib/learner-profile-scope.ts` + 14 tests; formally closes on `merge --no-ff` after `identity-p2-schema`). Ownership: AccountHolder owns LearnerProfile via `accountHolderId`; tombstone denies all callers. Guard **parameterized** pending AccountHolder session infra.
...
2. **On smoke pass:** `merge --no-ff identity-p1-2fa` → `v1-redesign` (**PROD: apply both p1 migrations** `20260531180000` + `20260601120000`), then `merge --no-ff identity-p2-schema` @ `e305d0b`, then `merge --no-ff identity-p2-ownership-guard` @ `f74f164` (backend ownership guard — test-gated, low-risk; no extra migration).
...
4. **More overnight branches may exist by morning** — check **BRANCH & COMMIT REALITY** (orchestrator appends; overnight summary above).
...
4. **Session-lifecycle + consent implementation — DECISION-UNBLOCKED (2026-05-31):** essentials-vs-optional split RATIFIED; H-1/H-2 RESOLVED; consent-gates §9 accepted. Dispatch Identity Phases 3–4 per design §8 when Phase-2 schema + ownership guard merged. **Remaining gates:** legal/umbrella disclosure floor; session-lifecycle Phase-3 acceptance BLOCKERs (table above); **AccountHolder session infra** (design pass with Andrew — do NOT improvise).
5. **BLOCKERs remain (active):** umbrella deploy + OpenAI DPA + functioning deletion-request inbox (disclosure floor); session-lifecycle Phase-3 BLOCKERs. **Satisfied-pending-merge:** TOTP encrypt-at-rest on `identity-p1-2fa`; Phase-2 additive schema on `identity-p2-schema`; `assertOwnsLearnerProfile` on `identity-p2-ownership-guard` @ `f74f164`.
