...
| Identity Phase 1 (mandatory tutor/admin TOTP 2FA) | **DELIVERED** on sub-branch `identity-p1-2fa` @ `4aeeb28` (pushed, **NOT merged**) | `AdminUser2FA` + `AdminUser2FABackupCode` additive migration; AES-256-GCM encrypt-at-rest ([`src/lib/crypto/totp-secret.ts`](../../src/lib/crypto/totp-secret.ts), key from `TOTP_ENCRYPTION_KEY`); backup codes (bcrypt hash, shown once); ADMIN-only reset (`adminResetTwoFactor`); middleware gate (non-test TUTOR/ADMIN must enroll+verify; `isTestAccount`/`isImpersonating`/env-`sub=admin` exempt); routes under `/admin/settings/2fa/*`; `tfa=` log prefix. Verification: `npx next build` exit 0, `tsc` exit 0, jest **1384 pass** (1 **PRE-EXISTING** unrelated fail in `auth.test.ts` — uses `file:./test.db` on a PG-only schema; confirmed failing before this branch). **Status: awaiting Andrew env-var + smoke + `merge --no-ff` to `v1-redesign` (before p2 — see F12 merge-order).** |
...
| Component Phase B2 (dashboard / student list / detail) | **UNBLOCKED — next build step** | Dashboard + tutor student-list + tutor student-detail redesign per component-redesign design. Identity additive schema exists on `identity-p2-schema`; tutor-facing pages keep current semantics (`learnerProfileId` unused today) — redesign now carries no rework; claim/consent affordances layer on additively later. **Dispatch next on its own sub-branch** (off current `v1-redesign` HEAD after spine commit). Behavioral identity work (claim flow, AccountHolder auth/sessions, `assertOwnsLearnerProfile` in routes, consent enforcement) remains gated on Andrew Phase-1 smoke + ratification. |
...
1. **Identity Phase 1 (Andrew, blocking merge):** set `TOTP_ENCRYPTION_KEY` on Vercel (all envs) → smoke `identity-p1-2fa` @ `4aeeb28` on real hardware → ratify F1–F4 → `merge --no-ff` to `v1-redesign` **first** (before p2 — F12 merge-order). See **ACTION (Andrew) — Identity Phase 1 smoke gate** above.
2. **Identity Phase 2 (delivered, awaiting merge):** additive schema on `identity-p2-schema` @ `e305d0b` (pushed, NOT merged). After p1 merges: `merge --no-ff identity-p2-schema` into `v1-redesign`. Purely additive — zero behavior change. `assertOwnsLearnerProfile` deferred to claim-flow enforcement pass.
3. **Component Phase B2 (next build dispatch):** dashboard + tutor student-list + tutor student-detail redesign — **UNBLOCKED** (identity schema exists; tutor pages unchanged semantically; claim/consent layers on later). Dispatch on its own sub-branch off `v1-redesign` HEAD. **B1 awaiting Andrew:** real-hardware smoke redesigned auth on v1-redesign preview; confirm Connect Gmail notice copy if desired; Playwright `--update-snapshots` + possible `color-contrast` findings on non-auth admin pages.
4. **Session-lifecycle + consent implementation — DECISION-UNBLOCKED (2026-05-31):** essentials-vs-optional split RATIFIED; H-1/H-2 RESOLVED; consent-gates §9 accepted. Dispatch Identity Phases 3–4 per design §8 when Phase-2 schema merged + BLOCKERs clear. **Remaining gates:** `assertOwnsLearnerProfile` before behavioral identity routes; legal/umbrella disclosure floor; session-lifecycle Phase-3 acceptance BLOCKERs (table above). **Behavioral identity** (claim flow, AccountHolder auth/sessions, consent enforcement) gated on Andrew Phase-1 smoke + ratification.
