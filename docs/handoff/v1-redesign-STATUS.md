...
- **Sarah pilot inputs (2026-05-31, validated):**
  ...
  - **Whiteboard near-full-screen / maximize canvas real estate, minimal chrome (Sarah-implied, Andrew 2026-06-01):** first-class V1 layout principle — informs component/layout phase (component doc §5.0), not necessarily immediate build.
...
| **`v1-redesign`** | `48e82ed` → advances with this spine commit | Long-running V1 epic integration branch. Advanced through design-integration commits + **Component Phase A** (`5aa3c7d`) + **Component Phase B1** (`b798494`, pushed) + notice-fix (`38a849a`) + legal facades (`29f5e88`). **This spine + bootstrapper + consent/identity/component/session-lifecycle design docs + Phase A (dark tokens + fonts) + Phase B1 (Tailwind 4.3 + shadcn + auth surfaces) + product `/privacy` + `/terms` facades live ONLY here** until the epic merges. **B1 Vercel deploy VERIFIED READY** (`dpl_2LK8drfHx8tuVhVMsxhVSjFvrbjH`). **Component B2 delivered on sub-branch** `component-b2-dashboard-students` @ `20de6fa` (pushed, NOT merged — see row below). |
| **`component-b2-dashboard-students`** | `20de6fa` | Branched off `v1-redesign` `4ea523e`. Pushed, **NOT merged.** **UNGATED** — Andrew can smoke + `merge --no-ff` anytime; **does NOT** depend on identity Phase-1 smoke or F12 merge order (no migrations). **Component Phase B2** — admin nav/layout, dashboard, student-list + detail (see sub-pass tracker). **Vercel deploy VERIFIED READY** (`dpl_83VnYBd7DZGfdsJAXEWGq2asjX5m`). |
| **`identity-p1-2fa`** | `3ea6b72` (HEAD) | Branched off `v1-redesign` `165084e`. Pushed, **NOT merged.** Migration `20260531180000_admin_user_2fa`. **Identity Phase 1** — mandatory tutor/admin TOTP 2FA (see sub-pass tracker). **Smoke-fix progression:** `2e8e641` (QR-local `data:` URI + notice removed) → `9c17a16` (session-mint fix + UX) → `3ea6b72` (email QR label). **`9c17a16` headline:** valid Google Authenticator codes looked rejected (endless `/verify` re-prompt) — **not TOTP math**; `verifyTotpCode` called next-auth v4 `getToken({ req: fakeReq })` but v4 reads `req.cookies`, not `req.headers.get('cookie')`, so `mintTwoFactorVerifiedSession` never ran; fixed via `cookies()` + `decode()`; independent-oracle test (raw HMAC-SHA1, no OTPAuth self-ref). **`9c17a16` UX:** Enter submits setup-confirm + verify; backup-codes Copy + Download `.txt`; dedicated `2fa:<ip>` bucket **20 req/min** (login auth bucket stays **10/min**). **`3ea6b72`:** QR account label = admin **email** (`Mynk: <email>`; new QRs only). **GAP `p1-reenroll-trap` (decision pending):** incomplete enrollment traps on `/verify` with no self-service re-QR (only `adminResetTwoFactor`, no UI) — see sub-pass tracker. **Awaiting:** Andrew re-smoke @ `3ea6b72` (recommend clean re-enroll Option B) → ratify F1–F4 → `merge --no-ff` **before** `identity-p2-schema`. |
| **`identity-p2-schema`** | `e305d0b` | Branched off `v1-redesign` `38a849a`. Pushed, **NOT merged.** Migration `20260531190000_identity_p2_principals`. **Identity Phase 2** — additive 3-principal schema only (see sub-pass tracker). **Awaiting:** `merge --no-ff` to `v1-redesign` **after** `identity-p1-2fa` (migration monotonicity) + ultimately Andrew Phase-1 smoke gate. |
...
> **ORCHESTRATOR — migration merge-order (F12):** `identity-p1-2fa` migration is `20260531180000_admin_user_2fa`; `identity-p2-schema` is `20260531190000_identity_p2_principals`. To keep migration history monotonic on `v1-redesign`, **`merge identity-p1-2fa into v1-redesign FIRST**, then `identity-p2-schema`**. If p2 merges first anyway, **bump the p1-2fa migration timestamp** before merging p1. Both identity branches gate on Andrew's Phase-1 smoke (`TOTP_ENCRYPTION_KEY` + real-hardware 2FA flow + F1–F4 ratification).
...
| Identity Phase 1 (mandatory tutor/admin TOTP 2FA) | **DELIVERED** on sub-branch `identity-p1-2fa` @ `3ea6b72` (pushed, **NOT merged**) | `AdminUser2FA` + `AdminUser2FABackupCode` additive migration; AES-256-GCM encrypt-at-rest ([`src/lib/crypto/totp-secret.ts`](../../src/lib/crypto/totp-secret.ts), key from `TOTP_ENCRYPTION_KEY`); backup codes (bcrypt hash, shown once); ADMIN-only reset (`adminResetTwoFactor`); middleware gate (non-test TUTOR/ADMIN must enroll+verify; `isTestAccount`/`isImpersonating`/env-`sub=admin` exempt); routes under `/admin/settings/2fa/*`; `tfa=` log prefix; enrollment QR via server-side `qrcode` `data:` URI (no third-party QR host). **Smoke-found + fixed @ `2e8e641`:** (1) **SECURITY** — `api.qrserver.com` QR leaked plaintext `otpauth` (CSP blocked); fixed with server-side `data:` URI + grep guard. (2) **ACCURACY** — Mortensen Apps notice removed from 2FA setup/verify. **Late-night smoke @ `9c17a16`:** (3) **HEADLINE** — valid authenticator codes looked rejected; root cause **session minting** (`getToken` on fake req never saw cookies → `twoFactorVerified` never set); fixed `cookies()` + `decode()`; independent-oracle TOTP test (raw HMAC-SHA1, not self-referential). (4) **UX** — Enter submits setup-confirm + verify; backup-codes Copy + Download `.txt`; `2fa:<ip>` rate limit **20/min** (login **10/min** unchanged). **`3ea6b72`:** QR label = admin **email** (`Mynk: <email>`; new QRs only). **GAP `p1-reenroll-trap` (PENDING Andrew):** incomplete enrollment (`lastVerifiedAt` NULL, 0 backup codes) traps on `/verify` — no self-service re-QR; only admin reset server action (no UI). Surfaced when Andrew's first (broken) attempt left a row; orchestrator unblocked smoke via greenlit DELETE on Neon **preview-dev** (`br-crimson-mode-amape02v` — previews' DB; read-only SELECTs OK without greenlight). **Fix options:** treat unconfirmed enrollment as re-enrollable and/or ship admin Reset-2FA UI — **pre-merge vs fast-follow TBD.** **Status:** preview READY @ `3ea6b72` — Andrew **clean re-enroll (Option B)** → ratify F1–F4 → `merge --no-ff` before p2. |
...
>
> Then: re-smoke the 2FA flow on real hardware @ `identity-p1-2fa` **`3ea6b72`** (recommend **clean re-enroll, Option B**, if a prior broken attempt left an incomplete `AdminUser2FA` row) → ratify **F1–F4** → decide **`p1-reenroll-trap`** timing → `git merge --no-ff identity-p1-2fa` into `v1-redesign`.
...
- **`p1-reenroll-trap` (Identity Phase 1, PENDING):** incomplete 2FA enrollment traps user on `/verify` with no self-service path to a fresh QR — only `adminResetTwoFactor` (no UI). **Andrew to decide:** fix before `identity-p1-2fa` merge (treat unconfirmed as re-enrollable and/or admin Reset-2FA UI) vs fast-follow.
...
> **AUTONOMOUS-DRIVE PARK POINT (2026-05-31):** The 2026-05-31 autonomous drive reached a **park point** after delivering: **B1** (smoked, on `v1-redesign`), **notice-fix** (`38a849a`), **Identity Phase 1 2FA** (`identity-p1-2fa`, incl. smoke-found QR egress + notice fixes @ `2e8e641` + late-night session-mint/UX/email-label fixes through `3ea6b72`), **Identity Phase 2 additive schema** (`identity-p2-schema` @ `e305d0b`), **Component B2** (`component-b2-dashboard-students` @ `20de6fa`). **ALL remaining V1 build work is GATED on Andrew:** behavioral identity (claim flow, AccountHolder auth/sessions, `assertOwnsLearnerProfile` in routes, consent enforcement — Phases 2-behavioral/3/4) requires Phase-1 re-smoke + F1–F4 ratification first. No further autonomous identity/component dispatch until Andrew unblocks.

**Andrew's queue (unblocks the rest):**

1. Set `TOTP_ENCRYPTION_KEY` on Vercel **all envs** (see **ACTION (Andrew) — Identity Phase 1 smoke gate** above).
2. Re-smoke `identity-p1-2fa` @ **`3ea6b72`** (preview READY) — real-hardware 2FA flow; **clean re-enroll (Option B)** if prior incomplete enrollment row → ratify **F1–F4** → decide **`p1-reenroll-trap`** (pre-merge fix vs fast-follow).
3. `merge --no-ff identity-p1-2fa` into `v1-redesign` **first** (F12 merge-order), then `merge --no-ff identity-p2-schema` @ `e305d0b`.

**Independently smokeable/mergeable now (UNGATED):** Component **B1** on `v1-redesign`; Component **B2** on `component-b2-dashboard-students` @ `20de6fa` (no migrations; not blocked by identity smoke).
