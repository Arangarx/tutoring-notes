...
| **`identity-p1-2fa`** | `2e8e641` | Branched off `v1-redesign` `165084e`. Pushed, **NOT merged.** Migration `20260531180000_admin_user_2fa`. **Identity Phase 1** — mandatory tutor/admin TOTP 2FA (see sub-pass tracker). **Smoke-found + fixed @ `2e8e641`:** (1) enrollment QR no longer uses `api.qrserver.com` — plaintext TOTP secret was egressing to a third party (CSP blocked the image); fixed with server-side `qrcode` `data:` URI + grep guard; (2) Mortensen Apps notice removed from 2FA setup/verify (usemynk.com 2FA, not Mortensen OAuth). **Awaiting:** re-smoke from top once new preview READY → ratify F1–F4 → `merge --no-ff` to `v1-redesign` **before** `identity-p2-schema` (see merge-order note below). |
...
| Identity Phase 1 (mandatory tutor/admin TOTP 2FA) | **DELIVERED** on sub-branch `identity-p1-2fa` @ `2e8e641` (pushed, **NOT merged**) | `AdminUser2FA` + `AdminUser2FABackupCode` additive migration; AES-256-GCM encrypt-at-rest ([`src/lib/crypto/totp-secret.ts`](../../src/lib/crypto/totp-secret.ts), key from `TOTP_ENCRYPTION_KEY`); backup codes (bcrypt hash, shown once); ADMIN-only reset (`adminResetTwoFactor`); middleware gate (non-test TUTOR/ADMIN must enroll+verify; `isTestAccount`/`isImpersonating`/env-`sub=admin` exempt); routes under `/admin/settings/2fa/*`; `tfa=` log prefix; enrollment QR via server-side `qrcode` `data:` URI (no third-party QR host). **Smoke-found + fixed @ `2e8e641`:** (1) **SECURITY** — initial build rendered enrollment QR via `api.qrserver.com` with plaintext `otpauth` secret in the URL (CSP `img-src` blocked it); fixed locally + grep guard. (2) **ACCURACY** — Mortensen Apps notice wrongly on 2FA setup/verify screens; removed (2FA is usemynk.com, not Mortensen OAuth). Verification: `npx next build` exit 0, `tsc` exit 0, jest **1384 pass** (1 **PRE-EXISTING** unrelated fail in `auth.test.ts` — uses `file:./test.db` on a PG-only schema; confirmed failing before this branch). **Status: awaiting Andrew re-smoke from top once preview @ `2e8e641` READY → ratify F1–F4 → `merge --no-ff` to `v1-redesign` (before p2 — see F12 merge-order).** |
...
- ~~**Q-5 — Tutor 2FA recovery**~~ **LOCKED Option 1:** backup codes → **admin reset**; **NO email self-serve** first pass. Phase 1 policy gate cleared. **TOTP-encrypt-at-rest BLOCKER SATISFIED** on `identity-p1-2fa` @ `2e8e641` — acceptance **strengthened** to include no plaintext secret egress to third-party services (formally closes when branch merges to `v1-redesign`).
...
- ~~TOTP secrets encrypted at rest **before Phase 1**.~~ **SATISFIED on `identity-p1-2fa` @ `2e8e641`** (AES-256-GCM via `TOTP_ENCRYPTION_KEY`; acceptance **strengthened** to include no plaintext secret egress to third-party services — smoke caught `api.qrserver.com` QR leak, fixed @ `2e8e641`; formally closes on `merge --no-ff` to `v1-redesign`).
...
2. Re-smoke `identity-p1-2fa` @ `2e8e641` from top once preview READY — real-hardware 2FA flow → ratify **F1–F4**.
