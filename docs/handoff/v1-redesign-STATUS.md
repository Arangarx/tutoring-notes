...
| **`v1-redesign`** | `b5ef4fe` → advances with this spine commit | Long-running V1 epic integration branch. **`identity-p1-2fa` MERGED `--no-ff` 2026-06-01** (smoked + approved @ `d782430`). Also: design-integration + **Component Phase A** (`5aa3c7d`) + **Component Phase B1** (`b798494`) + notice-fix (`38a849a`) + legal facades (`29f5e88`) + whiteboard viewport layout principle (`0bf2ea0`) + overnight p2 ownership-guard spine (`df821d1`). **Epic corpus + Phase A + B1 + mandatory tutor 2FA live ONLY here** until epic merges to `master`. **B1 Vercel deploy VERIFIED READY** (`dpl_2LK8drfHx8tuVhVMsxhVSjFvrbjH`). **PRODUCTION:** apply **both** p1 migrations (`20260531180000` + `20260601120000`) at/after this merge deploy. |
| **`component-b2-dashboard-students`** | `20de6fa` | Branched off `v1-redesign` `4ea523e`. Pushed, **NOT merged.** **UNGATED** on migrations — Andrew can smoke + `merge --no-ff` anytime. **Component Phase B2** — admin nav/layout, dashboard, student-list + detail (see sub-pass tracker). **Vercel deploy VERIFIED READY** (`dpl_83VnYBd7DZGfdsJAXEWGq2asjX5m`). **HOLD for Andrew visual smoke.** |
| **`identity-p1-2fa`** | `d782430` | **MERGED `--no-ff` into `v1-redesign` @ `b5ef4fe` (2026-06-01).** Andrew real-hardware smoke **PASSED** (QR gen, land-on-dashboard-after-signup, land-on-dashboard-after-login, exit-impersonation-keeps-2FA, sign-out-impersonation-exits, backup-code-hold + autofocus). Branch preserved for stale-sweep. |
...
> **Overnight autonomous mode (2026-06-01):** **`identity-p1-2fa` merged to `v1-redesign` after Andrew smoke PASS.** Still **HOLD:** `component-b2` (visual). **NOT merged:** `identity-p2-schema`, `identity-p2-ownership-guard`. **NO merge to master/prod** for the epic yet.
...
| **`component-b2-dashboard-students`** | `20de6fa` | **PUSHED-BUT-HELD** — awaiting Andrew visual smoke. |
...
> **2026-06-01:** Identity Phase 1 **MERGED** to `v1-redesign` @ `b5ef4fe` (smoke PASS, F1–F4 ratified). Phase 2 branches **explicitly NOT merged** — gate on AccountHolder auth/session design with Andrew.
...
2. **Phase-2 identity (HOLD):** schedule **AccountHolder auth/session pillar** design pass before merging `identity-p2-schema` @ `e305d0b` or `identity-p2-ownership-guard` @ `f74f164` — do NOT merge p2 branches until that design lands with Andrew.
3. **Component B2:** smoke + decide merge for `component-b2-dashboard-students` @ `20de6fa` (visual; independent of identity p2 gate).
4. **Session-lifecycle + consent implementation — DECISION-UNBLOCKED (2026-05-31):** dispatch Identity Phases 3–4 per design §8 **after** p2 schema + ownership guard merged. **Remaining gates:** legal/umbrella disclosure floor; session-lifecycle Phase-3 BLOCKERs; AccountHolder session infra.
5. **BLOCKERs (active):** umbrella deploy + OpenAI DPA + functioning deletion-request inbox; session-lifecycle Phase-3 BLOCKERs. **Satisfied-merged:** TOTP encrypt-at-rest. **Satisfied-on-branch-not-merged:** `assertOwnsLearnerProfile` @ `f74f164`.
6. **Andrew parallel (unchanged):** interim `interim-capture-attestation` migrate → smoke → merge; umbrella `coppa-312-10-disclosure` review+deploy; Phase A + **B1/B2** real-hardware smoke; pilot purge; optional B2 `npm run test:visual:update`.
