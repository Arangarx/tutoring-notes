...
| **IAC-13** | Intercepted claim link — tutor visibility + disconnect | **V1 SECURITY (2026-06-03):** Claim URL is a bearer token; interception can bind a minor's `Student` to the wrong `AccountHolder` (COPPA/privacy). **Required:** **(a)** tutor sees connected account (email/identity + connected-at); **(b)** tutor can sever the tie and revoke learner/AH sessions for that child; **(c)** retain single-use + expiry (`CLAIM_INVITE_TTL_MS` = 48h as of 2026-06-07). **Today:** (a)(b) missing; (c) partial — `claimedAt` single-use, `expiresAt` 48h, email verify on complete. Parent self-unlink **deferred.** Full spec: [`session-identity-access-design-2026-05-31.md`](session-identity-access-design-2026-05-31.md) §4.5; [`docs/BACKLOG.md`](../BACKLOG.md) § Identity / access. | Closes wrongful-claim recovery gap; defense-in-depth beyond bearer-token hygiene alone. |
