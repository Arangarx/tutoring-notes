...
| Customization | Rich customization API: `renderTopRightUI`, `renderBottomRightUI`, `UIOptions.tools` to hide/show tools, `initialData.appState` for defaults. Sarah's toolbar-reorder + default-draw-type requests are satisfiable with ~1 day of work. **CORRECTED 2026-06-07:** On pinned `@excalidraw/excalidraw` 0.18.1, `UIOptions.tools` only toggles the **image** tool — it cannot reorder the toolbar or regroup shape tools. Toolbar reorder, dropdown consolidation, and compact properties require hiding native Excalidraw UI and building custom chrome driving `excalidrawAPI`; default draw-type (pen width, roughness, font) remains cheap via `initialData.appState` / `updateScene`. |
