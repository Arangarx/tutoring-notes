...
>
> **Requirements cross-ref:** [`whiteboard-chrome-requirements.md`](whiteboard-chrome-requirements.md) (64 reqs). **Design cross-ref:** [`whiteboard-chrome-design-2026-06-07.md`](whiteboard-chrome-design-2026-06-07.md).
>
> **Audited:** 2026-06-08.
...
| T-12 | **Pen mode** | Ignore finger touches on tablet | Toolbar pen-mode toggle | Kb N | TM-04, TU-05 | **defer/open-question** — auto-detect vs Mynk toggle |
...
| T-16 | **Laser pointer** | Ephemeral laser trail for students | Extra-tools ▾; **K**; collab laser btn when `isCollaborating` | Kb Y | ST-05 | **Mynk toolbar** or **defer** Phase 3 alignment fix |
...
| P-03 | **Fill style** | Hachure / cross-hatch / solid | Properties | Kb N | — | **pulldown/overflow** “More styles” or **DROP** |
| P-04 | **Stroke width** | Line thickness presets | Properties | Kb N | DD-03, DD-04, PP-04 | **Mynk popover** (inline) |
| P-05 | **Stroke shape** (freedraw) | Pencil stroke profile | Properties (freedraw only) | Kb N | — | **DROP** or **More** if Sarah asks |
| P-06 | **Stroke style** | Solid / dashed / dotted | Properties | Kb N | — | **More styles** overflow |
| P-07 | **Sloppiness / roughness** | Architect / artist / cartoonist | Properties | Kb N | DD-01 | **Mynk popover** (default architect via `initialData`; expose in More) |
| P-08 | **Edges / roundness** | Sharp vs round corners | Properties | Kb N | DD-02 | **More styles** (default sharp via `initialData`) |
| P-09 | **Arrow type** | Round / sharp / elbow | Properties (arrows) | Kb N | — | **More styles** |
| P-10 | **Font family** | Virgil, Helvetica, etc. | Properties; **Shift+F** opens font popup | Kb partial | — | **More styles** (math uses Math insert) |
| P-11 | **Font size** | Point size | Properties; **Ctrl+Shift+,** / **Ctrl+Shift+.** | Kb Y | — | **More styles** |
| P-12 | **Text align** (horizontal) | Left / center / right | Properties | Kb N | — | **More styles** |
| P-13 | **Text align** (vertical) | Top / middle / bottom | Properties | Kb N | — | **DROP** or **More** |
| P-14 | **Arrowheads** (start/end) | Arrow cap styles | Properties (arrows/lines) | Kb N | — | **More styles** |
| P-15 | **Opacity** | Element opacity 0–100 | Properties | Kb N | PP-04 | **Mynk popover** (inline) |
| P-16 | **Send to back** | Z-order | Properties layers row; **Ctrl+Shift+[** (Win) / **Ctrl+Alt+[** (Mac) | Kb Y | — | **context-menu** or **keyboard-only** |
| P-17 | **Send backward** | Z-order −1 | Properties; **Ctrl+[** | Kb Y | — | **keyboard-only** |
| P-18 | **Bring forward** | Z-order +1 | Properties; **Ctrl+]** | Kb Y | — | **keyboard-only** |
| P-19 | **Bring to front** | Z-order max | Properties; **Ctrl+Shift+]** / **Ctrl+Alt+]** | Kb Y | — | **keyboard-only** |
...
| P-24 | **Delete** | Remove selection | Properties; **Delete** / **Backspace** | Kb Y | TU-10 | **Mynk** (eraser primary; delete via keyboard) |
...
| C-26 | Z-order (4 actions) | Ctx | Kb Y | — | **keyboard-only** |
...
| C-32 | Delete | Ctx | **Delete** Y | TU-10 | survives via eraser + keyboard |
...
| M-09 | Toggle light/dark theme | Default **on** | **Shift+Alt+D** Y | TU-12 | **DROP** native — app theme toggle (TU-12) |
| M-10 | Canvas background color | Default **on** (`changeViewBackgroundColor`) | Kb N | — | **defer/open-question** (PDF pages are per-page bg) |
...
| **Would be SILENTLY LOST** (Kb N, not in Mynk chrome plan, not intentional DROP) | **18** |
| **Keyboard-only survivors** (work but hidden affordance — TU-11 discoverability) | **34** |
| **Candidate new requirements** (for orchestrator ratification) | **12** |
...
|----------|-----|----------|----------------|
| **P0** | T-12 | **Pen mode toggle** | XPPen/tablet palm rejection (TM-04, Sarah #2) — may need Mynk toggle or auto-detect |
| **P0** | P-03–P-09, P-12–P-14 | **Fill, stroke style, roundness, arrow type, text align, arrowheads** | Advanced styling vanishes unless “More styles” ships in P1.2 |
| **P1** | T-10 | **Image tool file picker** (non-paste) | TB-09 covers Mynk button; **numeric 9** alone is undiscoverable — OK if Mynk ships |
| **P1** | M-10 | **Canvas background color** | White PDF pages vs whiteboard bg — may matter for dark mode (TU-12) |
| **P1** | P-05 | **Freedraw stroke shape** | Niche; only if Sarah uses varied pen profiles |
...
**Mitigated by P1 chrome plan (not silent if we ship on schedule):** T-01–T-09, T-16, P-01, P-04, P-07, P-15, F-01–05, F-08, X-01–05.
...
## Coverage gaps vs 64 requirements

Native functions Sarah may want that are **not** explicit requirements yet — **candidate new requirements** (do **not** add to requirements doc until Andrew/orchestrator ratifies):

| # | Candidate requirement | Native function(s) | Rationale |
|---|----------------------|-------------------|-----------|
| NR-01 | **Hand / pan tool discoverable** | T-11 | Pan is essential on large PDF pages; today only H / Space |
| NR-02 | **“More styles” overflow contract** | P-03, P-06–P-09, P-12–P-14 | Design §3 says “More ▾” but no enumerated field list |
| NR-03 | **Pen mode affordance** | T-12 | TM-04 tablet parity — toggle vs auto |
| NR-04 | **Canvas background policy** | M-10 | Per-page PDF bg vs Excalidraw `viewBackgroundColor` under TU-12 dark |
| NR-05 | **Delete selected vs eraser** | P-24, C-32 | Eraser in chrome; delete-selection only via keyboard — OK? |
| NR-06 | **Laser in tutor toolbar** | T-16 | ST-05 alignment fix is Phase 3; tool access may need earlier |
| NR-07 | **Multi-select + transform handles** | G-01 | Implicit; no req — confirm handles visible when chrome hidden |
| NR-08 | **Image paste + Mynk image btn parity** | T-10, G-05 | TB-09; paste survives, file picker needs Mynk |
| NR-09 | **Shortcuts help surface** | M-06, F-07 | TU-11 / open Q8 — optional `?` in Mynk overflow |
| NR-10 | **Stroke style for marking PDFs** | P-06 | Dashed emphasis on worksheets — rare? |
| NR-11 | **Z-order for overlapping annotations** | P-16–P-19 | Math over PDF — keyboard-only may be insufficient |
| NR-12 | **Native popup survival under CSS hide** | P-01–P-02, P-10 | Phase 0 POC must confirm S/G/Shift+F popups still render |
...
- **Tools:** T-01–T-04 primary strip; T-05–T-09 in pulldowns (TB-01–03, PU-01).
- **Inserts:** X-02–X-04 top bar (TU-04); image (TB-09).
- **Zoom:** F-01–03, F-08 top bar (TB-07).
- **Undo/redo:** X-01 (TB-11) — hide native footer (SR-09).
- **Properties popover:** P-01, P-04, P-07 (defaults), P-15 inline; **More** for NR-02 set.
- **Laser:** T-16 — **defer** toolbar slot to Phase 3 unless ST-05 escalates (NR-06).

### Pulldown / overflow (`···`)

- Remaining shapes (student ST-04).
- Hand tool (NR-01), image file picker fallback.
- Optional shortcuts help (NR-09).

### Context-menu (re-implement) — Phase 2+ or tutor overflow

- Z-order, align, flip (P-16–P-22) if Sarah needs discoverability beyond keyboard (NR-11).

### Keyboard-only (document in TU-11)

- Arrange shortcuts, group/ungroup, tool hotkeys, Space-pan.
- Do **not** rely on student discovering these (ST-04).
...
1. Native **S / G / Shift+F** color popups still work when rails are CSS-hidden.
2. **Pen mode** still auto-activates on stylus (TM-04) without toolbar toggle.
3. **Ctrl+Z** under Mynk overlay (TU-03) — already a design gate.
