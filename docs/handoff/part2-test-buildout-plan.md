...
- **2026-07-05:** plan captured. Executing SERIALLY (dispatch-safety note).
  ...
  - **FULL-JEST CHECKPOINT** ⏳ IN FLIGHT ([checkpoint](301b70d8-4b21-4726-a0b7-72da7d8b1723)) — full `npx jest` + tsc after the ~10 new suites, verifying no cross-file DB pollution (known isolation debt: `whiteboard-checkpoint-cursor.integration`). Run SOLO (no concurrent writer).
  - NEXT (after checkpoint green): remaining pure-jest P2 (P2-J3 consent waiting-room DOM) + REWRITE RW-B1/B3/B4/B5; then SERIAL-ONLY relay (P1-WB-*) + identity-e2e (P1-ID-*) groups (one at a time; need relay/dev-server).
