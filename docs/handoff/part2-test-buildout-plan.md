...
- **2026-07-05:** plan captured. Executing SERIALLY (dispatch-safety note).
  ...
  - **P1-J7** ⏳ IN FLIGHT ([impl](da0815aa-2ae3-496a-b679-76cb0e225950)) — `shouldHardStopSession` threshold (via existing max-seconds override seam, no 8h wait) + WhiteboardWorkspace hard-stop DOM surface.
  - NEXT (serial): P1-J8 notes-shimmer computed style. Then the SERIAL-ONLY relay + identity-e2e groups (one at a time).
