...
- **2026-07-05:** plan captured. Executing SERIALLY (dispatch-safety note).
  ...
  - **P1-J5** ⏳ IN FLIGHT ([impl](7c4bfdb2-973b-4e8e-9a15-cfffbe201c27)) — replay mixdown-only invariant (no peer/transcriptionOnly) + concat-audio & public-concat-audio route auth + no-SSRF DB-origin gate.
  - NEXT (serial): P1-J6 AH-login throttle durability → P1-J7 hard-stop UI seam → P1-J8 notes-shimmer computed style. Then the SERIAL-ONLY relay + identity-e2e groups (one at a time).
