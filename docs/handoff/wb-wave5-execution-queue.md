...
- **WS-T — Proactive defect hunt** · cross-cutting · plan: **this queue (record findings here)**
  ...
  **DEFECTS CAUGHT SO FAR (WS-T tally):**
  ...
  3. **WS-N admin-path multi-segment audio loss (NEW, this session)** — WS-N `32c95a7` made `RecordedAudio.blobUrl` optional (rollover segments defer remote upload to the outbox) and stopped setting it on rollover in the shared `useAudioRecorder`. The **whiteboard workspace** handles this via outbox enqueue, but the **admin AiAssist path** (`AudioInputTabs.tsx`/`AiAssistPanel.tsx`) has **no outbox** — it transcribes via `a.blobUrl`, so a multi-segment (VAD-cut) admin recording now **silently drops every rollover segment**. Surfaced as the `next build` TS blocker at `AudioInputTabs.tsx:75` but is a real **data-loss** bug, not just a type error. **FIX (not fragile — admin consumer only):** in `AudioInputTabs.handleRecorded`, when `!audio.blobUrl && audio.blob`, inline-upload via existing `uploadAudioWithRetry(uploadAudioDirect, …)` (same pattern as `AudioUploadInput.tsx`) before `onAudioReady`. **TEETH:** admin AiAssist multi-segment record → all segments uploaded/transcribed, none dropped. Do NOT use the type-only guard/return shortcut — it would drop segments. Landed as part of the BUILD-GREEN pass below.
