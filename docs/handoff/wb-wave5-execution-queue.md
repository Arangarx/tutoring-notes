...
- **WS-R — Pencil/freedraw roughness no-op** · P2 · plan: **none yet** · smokebook item 19.
  Roughness setting has no visible effect on pencil/freedraw strokes (live or replay). Likely Excalidraw **freedraw ignores `roughness`** (rough.js property for shapes, not perfect-freehand). Investigate: if freedraw genuinely can't take roughness → **hide/disable the control for pencil** (honest UI, no dead knob); else wire it through. Origin: item 19 (N/A-with-notes).
