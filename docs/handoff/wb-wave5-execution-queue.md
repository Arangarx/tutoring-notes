...
- **WS-T — Proactive defect hunt** · cross-cutting · plan: **this queue (record findings here)**
  ...
  **DEFECTS CAUGHT SO FAR (WS-T tally):**
  ...
  4. **`recording-end-to-end` replay auto-starts at ~7.3s (EXPOSED by phase-2 hermetic run, UNDER TRIAGE)** — was blob-skipped, now runs and fails deterministically. Replay/finalize/duration surface. [Readonly triage](a838105f-8b28-4bba-ba16-3415868e52e1) classifying real-bug vs fake-mic-WebM-duration artifact vs stale oracle before any fix. Committed RED in `eb3fb5d` (parked branch, documented).
  5. **`wb-e2-pdf-stroke-leak` PDF board has non-image elements (EXPOSED by phase-2, UNDER TRIAGE)** — was blob-skipped, now runs and fails deterministically. WB PDF import/apply-path (BUG-3). [Readonly triage](a838105f-8b28-4bba-ba16-3415868e52e1) classifying real stroke-leak vs legit non-image import elements vs pdfjs-headless artifact. Committed RED in `eb3fb5d` (parked, documented).
