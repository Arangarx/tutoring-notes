...
- [ ] **1. No auto SessionNote on end** — End a session → confirm **no** `SessionNote` is auto-created (institutional-memory list / parent share shows nothing yet).

  **Notes:**

- [ ] **2. Save creates one READY note** — From the review page, fill/edit notes → **Save** → confirm a single READY note is created and is **immediately visible to the parent** (open the share link).

  **Notes:**

- [ ] **3. Re-save updates in place** — **Save again** (edit then re-save) → confirm it updates **in place** (no duplicate note).

  **Notes:**

- [ ] **4. Delete saved note** — **Delete** a saved (READY) note → confirm it's removed for the parent AND you're returned to the student detail page (never stranded on an error / "Deleting…" screen).

  **Notes:**

- [ ] **5. Correct tutor name on share** — Open the **parent share page** → confirm it shows the **correct tutor's name** for that student (not an arbitrary admin).

  **Notes:**

**Overall Target A:** ☐ PASS ☐ FAIL
