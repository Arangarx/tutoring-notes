...
>
> **Last consolidated:** 2026-06-10 (phone-portrait **build** smoke — Andrew: mobile alt-shape picker UX on **PU-05**/**TB-12**; multi-point-line/n-gon QOL → [`BACKLOG.md`](../BACKLOG.md); **ST-01**/**ST-02** follow-viewport re-confirmed; **ST-05** student-laser color re-observed). Prior same day: mobile board-tab placement — **SR-06/14/17** spreadsheet-paradigm bottom strip + Boards list overlay; mobile whiteboard **design** review — **TM-12/13/14**, **TB-12**, **SR-15/16**, **ST-04/05**; mock [`whiteboard-mobile-mock-2026-06-10.html`](../brand-previews/whiteboard-mobile-mock-2026-06-10.html). Prior: 2026-06-08 Sarah live pilot batch (**IC-01**, **ST-07**, **PU-05**, **TU-15/16**, **SR-04** refresh, **SR-13/14**; audit dispositions; TU-13/TU-14/TM-10; PP-04/ST-05; **PR-01**; **TM-11**; **VP-01/VP-02/PP-06**). **Design doc (ratified forks + phasing):** [`whiteboard-chrome-design-2026-06-07.md`](whiteboard-chrome-design-2026-06-07.md). **Audit:** [`whiteboard-excalidraw-function-audit-2026-06-08.md`](whiteboard-excalidraw-function-audit-2026-06-08.md). **Session shell mock:** [`whiteboard-session-shell-design-2026-06-08.md`](whiteboard-session-shell-design-2026-06-08.md).
...
| **TB-12** | **Phone/tablet tier-1 tool calibration (discoverability).** First mobile cut pulled back too far — both reviewers (Andrew + student perspective) agreed tier-1 must be richer. **Agreed tier-1 (Andrew 2026-06-10):** **Phone student** — Select · Pencil · Eraser · Shapes▾ · Styles · Wand · ⋮ (matches tutor phone tier-1). **Shapes▾ promoted to student tier-1** (Andrew 2026-06-10): no bar-size cost; supports geometry/graphing lessons. **Select promoted to student tier-1** (Andrew 2026-06-10): no longer overflow-only — students need pointer/selection for pan, resize, and tap-to-select. **Phone tutor** — same tier-1 set (still lean vs desktop). **Tablet** — slim left rail (Select / Pencil / Eraser / Text / Wand) + bottom bar (Shapes▾ / Undo / Styles / ⋮); Shapes▾ visible for both roles. Phone use case = *mostly student, mostly watching + sometimes drawing*. Mock: [`whiteboard-mobile-mock-2026-06-10.html`](../brand-previews/whiteboard-mobile-mock-2026-06-10.html). **Mobile alt-shape interaction (Andrew 2026-06-10 phone-portrait build smoke):** current split-button pattern requires tap-to-activate then tap-again to open the picker; long-press does not open it — needs a more discoverable touch pattern before V1 student chrome ships (candidates: long-press opens shapes sheet, or always-visible shapes-sheet entry). Pairs with **PU-05**; see [`BACKLOG.md`](../BACKLOG.md). Pairs with **TB-06**, **ST-04**. | (ii) | Mobile review 2026-06-10; [`ORCHESTRATOR-STATE.md`](ORCHESTRATOR-STATE.md) |
...
| **PU-05** | **Alternate-shapes selector** — one pulldown holding line, arrow, rectangle, diamond, ellipse, etc. **Default shown icon and first menu option = straight diagonal line** (Sarah's most-used shape). Line is **not** promoted to its own top-level toolbar slot. Control shows the **current selection** like other compact controls (PP-06 pattern). **Mobile touch pattern (Andrew 2026-06-10 phone-portrait build smoke):** picking among alternate shapes must be more discoverable than tap-to-activate-then-tap-again (long-press currently does not open the picker). Candidate patterns: long-press opens shapes sheet, or dedicated always-visible shapes-sheet entry on touch. Pairs with **TB-12** Shapes▾ split-button; see [`BACKLOG.md`](../BACKLOG.md). Pairs with TB-02/TB-03 consolidation intent. | (ii) | Sarah live feedback 2026-06-08; mock; mobile build smoke 2026-06-10 |
...
| **TM-12** | **Explicit toolbar/panel show-hide button (mobile).** Persistent, **always-visible** affordance to **open AND close** the tool panel — not gesture-only slide-ins. Lives in the **slim top chrome bar** (session row, left cluster alongside wordmark/LIVE/timer) — **never floating over the canvas**; same placement in **portrait and landscape**, tools-shown and tools-hidden states (it is how users bring tools back). Student default = lean/uncluttered chrome, but the toolbar must be **obviously reachable** via this button. **Root issue:** mobile reviewer could not find the toolbar. Pairs with **TB-12**. Mock: [`whiteboard-mobile-mock-2026-06-10.html`](../brand-previews/whiteboard-mobile-mock-2026-06-10.html). | (ii) | Mobile review 2026-06-10 (student perspective + Andrew) |
...
| **SR-06** | **Board strip** ≤40px at the **bottom** of the session shell (spreadsheet paradigm — matches Google Sheets / Excel mobile: tabs live in the bottom chrome, not under the top bar). User-facing tabs `[Board 1] [Board 2] …` with active indicator on the **top** edge of the strip. Pairs with **SR-14** (labels) and **SR-17** (overflow). Mobile + tablet; desktop may keep existing footer placement. | (ii) | v1 §5.7; Andrew decision 2026-06-10 (Sheets/Excel research) |
...
| **SR-14** | **User-facing "boards" + spreadsheet-style bottom tab strip.** Rename the user-facing multi-canvas concept from **"page"** to **"board"** (internal code, DB fields, and log prefixes such as `pvs` may remain "page"). Bottom strip = **Google-Sheets-/Excel-style tabs** (bottom placement, not under top chrome) with a **"+"** affordance to add a board (tab at bottom, not a separate modal). Replaces pill-style "Pg N" strip and any top-placed mobile tab row. Rationale: tutors already know workbook tabs from Sheets/Excel; bottom placement matches live whiteboard footer + OS spreadsheet apps. Overflow behavior: **SR-17**. | (ii) | Sarah live feedback 2026-06-08; Andrew decision 2026-06-10; mock; terminology note below |
...
| **SR-17** | **Boards list button + overlay picker (overflow / tight width).** When the horizontal tab strip would overflow or space is tight (many boards, phone landscape, narrow viewports), collapse the visible strip to a **bottom-left "Boards" list button** (grid/list icon + label) that opens a **scrollable overlay** listing all boards to pick from — same pattern as Google Sheets on phone (bottom-left list icon beside tabs), iPad Google Sheets, and Excel mobile/iPad ("Sheets" button). When width allows, show **both** the list button and a scrollable tab strip (list button still opens full picker). Strip-only and list-only modes are the same control surface; implementation picks mode from available width. Mock meta toggles: *Boards list* (overlay), *Tabs collapsed* (list-only). | (ii) | Andrew decision 2026-06-10 (Sheets/Excel mobile research); mock [`whiteboard-mobile-mock-2026-06-10.html`](../brand-previews/whiteboard-mobile-mock-2026-06-10.html) |
...
| ID | Requirement | Tag | Source |
|----|-------------|-----|--------|
| **ST-01** | **Follow-tutor** toggle in bottom control bar; **checked by default** (sync pan/zoom). **Re-confirmed Andrew 2026-06-10** phone-portrait build smoke — students must track tutor viewport by default with an easy off toggle; student-chrome **Phase 7** / Gate A5. | (ii) | Sarah-Chat L41; v1 §5.7; mobile build smoke 2026-06-10 |
| **ST-02** | Student bottom bar ~48px: **follow toggle, mic, leave** — above compact **board** strip (bottom placement per **SR-06**). **Re-confirmed 2026-06-10** phone-portrait smoke (placement + affordance with **ST-01**). | (ii) | v1 §5.7; mobile build smoke 2026-06-10 |
| **ST-03** | **Student add-page:** **No** for v1 (ratified); architecture must allow enabling later. | product | Sarah-Chat L23; v1 §8 |
| **ST-04** | Student does **not** need full shape/text toolset by default — tutor drives structure. **Tier-1 student tools** per **TB-12** (Select · Pencil · Eraser · Shapes▾ · Styles · Wand · ⋮ — matches tutor phone tier-1); **Shapes▾** tier-1 for geometry/graphing (no bar-size cost); **Select** tier-1 (no longer overflow-only). Text and inserts remain overflow/tutor-driven. | (ii) | v1 §5.7; RELIABILITY-REDESIGN; mobile review 2026-06-10 |
| **ST-05** | **Pointer wand** (formerly "laser pointer") — **V1 tier-1** for **both tutor and student** (peer-visible highlight; not highlight-only on student). **Resolved (Andrew 2026-06-10):** students get the wand in tier-1 (**TB-12**). **Distinct color per role** so tutor vs student pointers are distinguishable — mock uses tutor=coral, student=cyan; **exact palette TBD** (judgment call). **Re-observed 2026-06-10 phone-portrait build smoke:** student laser currently renders **red** (should be **cyan**) — fix locus **Gate A5**; see [`BACKLOG.md`](../BACKLOG.md) (no duplicate tracking). **Verify** alignment and **bidirectional visibility** on student device in V1; fix if regressed — see [`BACKLOG.md`](../BACKLOG.md) tutor-laser-not-visible bug. Icon: **ST-07**. | (ii) + verify | Sarah-Chat L39–40; audit T-16/NR-06 ratified 2026-06-08; mobile review 2026-06-10 |
...
4. **Student vs tutor tool parity:** **Partially resolved 2026-06-10** — phone tier-1 calibration in **TB-12** (student and tutor share Select/Pencil/Eraser/Shapes▾/Styles/Wand/⋮). Full desktop parity still deferred; student add-page remains v1 no.
