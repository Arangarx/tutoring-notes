...
3. **Smokebooks / smoke-runbooks** (`<scope>-smoke-<date>.md`, etc.) — manual preview smoke checklists Andrew runs top-to-bottom. **MUST** follow [`SMOKEBOOK-TEMPLATE.md`](SMOKEBOOK-TEMPLATE.md) (Action / Expect / Ignore-this-run / separate PASS+FAIL / Notes per item; verified preview URL). Enforced by [`.cursor/rules/smokebook-template.mdc`](../../.cursor/rules/smokebook-template.mdc).
