...
## ⏩ HEAD — 2026-06-21 Wave 4 MERGED @ `a166f6c`; Wave 5 polish IN PROGRESS on `wb-wave5-polish`
...
| **In-flight subagents** | **None.** (Orchestrator running `test:wb-sync` locally — see below.) |
| **Wave 5 status — ALL CODE LANDED @ `7f7c438` on `wb-wave5-polish`, awaiting Andrew comprehensive re-smoke + pre-merge gates** | Slice 1 (visual polish), slice 2 (tab-highlight/view-lock/graph-entry), follow-sync view-lock regression fix (`01ca16d`: `tutorViewportApplyRef` guards `onCanvasChange` across rAF + lock from actual post-clamp `getAppState()`), and round-2 fix batch (`7f7c438`): **(1)** follow-pill vertical centering; **(2)** left tool-rail `.mynk-wb-strip` scroll so "More" reachable (both roles); **(3)** FREEZE fix — `useEffect(()=>setOpenMenu(null),[layoutMode])` closes ⋯ dropdown before its trigger goes `display:none` on resize (was a re-attaching outside-click listener loop); **(4)** student graph → **bidirectional sync** (pass `excalidrawAPIRef.current` unconditionally; clean re-enable of tutor persist path, no protocol/engine change; last-write-wins); **(5)** graph **thumbnail** renders real graph via `ReviewBoardThumbnail` hybrid (graph sessions mount viewModeEnabled Excalidraw w/ `renderEmbeddable`→`GraphEmbeddable readOnly`; non-graph = static PNG unchanged). **Andrew smoke confirmed GOOD:** #1 coral Exit, #4 overflow-drops-down, #5 grid icon, #6 tab-highlight, #7 view-lock-blocks-pan. **Smokebook items 9–13 (round-2) + item 7 (extreme-pan follow) NEED Andrew hardware re-smoke** ([`wb-wave5-polish-smokebook-2026-06-21.md`](wb-wave5-polish-smokebook-2026-06-21.md)). **PRE-MERGE GATES (orchestrator):** (a) `test:wb-sync` (required — #4 sync path; subagents had no Docker), (b) verify the ~11 pre-existing jest failures (auth/2FA/impersonation/password-reset) also fail on `v1-redesign` base, (c) Andrew re-smoke PASS → then `merge --no-ff` to `v1-redesign`. |
| **Uncommitted / unmerged** | **`wb-wave5-polish` @ `c79573a`** (pushed; orchestrator parked here) — Wave 5 polish, BOTH slices landed; **awaits Andrew smoke (items 1–8)** → `test:wb-sync` (REQUIRED, see HEAD) + verify 11 pre-existing jest failures + `merge --no-ff` into `v1-redesign`. Smokebook [`wb-wave5-polish-smokebook-2026-06-21.md`](wb-wave5-polish-smokebook-2026-06-21.md). **None uncommitted** on `v1-redesign` @ `a166f6c`. `wb-wave4-responsive` @ `64108cf` **merged** (branch preserved for cleanup). **`docs/phase3-consent-model` @ `4f9dbcd`** (pushed) — awaits `merge --no-ff` into `v1-redesign`; **conflict risk** on P3 handoff docs (union-merge required). **`phase1/wb-reliability-floor` @ `d63ac22`** — awaits DESKTOP smoke. `phase2/wb-student-new-shell` **superseded + absorbed** by wb-unify merge. |
