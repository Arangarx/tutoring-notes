...
| **In-flight subagents** | **Sonnet Wave 5 round-2 fix batch** (claude-4.6-sonnet, background) on `wb-wave5-polish` — 5 items from Andrew's 2026-06-21 smoke: **(1)** follow-toggle pill vertical centering (still too high); **(2)** left tool-rail "More" unreachable/no-scroll (distinct from the top-bar overflow sheet slice-1 fixed; = the deferred left-rail-scroll, now pulled in); **(3)** FREEZE bug — open top ⋯ dropdown then resize until ⋯ unmounts → page non-responsive (wait/kill) then recovers (dropdown open-state/outside-click loop); **(4)** student graph entry → **bidirectional sync** (slice-2 made it local-only via `excalidrawAPI={undefined}` — Andrew wants it to broadcast like strokes; last-write-wins; sync-layer touch); **(5)** graph **thumbnail** renders actual graph not `mynk://graph` placeholder (= deferred `WB-THUMBNAIL-GRAPH-PLACEHOLDER`, pulled in alongside #4). **Follow-sync view-lock regression FIXED @ `01ca16d` (tip `0dbcca4`):** `tutorViewportApplyRef` guards `onCanvasChange` across the rAF window; lock set from actual `getAppState()` post-clamp. 2 new race/clamp unit tests; tsc/eslint/jest/next-build green. **wb-sync COULD NOT run in subagent (no Docker) → orchestrator runs pre-merge.** Andrew confirmed in smoke: #1 coral Exit, #4 overflow-drops-down, #5 grid icon all GOOD; #6 tab-highlight + #7 view-lock-blocks-student-pan GOOD. |
