...
| **In-flight subagents** | **▶ IDENTITY-E2E PHASE (attended, Andrew go 2026-07-06 ~10:05); specs enroll in `identity-e2e` project (NOT `integration`).** ✅ **P1-ID-1 PIN lockout DONE `67cffaa`** — hard-lock@13 (HTTP 423 `account_locked`), correct-PIN-denied-while-locked teeth, DOM signal, parent-unlock cycle; real route + `LearnerLoginThrottle` DB oracle; 6/6 green, boundary holds. ▶ **P1-ID-2 tutor 2FA login→land + QR local-gen [sa](517e43f1-7e01-4327-b652-ad91182dea11) in flight** — SECURITY-CRITICAL QR no-external-egress teeth (2026-05-31 lesson: TOTP secret must NOT egress to 3rd-party QR host); stop-and-report if 2FA bypassable or secret leaks. Queue after: P1-ID-3 (erasure post-grace purge), P1-ID-4 (share-page audio scrub). **CRLF-flip artifact REVERTED (tree clean).** |
