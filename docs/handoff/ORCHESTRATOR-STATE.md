...
| Integrated gate — relay `test:wb-sync` on green tip 912c886 | ⬜ IN PROGRESS (∥ smokebook) | REQUIRED — E4 event-log + WS-B/D WB paths. ~38 min. |
| Both-theme MASTER-CUT smokebook | ⬜ IN PROGRESS (∥ relay) | per SMOKEBOOK-TEMPLATE; preview alias VERIFIED via Vercel MCP = `tutoring-notes-git-wb-wave5-polish-arangarx-5209s-projects.vercel.app` (tip 912c886); CUT-1..6 both themes; [human-only] E3 reconnect + VAD/per-speaker iOS. PARK at merge gate. |
