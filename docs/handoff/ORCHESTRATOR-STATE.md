...
| **Uncommitted / unmerged** | **FOUR unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (merge after Andrew DESKTOP smoke). (2) `phase1/wb-replay-in-frame` @ `52a9dae` — in-frame two-state replay (awaits smoke; orchestrator owes `test:wb-sync` pre-merge). (3) `auth/2fa-remember-device` @ `9ff545f` (code fix `0e755bc` + smokebook tip) — 2FA remember-device (carries badge via back-merge `55ba810`), **awaits Andrew re-smoke tests 1–3 + 12** then `--no-ff` merge. `ADMIN_TFA_DEVICE_HMAC_SECRET` set in Vercel + live. Not whiteboard → no `test:wb-sync` gate. (4) `fix/preview-badge-copy-commit` @ `75451ad` — pill click copies commit id (preview-only badge), awaits preview confirm → `--no-ff`. (5) `fix/password-manager-username-field` @ `394f6b6` — username `autocomplete` anchor on change/reset forms + **FIX for false "new must differ from current"**: `changePassword` now bcrypt-verifies the current password BEFORE the must-differ check, and the must-differ check is now a bcrypt compare of the new password against the stored hash (was a plain-text equality of the two form fields, which false-fired). Token reset-link path has no must-differ check — the error was the in-settings change-password form. Awaits Andrew smoke → `--no-ff`. **Badge base MERGED — done.** **⚠️ Dispatch-safety near-miss (2026-06-14):** badge executor `git stash`+reset on orchestrator's uncommitted state edits instead of STOPPING — preserved+recovered; reinforce STOP-don't-stash; commit state promptly. |
