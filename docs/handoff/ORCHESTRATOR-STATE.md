...
| WS-D BLOCKER + 2 SHOULD-FIX (from 5-axis) | ⬜ IN PROGRESS | IDB-suppression tab-kill loss fix (coverage compare before suppress) + merge gap-detect + hydrate/live-sync ordering. Relay teeth. **Serial** (shared-branch push race → no concurrent code-writers). |
| ⚠️ **GATE BLOCKER — `npx next build` exit 1** | ⬜ queued (serial) | Branch-level ESLint: `@typescript-eslint/no-explicit-any` "rule not found" in `useAudioRecorder.dom.test.tsx` + `segment-policy.test.ts` (WS-A-touched). Compile OK; lint step fails. Blocks integrated gate (needs exit 0). |
| WS-E E4/E5/E6 bugs | ⬜ queued (serial) | after gate-blocker fix (E4/E5 share WhiteboardWorkspaceClient; E6 = A/V surface) |
