...
| **In-flight subagents** | **E7 (blob-cleanup L-1 fix)** — Composer 2.5, dispatched 2026-07-01 (`scripts/blob-cleanup.mjs` add `chunkBlobUrl` to reference set). **E4 (erasure orchestrator)** — Composer 2.5, dispatched 2026-07-01 (`processErasureJob` grace-gate → blobs_purging → db_scrubbing → completed + `cancelErasureJob`; sets `Student.erasedAt`; H-2 re-enum in db_scrubbing; `ers` logging; NO endWhiteboardSession/route changes). (E2 @ `7792f7d` + E3 @ `fa6f449` pushed.) |
