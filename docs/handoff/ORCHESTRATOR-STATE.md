...
| **Next action(s)** | **PRE-MASTER gate = ONE re-smoke on `phase1/wb-review-correct` @ [`e2c290a`](https://github.com/Arangarx/tutoring-notes/commit/e2c290a):** LV-2 two-device — student enables cam after joining → tutor sees student video **paint on arrival without** manual cluster resize; resize still scales both tiles smoothly (no squish). Remaining failure is **purely video PAINT-on-arrival** (Andrew confirmed: hard-refresh scrub works; tile space allocated — track arrival + tile render OK). If THIS fails → `ResizeObserver`-on-tile fallback still available, but key-remount has a coherent root cause so it's the **strongest attempt yet**. *(Scrub re-smoke [`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08) — **CONFIRMED**.)* Smokebook [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — Andrew actively entering results (**do NOT edit**). **Once LV-2 passes →** `npm run test:wb-sync` + merge `--no-ff` to `v1-redesign`/`master`. Everything else confirmed or deferred. **Decision pending:** WB-NEW-SESSION-FALSE-RECOVERY (stale IndexedDB recovery banner on new sessions) — pull forward pre-master vs leave post-master (Andrew asked). **(2) P2 student-shell** execute-ready @ [`80ac571`](https://github.com/Arangarx/tutoring-notes/commit/80ac571) — awaiting greenlight (unchanged). **(3) Prior threads:** `phase1/wb-reliability-floor` @ `d63ac22` merge after Andrew desktop smoke; Gate A→A6 unchanged; `Experience-Driven Wedge` program + consent v2 when Andrew prioritizes. **Deferred post-master (Andrew 2026-06-16, captured in BACKLOG):** DRY viewport consolidation (**WB-DRY-VIEWPORT**), review transition animation polish (**WB-REVIEW-TRANSITION**), impersonation session-recovery bypass (**WB-IMPERSONATION-SESSION**), eraser reliability + partial erase (**WB-ERASE-RELIABILITY**), new-session false recovery banner (**WB-NEW-SESSION-FALSE-RECOVERY**). |
| **Open Andrew-confirms** | **P1 replay (2026-06-16) — confirmed:** replay first-play starts at 0:00 ✓; live+replay center tracking ✓; thumbnail-pin + button layout ([`8380bf8`](https://github.com/Arangarx/tutoring-notes/commit/8380bf8)) ✓; #5 audio pauses when leaving replay to notes-hero ✓; hard-refresh stored-recording scrub ([`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08)) ✓; in-page delete warning (no `alert()`) ✓; notes/form across prominent↔docked ✓; LV-1 self-view both sides ✓; idempotency #13 tentative (will reconfirm); legacy standalone replay D1-D3 skipped. **Pending re-smoke (pre-master gate — 1 item):** LV-2 two-device @ [`e2c290a`](https://github.com/Arangarx/tutoring-notes/commit/e2c290a) — student enables cam after join → tutor sees video **paint on arrival** without manual cluster resize; resize scales both tiles smoothly (no squish). **Clarified (Andrew 2026-06-16):** earlier "impersonation hid the sessions" was an **UNCONFIRMED hypothesis** — NOT the real cause (Continue/open-sessions list shows normally). Real recurring annoyance = **WB-NEW-SESSION-FALSE-RECOVERY** (stale IndexedDB recovery banner on new sessions) — already in BACKLOG; **decision still pending** pre-master vs post-master. **Resolved — deferred post-master:** DRY consolidation, transition animation #2, impersonation bypass, eraser reliability → [`BACKLOG.md`](../BACKLOG.md) § P1 replay-in-frame post-master follow-ups. **Standing:** (c) student URL · (d) learner-swap · (e) student camera default · (f) C1–C4 · (g) env scoping · B2 D-1/D-2/D-5 · B1 deferred TODOs · N-2. **P2 Q1/Q7** branch strategy + `defaultShowLocalVideo`. **Ratified 2026-06-11:** wall-clock metering. **Lawyer-needed:** VPC, retention, FERPA/SOPIPA. |
...
| **Live-A/V tutor video regression (IN PROGRESS 2026-06-16)** | **LV-1 CONFIRMED** (Andrew real browser): tutor self-view + student sees self + tutor. **LV-2 fix chain (paint-on-arrival):** (1) per-peer pending-track buffer @ [`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8) — no-op for paint (defense-in-depth kept). (2) Late-add renegotiation @ [`cbe6d20`](https://github.com/Arangarx/tutoring-notes/commit/cbe6d20) — track arrival OK. (3) `play()` + resize height @ [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130) — tile SPACE appears; video still black until manual cluster resize. (4) Attempt 3 @ [`a3a94f7`](https://github.com/Arangarx/tutoring-notes/commit/a3a94f7): double-rAF deferred `play()` — insufficient (73 tests). (5) **Attempt 4 @ [`e2c290a`](https://github.com/Arangarx/tutoring-notes/commit/e2c290a):** key-remount on `stream.id` — **first coherent root cause:** remote `<video>` born `display:none`; same-element `display:none→block` on stream arrival doesn't wire Chrome compositor without layout event (manual cluster resize was providing that); self-view works because tutor cam mounts visible; key-remount → fresh element `display:block` from birth. double-rAF `play()` kept as backup. 24 AVTile + 50 useLiveAV tests. **If [`e2c290a`](https://github.com/Arangarx/tutoring-notes/commit/e2c290a) fails:** `ResizeObserver`-on-tile fallback still available, but key-remount is strongest attempt yet. **Andrew-confirmed:** scrub after hard-refresh works; tile space allocated; **remaining failure = video paint-on-arrival only**. **Awaits Andrew two-device re-smoke @ [`e2c290a`](https://github.com/Arangarx/tutoring-notes/commit/e2c290a).** Prior tutor cam-acquisition fix (`682fc15`/`add3b81`) + tile-body min-height still in place. Part of ship-to-Sarah gate (A2 A/V verify). Student-side A/V cluster unification in P2. `test:wb-sync` owed pre-merge. |
| **Uncommitted / unmerged** | **TWO unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (merge after Andrew DESKTOP smoke). (2) **`phase1/wb-review-correct` @ [`e2c290a`](https://github.com/Arangarx/tutoring-notes/commit/e2c290a)** — in-frame unified review surface + full replay/A/V fix chain (seek-on-load [`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08), late-add renegotiation [`cbe6d20`](https://github.com/Arangarx/tutoring-notes/commit/cbe6d20), play()+resize [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130), video paint attempt 3 [`a3a94f7`](https://github.com/Arangarx/tutoring-notes/commit/a3a94f7), video paint key-remount attempt 4 [`e2c290a`](https://github.com/Arangarx/tutoring-notes/commit/e2c290a), CTA layout [`8380bf8`](https://github.com/Arangarx/tutoring-notes/commit/8380bf8), center-match, thumbnail pin, track buffer, audio seek, WebM duration, seek-storm, resize-recenter). **Andrew actively smoking** via [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — do NOT edit. **Confirmed:** replay first-play at 0:00; live+replay center; thumbnail-pin + button layout; #5 audio pause on notes-hero exit; hard-refresh scrub; delete warning; notes/form dock; LV-1; tile space on track arrival; audio seek/header/volume/thumbnail-dark/resize-recenter. **Pending re-smoke (1 item):** LV-2 video paint-on-arrival @ [`e2c290a`](https://github.com/Arangarx/tutoring-notes/commit/e2c290a). **If passes →** `npm run test:wb-sync` + merge `--no-ff` to `v1-redesign`. **DONE/MERGED:** `auth/2fa-remember-device` @ [`8ffd344`](https://github.com/Arangarx/tutoring-notes/commit/8ffd344); `fix/preview-badge-copy-commit` @ [`feccae9`](https://github.com/Arangarx/tutoring-notes/commit/feccae9). **Known pre-existing:** `identity-2fa-management` stale jest assertion; `test:wb-sync` broadcast-throttle half fail (pre-existing, not redesign). **Reset-screen generate-offer → BL-RESET-GENERATE** (backlog). |
