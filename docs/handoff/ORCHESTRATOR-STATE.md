...
**DISPATCH QUEUE (writers, serialize — tree free as of `953efa0`):** (1) login-UX fix (security branch); (2) `SameSite=Lax` + setup/page.tsx helper alignment (off v1-redesign); (3) logged-in→dashboard redirect (off v1-redesign). Lesson re-applied: do state edits BEFORE dispatching a writer that checks out a branch (a writer's `git checkout` moves the shared tree out from under in-chat edits).
