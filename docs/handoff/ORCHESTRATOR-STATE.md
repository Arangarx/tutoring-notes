...
| **In-flight subagents** | **Workstream C infra + consent-save e2e** (Composer 2.5, code-writing — building `identity-e2e` project + parent/ADMIN fixtures + seed helpers + consent-save spec); **read-only explore** scoping erasure/routing UI for the next flow dispatch. Code-writing serialized (single writer on the shared worktree). **Continuation bootstrapper:** [`session-experience-arc-continuation-bootstrapper.md`](session-experience-arc-continuation-bootstrapper.md). |
