...
**`feat/landing-logged-in-redirect` — works (1,2 PASS; 3 deferred) but DESIGN Q (Andrew undecided):** hard `/`→`/admin` redirect means a logged-in tutor can't reach the landing. Two models: (A) Netflix-style (authed users only ever see their dashboard, no landing) vs (B) landing stays reachable with easy dashboard links. **DECIDED 2026-06-05 = B:** keep the `/`→dashboard redirect so "open the app" stays fast, BUT landing/features/about must stay reachable via explicit nav (app header/logo link) without re-redirecting when reached deliberately (query-param escape hatch like `/?view=home`, or logo → a non-redirecting marketing route). → small fix-forward on `feat/landing-logged-in-redirect` (queue; pairs naturally with the `/login` home-nav item — both are header/nav). Not blocking; dispatch after session-fix frees the tree.

---

## 2026-06-05 PM — 3 investigations done + session-fix shipped + queue

**✅ SESSION-FIX SHIPPED, awaits Andrew hardware smoke (NOT merged — P0, merge-after-smoke):** `fix/session-wrong-identity` @ `9de290a`, pushed. Implements ratified Q1-A (same-site `/auth/verify-done` intermediate page sets `mynk_ah_session` on a clean top-level GET via a 90s HMAC handoff token — fixes RC-B Set-Cookie-on-redirect race), Q2-A (revoke-on-verify-only — login paths untouched, multi-device safe), Q3-A (`getAccountHolderSessionFromHeaders` uses `cookies().get()` Map API last-value, kills first-match scan), Q4 (Strict kept, Lax deferred). RC-A = residual preview-only artifact (prod has one domain → fully safe; no host-header-injection vector introduced). Gates green (tsc 0, build 0, 32 new tests, regression 1694/1698; pre-existing `auth.test.ts`/`password-reset.test.ts` RED + flaky `upload-outbox` unchanged). **Preview:** `tutoring-notes-git-fix-session-w-0c21aa-arangarx-5209s-projects.vercel.app` (READY). Smoke script = A→B→claim shows B (same-tab + fresh-tab) + 2nd-device-login-doesn't-logout-1st.

**Investigation conclusions (all 3 done, no subagents running):**
- **Live-session join/viewport** (`4008c272`): Problem 1 = NOT a token-routing bug (tokens bind 1:1 to session id); product allows multiple un-ended sessions per student with no supersede guard. Problem 2 = live viewport-follow is event-driven, NOT pause-gated; "drift while paused" = tutor stationary → no follow packets; "centers off" = likely residual viewport-align layout bug (jsdom-blind → hardware).
- **Slow-join** (`c104caf8`) — REFRAMES Problem 1: "connected" is multi-layered. Relay socket + peer-count pill go green fast, but student canvas only paints after the tutor's v3 **welcome push** (`onNewRemotePeer → flushDocumentBroadcastNow`), which **no-ops if the tutor is idle / sync not yet enabled** (`lastBroadcastPayload === null`) → student blank until tutor draws. A/V is a 4th layer, lags 30–90s, doesn't block board. So Andrew's "wrong session" was almost certainly **connected-but-blank**, not a routing error. Fix dirs: unconditional welcome push decoupled from `enabled&&sync` + retry on presence/`room-user-change` (not only `new-user`); UI honesty splitting "relay connected" vs "board synced"; (bigger) durable relay-side snapshot for late joiners.

**DECISIONS (Andrew 2026-06-05 PM):** Problem 1 → **warn-then-end** (new session warns prior open session(s) for that student will be ended, then supersedes — single-student stacking has no use case; demoted to hygiene since slow-join is the real "neither saw each other" cause). Problem 2 → **viewport stays locked when "stay synced" is ON even with tutor stationary** (heartbeat / re-align on student pan); unchecking the box = freeform — honor the toggle. Hardware-verified fix (viewport-align jsdom-blind zone).

**BACKLOG add (deferred-commit done here):** (a) tutor **recover an ended-but-navigated-away session** — NOT currently backlogged (closest: cold-refresh, Excalidraw "Load draft", "recording controls broken after tab close"); add dedicated item. (b) "Remember this device" login. (c) `/login` + login pages home nav (Mynk wordmark → `/`). (d) Final WB design: maximize student viewport, no "recording saved" on pause, page-list layout.

**DISPATCH QUEUE (serialize whiteboard-gated writers — shared relay + test DB):** (1) ✅ **replay-v2 DONE @ `46de859`** (`fix/replay-multi-segment`, pushed, gates green: tsc 0, build 0, regression 92/92, 25 timeline tests) — all 7 defects fixed (fixed-width controls, true-end stop, seek label, no-flash, seamless boundary, **no-audio synthetic-clock real-time playback**, single-segment unified onto custom player). **Awaits Andrew hardware re-smoke** (all items hardware-only — jsdom-blind). (2) ✅ **join-reliability DONE @ `d66c603`** (`fix/join-welcome-reliability`, pushed): stale-closure fix (`emitDocument`/`flushDocumentBroadcastNow` read `syncRef.current`, no more null-on-first-render no-op), bounded-retry welcome push (`doWelcomeEmit` 500ms cooldown, ≤2 retries @2s, also fires on `room-user-change` peer-increase), UI honesty (amber "Student connected — syncing board…" 5s → green). Invariant-11 Playwright test added. Gates: tsc 0, build 0, regression 92, wb-jest 509. ⚠️ **`wb-playwright` ENV-BLOCKED** (pre-existing `waitForWbE2eBridge` 90s timeout on base `v1-redesign` too — NOT a regression). **Awaits Andrew 2-device hardware smoke** (idle tutor + student joins → board appears ~2s without tutor drawing; pill amber→green). (3) **Problem-1 warn-then-end** (Composer). (4) **Problem-2 viewport-lock** (Sonnet + hardware). (5) 🔄 **landing-B + `/login` home-nav DISPATCHED** (Composer, non-whiteboard).

**⚠️ MERGE-GATE GAP (Andrew decision):** the merge convention requires `npm run test:wb-sync` green before merging any whiteboard-sync change, but the **`wb-playwright` half is env-blocked locally** (`waitForWbE2eBridge` 90s timeout — standing caveat, pre-dates today). So join-reliability (and Problem-2 next) can't satisfy the relay-proof gate locally; only the wb-jest half + Andrew's hardware smoke cover them. Decision needed: (a) fix the wb-playwright env so the relay gate runs (compounding value — unblocks all future sync proof), or (b) explicitly accept hardware-smoke-as-substitute for these merges. Item 4 (Problem-2 viewport, also jsdom-blind) is blocked on this same gate.
