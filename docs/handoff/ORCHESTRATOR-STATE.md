...
| **Next action(s)** | (1) **⏳ AWAITING ANDREW WAVE 4 ROUND-4 RE-SMOKE.** Round-1 smoked 2026-06-20: **tutor desktop PASS** (no merge regression on desktop — the key check); rest PARTIAL (mobile styles "more styles" missing, mobile shapes selector/affordance missing, phone-landscape over-compaction + "More" off left rail, student desktop double-Connected-pill + half-width off-screen overflow, follow-toggle not inline). Round-2 smoked 2026-06-20 (R2-2/R2-4 PASS clean, R2-1/R2-5 PASS-with-notes, R2-3/R2-6 PARTIAL — over-compaction + jitter + landscape rail cutoff + follow centering). Round-3 smoked (R3-2 PASS; R3-1/R3-5 PARTIAL; R3-3/R3-4 FAIL). **Round-4 fixes landed @ `d07568d`, +rail-collapse hotfix @ [`f73b52d`](https://github.com/Arangarx/tutoring-notes/commit/f73b52d)** (action-sheet × stops pointer-capture so mouse-click closes; student compaction = binary-search min-fit, recovers on widen, no hysteresis/snap; phone-landscape rail height chain fixed; follow chip flex-centered over global `.mynk-wb-chip`). **`f73b52d`** fixes a `d07568d` regression where the phone-landscape left rail collapsed to a 52px tiny scroll pane (base `.mynk-wb-bottom-toolbar` 52px height not overridden) — Andrew caught pre-smoke; restored `height:100%` + `overflow-y:auto`. Round-4 re-smoke **R4-1..R4-4** added to smokebook [`wb-wave4-responsive-smokebook-2026-06-19.md`](wb-wave4-responsive-smokebook-2026-06-19.md) (on `v1-redesign`; preview = same branch alias `tutoring-notes-git-wb-wave4-responsive-arangarx-5209s-projects.vercel.app` now serving `f73b52d`). Investigation (2026-06-20) confirmed WB layout mode is **viewport-dimension-only** (`useWbLayoutMode.ts` width/height buckets), NOT touch/pointer — desktop-shrunk ≠ phone unless height&lt;500px enters `phone-landscape`. **Smokebook canonical copy = `v1-redesign`** (Andrew's round-1/2/3 results live here) → at `wb-wave4-responsive` merge, take `v1-redesign`'s version. On round-4 PASS → `merge --no-ff` `wb-wave4-responsive` into `v1-redesign`; on tutor regression → that's priority (merge-introduced). **Do NOT stack Wave 5 on this branch until smoked.** (2) **Wave 5** polish (post-Wave-4-smoke) — coral Exit btn, smaller Match-tutor's-view control, 2 design Qs (highlight non-clickable student tab? / allow student graph entry?), student view-lock-while-synced — see [`wb-unify-stabilization-plan-2026-06-17.md`](wb-unify-stabilization-plan-2026-06-17.md) § Waves 4–5 + W1-3 backlog IDs. (3) **`docs/phase3-consent-model` @ `4f9dbcd`** → union-merge to `v1-redesign` (conflict risk — see lesson below). (4) `phase1/wb-reliability-floor` @ `d63ac22` — awaits DESKTOP smoke. (5) Gate A→A6 + Ship-to-Sarah gate burndown per table below. |
