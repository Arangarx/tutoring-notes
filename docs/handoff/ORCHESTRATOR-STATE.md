...
## ⏩ HEAD — 2026-06-17 WB unify W1-3 done; wb-sync GREEN; smokebook delivered — READY FOR ANDREW SMOKE

| Field | Value |
|---|---|
| **Last action completed (2026-06-17)** | **WB unify Waves 1-3 COMPLETE** on `wb-unify-stabilize`. **W1** (unify) [`facf2a1`](https://github.com/Arangarx/tutoring-notes/commit/facf2a1)→[`90afc2a`](https://github.com/Arangarx/tutoring-notes/commit/90afc2a): student logic ported into unified `WhiteboardWorkspaceClient` behind `role="student"`; **`StudentLiveWorkspaceClient.tsx` DELETED (−1784)**; engine internals untouched; by-construction fixes (tutor-sees-student-laser, right-click-end-line, dark-bg push gone). **W2** (A/V cluster+mesh) [`1f193a4`](https://github.com/Arangarx/tutoring-notes/commit/1f193a4)/[`6b7d709`](https://github.com/Arangarx/tutoring-notes/commit/6b7d709)/[`eac6ab2`](https://github.com/Arangarx/tutoring-notes/commit/eac6ab2): Mechanism-A keyed on **tileCount** not remoteVideoCount (fixes student-no-video / no-cam-initials / disconnect-no-shrink), placeholder offsetHeight flush, videoKey-stable tile-flash fix, `sendLeave`-before-dispose + rejoin re-detect (exit-rejoin-ghost), student cold-start gated on syncClient, audio-indicator debounce. **W3** (engine correctness) [`31e8c45`](https://github.com/Arangarx/tutoring-notes/commit/31e8c45)/[`2cfc6c2`](https://github.com/Arangarx/tutoring-notes/commit/2cfc6c2)/[`d26b922`](https://github.com/Arangarx/tutoring-notes/commit/d26b922): cross-page bleed guard (`boardGenerationRef` + pdf-batch guard-hold), `captureUpdate:"NEVER"` on **all 5** sync `updateScene` sites (fixes undo-poisoning + eraser-restore), undo/redo canvas-focus fix, eraser pointer-down-defer in `applyRemoteToCanvas` (tutor). Gates: tsc/eslint clean; jest whiteboard/AV **894/894**; **paint (W2 2.1/2.2) + bleed/eraser (W3 3.1/3.4) are jsdom-blind → hardware smoke is the gate.** Andrew chose **smoke-cumulative** (not per-wave). |
| **Next action(s)** | **wb-sync GREEN @ [`52a7d0d`](https://github.com/Arangarx/tutoring-notes/commit/52a7d0d)** (13 Playwright passed / 1 intentional skip / 0 fail; jest 666/666) — **the 12 real sync invariants SURVIVED Waves 1-3**; the relay failure was test-harness (bridge keyed `tutor` on every page → [`e90c4f7`](https://github.com/Arangarx/tutoring-notes/commit/e90c4f7) `registerWbE2eSceneBridge(role,...)`), and it ALSO caught a real minor parity miss: student `GraphEmbeddable` lost `readOnly` (invariant 12) → [`52a7d0d`](https://github.com/Arangarx/tutoring-notes/commit/52a7d0d). **NEXT: (1) ← ANDREW: hardware smoke** the `wb-unify-stabilize` preview (`https://tutoring-notes-git-wb-unify-stabilize-arangarx-5209s-projects.vercel.app`) using smokebook [`wb-unify-stabilize-smokebook-2026-06-17.md`](wb-unify-stabilize-smokebook-2026-06-17.md) @ [`811d67c`](https://github.com/Arangarx/tutoring-notes/commit/811d67c) (23 items; Wave 4-5 chrome/laser-COLORS/polish marked Ignore-this-run). **(2)** On PASS → `merge --no-ff wb-unify-stabilize → v1-redesign`. **(3)** Waves 4-5 (chrome/responsive + laser role-colors; polish) as the follow-on cycle. Smokebook + state live on `wb-unify-stabilize` for Andrew (working tree left there). |
| **Open Andrew-confirms** | **Cumulative hardware smoke** of the W1-3 unified result before merge (smoke-timing RESOLVED 2026-06-17: Andrew chose smoke-cumulative-after-W3). Plan approved. *(Prior resolved 2026-06-14/16 unchanged — see body.)* |
| **In-flight subagents** | None — Waves 1-3 + bridge fix + smokebook all complete; awaiting Andrew hardware smoke. |
| **Uncommitted / unmerged** | **`wb-unify-stabilize` @ [`52a7d0d`](https://github.com/Arangarx/tutoring-notes/commit/52a7d0d)** — Waves 1-3 DONE, **wb-sync relay GREEN**, **NOT merged** (awaiting Andrew cumulative smoke); Waves 4-5 follow post-merge. **`phase2/wb-student-new-shell` SUPERSEDED** (unify branch built on it + deleted the student copy; will NOT merge independently). **ONE other unmerged feature branch:** `phase1/wb-reliability-floor` @ `d63ac22`. **wb-sync cadence rule added to AGENTS.md (merge-boundary not per-wave).** |
...
| **P2 student-on-new-shell (SMOKE FAILED 2026-06-17)** | Parity rework + N-fixes on `phase2/wb-student-new-shell`; hardware smoke **FAILED** @ [`5a04689`](https://github.com/Arangarx/tutoring-notes/commit/5a04689). Triage [`wb-student-shell-smoke-triage-2026-06-17.md`](wb-student-shell-smoke-triage-2026-06-17.md). **NOT mergeable** until WB stabilization. |
