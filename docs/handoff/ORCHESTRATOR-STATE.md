...
| **Uncommitted / unmerged** | **`wb-wave5-polish` @ [`07fbfd5`](https://github.com/Arangarx/tutoring-notes/commit/07fbfd5)** — 7a fail-closed-universal + reachability finding docs committed. **Not merged** to `v1-redesign` (single `merge --no-ff` at final Sarah gate only). **`v1-redesign` @ `7397abc`** unchanged. |
