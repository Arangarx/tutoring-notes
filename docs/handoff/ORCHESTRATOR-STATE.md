...
**OVERNIGHT PLAN (2026-06-05, Andrew asleep, authorized "queue what you can" + asked for a consolidated morning smoke runbook):**
1. ✅ state recorded (this edit). 2. Dispatch **session-fix DESIGN doc** (`docs/handoff/session-wrong-identity-fix-design-2026-06-05.md`) — root causes + 4 fix dirs + tradeoffs + recommended defaults for **Andrew AM ratify** (host-injection #1, single-session #3 are his calls; #4 pure-win I recommend just doing). 3. Dispatch **logged-in→dashboard redirect** (own branch `feat/landing-logged-in-redirect`; reads same session helpers — smoke AFTER session fix). 4. Dispatch **morning smoke runbook** (`docs/handoff/MORNING-SMOKE-RUNBOOK-2026-06-05.md`) aggregating ALL unsmoked branches (security login-UX `7fa34a1`, replay `953efa0`, family-id/page-insert, dashboard-redirect) with per-branch preview URL + ordered steps + expected results + merge-gate. Writers serialize (shared tree). NOT doing the session CODE fix overnight (judgment/security tradeoff → AM ratify).
