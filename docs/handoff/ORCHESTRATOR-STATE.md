...
| **In-flight subagents** | **Workstream C slice 2 — erasure e2e** (Composer 2.5, code-writing): erasure request/cancel/restore via `/admin/erasure` + tutor content-route 404 post-erasure + family share-link (×6) denial → 200-after-cancel. Extends `identity.helpers.ts` (add `seedErasureJob`/erasure fixture mirroring `requestErasureByAdmin`+tombstone) + adds `data-testid`s to admin erasure form/cancel. Code-writing serialized (single writer on shared worktree). Erasure/routing UI scoping report captured (enums: `requested`/`blobs_purging`/`db_scrubbing`/`completed`/`failed`/`canceled` — NO `PENDING`/`GRACE`; page guard `notFound()` vs API 404 JSON). **Continuation bootstrapper:** [`session-experience-arc-continuation-bootstrapper.md`](session-experience-arc-continuation-bootstrapper.md). |
