...
## ⏩ HEAD — 2026-06-17 WB unify Wave 1 DONE (unified shell); smoke-timing decision pending

| Field | Value |
|---|---|
| **Last action completed (2026-06-17)** | **WB unify Wave 1 COMPLETE** on `wb-unify-stabilize` (off phase2). Wave 1a [`facf2a1`](https://github.com/Arangarx/tutoring-notes/commit/facf2a1)/[`055d24a`](https://github.com/Arangarx/tutoring-notes/commit/055d24a): tutor `useCollaboratorPointers` + role-props scaffold. Wave 1b [`f12f359`](https://github.com/Arangarx/tutoring-notes/commit/f12f359)→[`90afc2a`](https://github.com/Arangarx/tutoring-notes/commit/90afc2a): student sync/A-V/gates/chrome ported into unified `WhiteboardWorkspaceClient` behind `role="student"`; **`StudentLiveWorkspaceClient.tsx` DELETED (−1784)**; students rerouted. **Engine internals untouched** (selectTutorPage/commitPdfBatch/handleEndSession/applyRemoteToCanvas bodies unchanged; v2/v3 apply paths kept separate). Gates: tsc/eslint clean, jest 669/669, wb-sync 665/666 (1 pre-existing flaky timing test). **By-construction fixes landed:** tutor-sees-student-laser, student right-click-end-line, dark-bg push removed. Spike confirmed feasibility (Option A, additive, no engine rewrite). |
| **Next action(s)** | **SMOKE-TIMING decision pending Andrew** (smoke Wave-1 unification NOW — focused: tutor item-14 + student-renders + laser/right-click/dark-bg wins, IGNORE A/V-no-video/disconnect/bleed/eraser/undo = Waves 2-3 — **vs** continue Waves 2-3 then smoke the cumulative unified+fixed result). Then **Wave 2** A/V cluster paint-reflow + mesh leave/rejoin (Sonnet — peer-mesh concurrency; A/V root-cause report has fix directions), **Wave 3** engine correctness (bleed guard over commitPdfBatch + generation counter, `captureUpdate:"NEVER"` on sync updateScene, undo-under-zenMode, eraser), **Wave 4** chrome/responsive + laser role-colors, **Wave 5** polish. **Merge `wb-unify-stabilize` → `v1-redesign` only after Andrew hardware smoke.** |
| **Open Andrew-confirms** | **SMOKE-TIMING** (smoke Wave-1 now vs smoke cumulative after Waves 2-3) — see Next action(s). Plan approved; merge gated on Andrew hardware smoke. *(Prior resolved 2026-06-14/16 unchanged — see body.)* |
| **In-flight subagents** | None (Wave 1a + 1b complete). |
| **Uncommitted / unmerged** | **`wb-unify-stabilize` @ [`90afc2a`](https://github.com/Arangarx/tutoring-notes/commit/90afc2a)** — Wave 1 unification DONE, green, **NOT merged** (awaiting Andrew smoke); Waves 2-5 still to do on this branch. **`phase2/wb-student-new-shell` SUPERSEDED** (unify branch built on top of it + deleted the student copy; phase2 will NOT merge independently). **ONE other unmerged feature branch:** `phase1/wb-reliability-floor` @ `d63ac22`. |
