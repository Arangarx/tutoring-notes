...
| **Next action(s)** | P2 UX train (WS-F waiting-room exit, WS-H mic persistence, WS-J billable rounding, WS-M student mic boost, WS-Q time-alert copy/config, WS-R roughness UI-hide + false-green spec); WS-U-COPY non-fragile copy/nav batch (anchors in queue); then WS-U-FRAGILE (Sonnet+FSM gating), known-issues page, Part-2 site-wide test buildout (WS-V audit), WS-A F-1 outbox cap. Merge to master = Andrew hard stop. |
...
| **Uncommitted / unmerged** | Branch `wb-wave5-polish` at [`b386ef6`](https://github.com/Arangarx/tutoring-notes/commit/b386ef6) + this docs commit on top, all pushed, unmerged (merge = Andrew hard stop). No code uncommitted at time of this write. |
