...
>
> **✅ RESOLVED 2026-06-16 — fallback path (b) taken.** [`caaabf2`](https://github.com/Arangarx/tutoring-notes/commit/caaabf2) (principled CSS-only fix, Mechanisms A+B removed) **FAILED on-device** — Andrew tested @ [`19d1f47`](https://github.com/Arangarx/tutoring-notes/commit/19d1f47) (= caaabf2 code): "video does not show up without resize again." Per Andrew's directive, restored the working bandaid: **[`1cc268d`](https://github.com/Arangarx/tutoring-notes/commit/1cc268d)** reverts the 5 files caaabf2 touched (`WbAVCluster.tsx`, `AVTile.tsx`, whiteboard-chrome.css + 2 dom tests) back to their [`3b996ae`](https://github.com/Arangarx/tutoring-notes/commit/3b996ae) versions → Mechanisms A+B (the confirmed-working bandaid) are BACK. tsc clean, eslint 0 errors, jest 82/82. Backlogged **WB-AV-VIDEO-PAINT-REAL-FIX** (real fix tied to WB-COMPONENTS-PASS). **LV-2 re-smoke @ [`1cc268d`](https://github.com/Arangarx/tutoring-notes/commit/1cc268d) should PASS** (it's byte-identical AV code to the Andrew-confirmed [`3b996ae`](https://github.com/Arangarx/tutoring-notes/commit/3b996ae)); on confirm → `npm run test:wb-sync` + merge `--no-ff`.
>
...
| **Next action(s)** | **✅ Video-paint decision tree RESOLVED — fallback (b) taken; bandaid restored @ [`1cc268d`](https://github.com/Arangarx/tutoring-notes/commit/1cc268d) (branch tip).** caaabf2 CSS-only fix failed on-device → reverted the 5 AV files to [`3b996ae`](https://github.com/Arangarx/tutoring-notes/commit/3b996ae) (Mechanisms A+B back), backlogged WB-AV-VIDEO-PAINT-REAL-FIX. **AWAIT Andrew LV-2 re-smoke @ [`1cc268d`](https://github.com/Arangarx/tutoring-notes/commit/1cc268d)** (two-device — student enables cam after joining → tutor sees video paint WITHOUT manual resize; resize still scales both tiles smoothly). This is byte-identical AV code to the Andrew-confirmed [`3b996ae`](https://github.com/Arangarx/tutoring-notes/commit/3b996ae), so it **should PASS**. **On LV-2 confirm → pre-master gate cleared → `npm run test:wb-sync` + merge `--no-ff` to `v1-redesign`/`master`.** *(Scrub re-smoke [`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08) — **CONFIRMED**.)* Smokebook [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — Andrew actively entering results (**do NOT edit**). Everything else confirmed or deferred. **Decision pending:** WB-NEW-SESSION-FALSE-RECOVERY (stale IndexedDB recovery banner on new sessions) — pull forward pre-master vs leave post-master (Andrew asked). **(2) P2 student-shell** execute-ready @ [`80ac571`](https://github.com/Arangarx/tutoring-notes/commit/80ac571) — awaiting greenlight (unchanged). **(3) Prior threads:** `phase1/wb-reliability-floor` @ `d63ac22` merge after Andrew desktop smoke; Gate A→A6 unchanged; `Experience-Driven Wedge` program + consent v2 when Andrew prioritizes. **Deferred post-master (Andrew 2026-06-16, captured in BACKLOG):** DRY viewport consolidation (**WB-DRY-VIEWPORT**), review transition animation polish (**WB-REVIEW-TRANSITION**), impersonation session-recovery bypass (**WB-IMPERSONATION-SESSION**), eraser reliability + partial erase (**WB-ERASE-RELIABILITY**), new-session false recovery banner (**WB-NEW-SESSION-FALSE-RECOVERY**), WB componentization tech-debt (**WB-COMPONENTS-PASS**). |
