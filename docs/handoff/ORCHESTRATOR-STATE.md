...
| **🟥 6-spec isolated re-run DONE ([run](62b75c67-aa14-492f-a176-5b83c7d513f2)) — DEFINITIVE** | Fresh dev server per spec, `--workers=1`. **2 CONFIRMED REAL bugs** (deterministic, fail initial+retry — both FRAGILE surfaces, STOP before fixing): **(A) `wb-tutor-recording-mute` #19** — mute BEFORE audio graph ready leaves recording-branch gain=**1** (expected 0) → audio captured when tutor expected muted; RECORDER/audio-graph seam (`mic-recorder-audio.ts`/recording branch). **(B) `wb-e2-apply-remote-pdf-stroke-leak` #7 — REFINED by root-cause ([83b45842](83b45842-d250-4c37-99fa-ffb4a661a3ad)): NOT a confirmed product bug.** The `fingerprintActive=false` (L159) failure is a **stale/mistimed TEST ORACLE** — the fingerprint is cleared by design once PDF import settles, so `false` is correct steady state; product guard (`ef5fb1a`) is logically sound but the spec never reaches its real `__WBX_INJECT_APPLY_REMOTE__` oracle (fix-a unproven, not disproven). **Branch-introduced (spec + fix both in `ef5fb1a`, NOT on master ⇒ not Sarah-facing).** Residual OPEN question: intermittent L146 `every type==="image"` failure may be a SEPARATE onChange-path stroke leak (orthogonal) — confirm via sibling `wb-e2-pdf-stroke-leak.spec.ts` in isolation before any apply-path edit (Sonnet + plan-mode if real). Recommended: realign this spec (Composer, test-only); product change only if L146 proves a real leak. **1 STALE ORACLE** (test-only): **`wb-wave5-polish` item13 #20** — expects `/live/i` but product correctly shows **"Solo rehearsal"** during solo grace (verified vs `lifecycle-machine.ts:627` + its unit test); oracle should assert "Solo rehearsal"/"Recording". **3 ENVIRONMENTAL** (pass in isolation): #1 audio-upload dropzone, #3 review-overlay roster-VAD, #9 wb-end-from-roster `endedAt`. **⇒ REVISED tally of 22 relay reds: 1 confirmed-real-candidate (A, mute-gain — its root-cause [81813fa3] pending), Bug B's L159 = TEST ORACLE (not product) + L146 = OPEN (sibling-spec run pending), 2 stale oracles (#20 + B/L159), ~18 environmental.** So the "2 real bugs" headline is now really **1** (Bug A) pending its root-cause, plus one open sync question (B/L146). |
| **Next action(s)** | **Andrew chose 'root-cause both bugs now' (2026-07-06).** ✅ Stale oracle #20 FIXED + pushed (`0118359`, oracle → `/solo rehearsal|recording/i`, verified PASS — also progresses P2-WB-2). ✅ Bug B refined (L159 = test oracle; guard sound; branch-only). **IN FLIGHT:** Bug A mute-gain root-cause ([81813fa3](81813fa3-b908-4363-8a48-600387a70c50), read-only) + Bug B/L146 sibling-spec confirmation ([fda8a070](fda8a070-3faa-423b-a41e-43fa877d711f), 3× isolated run). When both land → **consolidated plan to Andrew**: Bug A fix approach + origin, B/L146 real-vs-timing verdict, realign the apply-remote spec (Composer test-only), log A(+B) to BACKLOG + queue WS-T tally, infra follow-ups (relay SHARDS not one 20-min marathon; jest isolation-class-2 per-test DB cleanup). Harness fixes committed (`3cb9a7b`,`8a381ce`); state pushed through `f3529bd`. **DEFERRED (Tranche E remaining — Andrew chose harness first):** — recommend harness-hardening (Tranche D: per-worker DB or transactional per-test cleanup or unique fixture keys across seeding files; + auth.setup signout/csrf robustness) FIRST, because a suite that can't run green isn't trustworthy wholesale regardless of individual test quality, and it unblocks merge-gate #1. Flagged "do not rush unattended" (touches seeding across many files) → careful attended pass. Deferred alt: continue **P2-WB-2** → **P3-J1..5**, batch harness later. **Still owed pre-master (gated):** A4 WS-X relay red/green, WS-T #9 outbox self-heal (design-first), Tranche C WS-G-A poll. Docker relay :3002 + Postgres :5432 UP. | **Gated/deferred:** WS-T #9 upload-outbox self-heal (design-first), WS-T #7 relay triage (4 pre-existing `wb-session-lifecycle` reds, owed before master-cut), A4 WS-X relay red/green (deferred — fragile, no blind 3rd fix), Tranche C WS-G-A "preparing seamless replay…" poll (design-first), Tranche D jest-isolation pass. P1-WB-10 IN_PERSON already discharged via A3; Tranche B P1-WB 9/9 complete. Tranche A discharged: A1 build ✅, A2 WS-P defer-reload ✅, A3 in-person ✅ relay-proven. |
