...
| Field | Value |
|---|---|
| **Last action completed (2026-06-16)** | **`phase1/wb-review-correct` @ [`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8)** — session fixes: (1) **Replay center-match** [`ed04279`](https://github.com/Arangarx/tutoring-notes/commit/ed04279): recorded camera = scene-center+zoom; re-derives scroll per paint frame for CURRENT replay container (fixes play snap-back + off-center); NEW recordings auto-capture `.mynk-wb-canvas` dims; LEGACY fall back to `window.innerWidth/innerHeight` (approximate, no snap-back); reuses viewport-align student-follow math. (2) **Review chrome** [`a05abdf`](https://github.com/Arangarx/tutoring-notes/commit/a05abdf): pin "Replay session" button (`flex-shrink:0`); hero thumbnail `object-fit:contain` — tall/portrait thumb no longer pushes CTA off-screen; 5/5 DOM tests. (3) **Live-A/V LV-2** [`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8): per-peer pending-track buffer in `useLiveAV` — student's `ontrack` arriving before sync presence no longer drops track; drained on entry creation; additive, no peer-mesh changes; 48/48 DOM tests. (4) Docs [`c63d6a7`](https://github.com/Arangarx/tutoring-notes/commit/c63d6a7). **Andrew-confirmed (real browser):** live+replay center tracking; in-page delete warning (no `alert()`); notes/form across prominent↔docked; LV-1 self-view both sides; idempotency #13 tentative (will reconfirm); legacy standalone replay D1-D3 skipped. *(Prior milestone — 2026-06-14 login-friction/auth merged @ [`8ffd344`](https://github.com/Arangarx/tutoring-notes/commit/8ffd344); 2026-06-12 `v1-design-system` @ [`36727ea`](https://github.com/Arangarx/tutoring-notes/commit/36727ea).)* |
...
| **Next action(s)** | **(1) Andrew re-smoke on `phase1/wb-review-correct` @ `3261da8`:** LV-2 two-device (student cam on → tutor sees second tile with live video — fix pending real-browser confirm); thumbnail-pin ([`a05abdf`](https://github.com/Arangarx/tutoring-notes/commit/a05abdf)); replay centering with **NEW** recording (legacy recordings approximate only due to window-size fallback); smokebook [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — Andrew actively entering results (**do NOT edit**). **(2) OPEN polish — Andrew decision:** transition animation #2 (hero-notes→docked + hero→replay feels like load-flash not smooth animation; thumbnail + Replay button flash on entry) — pre-master vs defer. **(3) Awaiting Andrew decisions:** DRY consolidation (`computeResizeScroll` duplicates viewport-align center-match math; equivalence locked by test; recommend viewport-align as single source, thin `computeResizeScroll` to delegate) now vs defer; impersonation bypass for `assertOwnsWhiteboardSession` (admin-only "lost session"; NOT Sarah ship-blocker; separate SSG-2 UX gap) now vs defer. **(4) P2 student-shell** execute-ready @ [`80ac571`](https://github.com/Arangarx/tutoring-notes/commit/80ac571) — awaiting greenlight (unchanged). **(5) Prior threads:** `phase1/wb-reliability-floor` @ `d63ac22` merge after Andrew desktop smoke; Gate A→A6 unchanged; `Experience-Driven Wedge` program + consent v2 when Andrew prioritizes. |
| **Open Andrew-confirms** | **P1 replay (2026-06-16):** live+replay center tracking ✓; in-page delete warning (no `alert()`) ✓; notes/form across prominent↔docked ✓; LV-1 self-view both sides ✓; idempotency #13 tentative (will reconfirm); legacy standalone replay D1-D3 skipped. **Pending re-smoke:** LV-2 ([`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8)); thumbnail-pin ([`a05abdf`](https://github.com/Arangarx/tutoring-notes/commit/a05abdf)); replay centering with NEW recording. **Pending decisions:** DRY consolidation (`computeResizeScroll` vs viewport-align); transition animation #2 pre-master vs defer; impersonation bypass now vs defer. **Standing:** (c) student URL · (d) learner-swap · (e) student camera default · (f) C1–C4 · (g) env scoping · B2 D-1/D-2/D-5 · B1 deferred TODOs · N-2. **P2 Q1/Q7** branch strategy + `defaultShowLocalVideo`. **Ratified 2026-06-11:** wall-clock metering. **Lawyer-needed:** VPC, retention, FERPA/SOPIPA. |
...
| **P1 replay-in-frame (ACTIVE thread 2026-06-16)** | Branch `phase1/wb-review-correct` @ [`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8) — unified in-frame review surface. **Session fixes (2026-06-16):** replay center-match per frame [`ed04279`](https://github.com/Arangarx/tutoring-notes/commit/ed04279); hero thumbnail pin + CTA on-screen [`a05abdf`](https://github.com/Arangarx/tutoring-notes/commit/a05abdf); live-A/V track buffer [`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8). **Andrew-confirmed:** live+replay center; delete warning; notes/form dock toggle; LV-1; audio seek/header/volume/thumbnail-dark/resize-recenter (prior fixes). **Pending re-smoke:** LV-2; thumbnail-pin; replay center with NEW recording; transition animation #2 (OPEN polish — pre-master vs defer). **Findings (no active bug):** DRY — `computeResizeScroll` duplicates viewport-align center-match (equivalence test-locked); impersonation "lost session" = admin-only ownership gate (NOT Sarah blocker). Smokebook [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — Andrew actively entering results. Prior FIX 1–7 + WebM duration + seek-storm + resize-recenter chain still in place. |
...
| **Live-A/V tutor video regression (FIXED 2026-06-16)** | **LV-1 CONFIRMED** (Andrew real browser): tutor self-view + student sees self + tutor. **LV-2 fix @ [`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8):** per-peer pending-track buffer in `useLiveAV` — student's `ontrack` arriving before sync presence created internal peer entry no longer drops track; drained on entry creation; additive, no peer-mesh changes; 48/48 DOM tests. **Awaits Andrew two-device re-smoke:** student cam on → tutor sees second tile with live video. Prior tutor cam-acquisition fix (`682fc15`/`add3b81`) + tile-body min-height still in place. Part of ship-to-Sarah gate (A2 A/V verify). Student-side A/V cluster unification in P2. `test:wb-sync` owed pre-merge. |
| **Uncommitted / unmerged** | **TWO unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (merge after Andrew DESKTOP smoke). (2) **`phase1/wb-review-correct` @ [`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8)** — in-frame unified review surface + full replay/A/V fix chain (audio seek, WebM duration, seek-storm, resize-recenter, center-match per frame, thumbnail pin, live-A/V track buffer). **Andrew actively smoking** via [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — do NOT edit. **Confirmed:** live+replay center; delete warning; notes/form dock; LV-1; audio seek/header/volume/thumbnail-dark/resize-recenter. **Pending re-smoke:** LV-2; thumbnail-pin; replay center NEW recording; transition animation #2. **If clean →** `test:wb-sync` + merge `--no-ff` to `v1-redesign`. **DONE/MERGED:** `auth/2fa-remember-device` @ [`8ffd344`](https://github.com/Arangarx/tutoring-notes/commit/8ffd344); `fix/preview-badge-copy-commit` @ [`feccae9`](https://github.com/Arangarx/tutoring-notes/commit/feccae9). **Known pre-existing:** `identity-2fa-management` stale jest assertion; `test:wb-sync` broadcast-throttle half fail (pre-existing, not redesign). **Reset-screen generate-offer → BL-RESET-GENERATE** (backlog). |
