...
| **In-flight subagents** | **None.** Tonight's login-friction thread: **preview-only branch badge MERGED to `v1-redesign`** ([`a18f4fb`](https://github.com/Arangarx/tutoring-notes/commit/a18f4fb), `--no-ff`, Andrew-approved smoke — server-side `VERCEL_ENV==='preview'` gate → bottom-right dismissible pill, 5 tests prove no prod render). **2FA "remember this device"** still on its branch awaiting smoke (`auth/2fa-remember-device`; chain plan [`b079a1d`](https://github.com/Arangarx/tutoring-notes/commit/b079a1d) → 5-axis [`588f732`](https://github.com/Arangarx/tutoring-notes/commit/588f732) 3BLOCKER+6SHOULD-FIX → Rev 2 [`f0bcbdf`](https://github.com/Arangarx/tutoring-notes/commit/f0bcbdf) → impl [`7e23f88`](https://github.com/Arangarx/tutoring-notes/commit/7e23f88); B1 impersonation step-up, B2 fail-closed skip mint, B3 step-up shares `check2faVerifyRateLimit`; `@ts-nocheck` on 2 new TEST files only; migration auto-applies on push). `v1-redesign` merged INTO the 2FA branch [`55ba810`](https://github.com/Arangarx/tutoring-notes/commit/55ba810) so it now carries the badge too. |
| **Uncommitted / unmerged** | **THREE unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (VERIFIED-CLEAN; merge after Andrew DESKTOP smoke). (2) `phase1/wb-replay-in-frame` @ `52a9dae` — in-frame two-state replay (awaits Andrew smoke; orchestrator owes `test:wb-sync` pre-merge). (3) `auth/2fa-remember-device` @ `55ba810` — 2FA remember-device + merged-in `v1-redesign` (incl. badge), gates green, **awaits Andrew smoke** then `--no-ff` merge. Migration already live on preview DB. Not whiteboard → no `test:wb-sync` gate. **Badge `feat/preview-branch-badge` MERGED ([`a18f4fb`](https://github.com/Arangarx/tutoring-notes/commit/a18f4fb)) — done.** `v1-redesign` tree clean, in sync with origin. **⚠️ Dispatch-safety near-miss (2026-06-14):** badge executor hit orchestrator's uncommitted `ORCHESTRATOR-STATE.md` edits and `git stash`+reset instead of STOPPING — work preserved (stash, not dropped) + recovered; reinforce STOP-don't-stash in future branch-switching dispatch prompts; orchestrator now commits state promptly. |
