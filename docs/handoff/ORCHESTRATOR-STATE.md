...
## ⏩ HEAD — 2026-07-01 pre-merge smoke NOT PASS; safe-erasure mini-phase planned

> **Active branch:** [`wb-wave5-polish`](https://github.com/Arangarx/tutoring-notes/tree/wb-wave5-polish) — docs @ tip after state refresh (see **Uncommitted / unmerged**). **Worktree:** `tutoring-notes-polishwt` (default `tutoring-notes` checkout is on `v1-redesign` — NOT current). **All remaining Sarah-gate work lands here; single `merge --no-ff` to `v1-redesign` at final gate only (no interim merge, Andrew confirmed).** Integration base remains **`v1-redesign` @ [`7397abc`](https://github.com/Arangarx/tutoring-notes/commit/7397abc)**.
>
> **2026-07-01 — pre-merge smoke verdict: NOT PASS.** Andrew ran smoke against tip `8e38935` (code through [`e66c177`](https://github.com/Arangarx/tutoring-notes/commit/e66c177)). Annotations captured [`f85af03`](https://github.com/Arangarx/tutoring-notes/commit/f85af03)..[`e47f41a`](https://github.com/Arangarx/tutoring-notes/commit/e47f41a); triage [`consent-honesty-smoke-findings-2026-07-01.md`](consent-honesty-smoke-findings-2026-07-01.md). **Six merge-blockers** (MB-1..MB-6): Start silent failure, erasure false-success / incoherent grace, IN_PERSON no replay, tutor_only no notes, student→parent routing security. Andrew ratified **safe-then-merge** + **reversible tombstone** (grace = suspended access + recoverable account; **REVERSES** prior "tutor retains read access during grace"). Executor plan: [`consent-honesty-safe-erasure-plan.md`](consent-honesty-safe-erasure-plan.md).
>
> **⛔ MERGE BLOCKED** until mini-phase complete + clean re-smoke. On re-smoke PASS: `test:wb-sync` on final tip, then **`merge --no-ff wb-wave5-polish → v1-redesign`**. Then **Part 3 reliability spine** (`p3-clock` → per-speaker capture → transcription) in a **FRESH chat**. **POST-SARAH-PRE-RELEASE backlog** (out of mini-phase scope): C-1 replay Play/Pause overlaps Board tab; C-2 student mic-boost parity — see findings §C.

| Field | Value |
|---|---|
| **Last action completed** | **2026-07-01 — Andrew pre-merge smoke run surfaced merge-blockers (verdict NOT PASS).** Smoke annotations captured [`f85af03`](https://github.com/Arangarx/tutoring-notes/commit/f85af03)..[`e47f41a`](https://github.com/Arangarx/tutoring-notes/commit/e47f41a): [`f85af03`](https://github.com/Arangarx/tutoring-notes/commit/f85af03) verbatim smoke results; [`75ed7c7`](https://github.com/Arangarx/tutoring-notes/commit/75ed7c7) smokebook formatting; [`3f77c1f`](https://github.com/Arangarx/tutoring-notes/commit/3f77c1f) triage [`consent-honesty-smoke-findings-2026-07-01.md`](consent-honesty-smoke-findings-2026-07-01.md); [`e47f41a`](https://github.com/Arangarx/tutoring-notes/commit/e47f41a) backlog capture (bugs B-1..B-3, UX C-1..C-10, design Qs DQ-1..DQ-6). Investigations done — MB-1 impersonation-exit race; MB-2 DELETE bypass + ineffective full-family tombstone; MB-3 grace read-access **reversed**; MB-4 audio-policy gates WB event log; MB-5 tutor_only notes gap; MB-6 learner `/` routing. **Andrew ratified:** safe-then-merge + reversible tombstone (DQ-1 resolved). Mini-phase plan authored (commit 1 of this session). — — — **Prior:** consent-honesty blocker CODE-COMPLETE @ [`e66c177`](https://github.com/Arangarx/tutoring-notes/commit/e66c177); erasure E1–E8 shipped; jest **769/769** green. |
| **Next action(s)** | **Execute [`consent-honesty-safe-erasure-plan.md`](consent-honesty-safe-erasure-plan.md):** **(1)** 5-axis reliability review of erasure redesign (Sonnet dispatch) **before** ER code. **(2)** Workstream A consent fixes (CF-1..CF-4). **(3)** Workstream B erasure redesign (ER-1..ER-7) — reversible tombstone, suspended grace access, true cancel restore. **(4)** Workstream C Playwright e2e (CC-1/CC-2 + erasure-admin). **(5)** Workstream D re-smoke + gates (`npx next build`, `test:wb-jest`, `test:wb-sync`, full jest, both themes) → `merge --no-ff wb-wave5-polish → v1-redesign`. **(6)** Then Part 3 spine in fresh chat. Clean orphan mis-scoped `ErasureJob` on preview DB. |
| **Open Andrew-confirms** | **None pending** — mini-phase scope ratified (safe-then-merge + reversible tombstone). **Standing (unchanged):** debounced-disconnect pause trigger (confirm at `p3-clock`); **WB-LABEL-PARENT-SIGNIN**; **Sarah primary device** ([`SARAH-CALL-PREP.md`](../SARAH-CALL-PREP.md)); **Ship-to-Sarah gate**; **iOS student WB/A/V** ([`BACKLOG.md`](../BACKLOG.md) **WB-STUDENT-MOBILE-VALIDATION**). |
| **In-flight subagents** | **None.** |
| **Uncommitted / unmerged** | **`wb-wave5-polish` @ tip after this session's commits** — plan + state refresh; **NOT merged** to `v1-redesign`. **Merge BLOCKED** until mini-phase complete + clean re-smoke. **`v1-redesign` @ `7397abc`** unchanged. **⚠️ THROWAWAY UNTRACKED COPIES in main `tutoring-notes` (v1-redesign) working tree:** `docs/handoff/{consent-honesty-premerge-smoke-index, wb-block-b-consent-gate-smokebook-2026-06-30, cc1-cc2-consent-gate-smokebook, erasure-smokebook}.md` — delete before merge. Tracked authoritative copies on `wb-wave5-polish`. |
