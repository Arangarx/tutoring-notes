...
> **Wave 5 agent DONE checklist:** All 13 smokebook items mapped to **wb-regression** Playwright. **New:** `tests/integration/wb-wave5-polish.spec.ts` (items 1–7, 10, 11, 13). **Extended:** invariants **12d** (equal vertical inset + midline), **12e** (student→tutor graph UI persist via `onAfterPersist` broadcast); **12b/12c** + `wb-student-exit-rejoin.spec.ts` cover items 8/12 + rejoin/banner. **Product fixes:** `GraphEmbeddable.onAfterPersist` + `ReviewGraphOnlyThumbnail` (direct read-only graph hero); `ReviewBoardThumbnail` scene-map graph detection. **Gates:** Wave 5 polish 10/10 + tagged wb-regression 24/24 green (2026-06-24). **`test:wb-sync` not re-run this turn** — run before merge to `v1-redesign`. **Next:** Andrew **one** hardware smoke per [`smoke-when-done.mdc`](../../.cursor/rules/smoke-when-done.mdc).
...
| **Wave 5 status** | **Agent DONE** — all 13 smokebook items in wb-regression; Playwright gates green (2026-06-24). `test:wb-sync` pending before merge. Awaiting Andrew single smoke. |
| **Uncommitted / unmerged** | **`wb-wave5-polish`** — tip pushing after item 13 visibility fix. |
