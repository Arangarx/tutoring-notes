...
| **Open Andrew-confirms** | (1) **Slice-3 Target A = PASS-with-notes (Andrew 2026-06-07)**; merge **HELD** pending the delete-list investigation outcome ([`0e905cb5`](0e905cb5-2b0a-4900-aa6f-08b12ac9d7df)) + `test:wb-sync` green. (2) **Target B (auth-role-refresh) not yet smoked** — then merge `harden/auth-role-refresh` (role-bleed Fix A+B). (3) **IAC-13 disconnect-copy = HARD pre-master gate** (Andrew: student-record-name vs learner-name terminology is backwards/confusing; parent — not tutor — should name the child). |
| **In-flight subagents** | [`0e905cb5`](0e905cb5-2b0a-4900-aa6f-08b12ac9d7df) — delete-list staleness investigation (read-only; slice-3 merge-readiness gate). |
...
**Target A re-smoke result (Andrew 2026-06-07): PASS with notes — merge HELD pending one investigation.** Steps 1-5 all checked. Follow-ups from the typed notes:
1. **Delete-list staleness (merge-readiness gate):** "Deleted a note → it stayed on the notes list." The review-page **Cancel and delete session data** op correctly navigated away; but a **per-note delete from the student notes list** appears to leave the row visible (likely missing `revalidatePath` on the `[id]/notes` route). **Under investigation** ([`0e905cb5`](0e905cb5-2b0a-4900-aa6f-08b12ac9d7df)) — decide blocker vs follow-up before slice-3 merge.
