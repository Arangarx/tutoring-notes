...
**2026-06-03 (PM) — Integration merge to `v1-redesign`:** `merge --no-ff` **`identity-p2-multitutor`** → **`v1-redesign`** @ **`b3da280`**; then `merge --no-ff` **`feature/phase-d-landing-about`** (landing/features copy + sign-in hover). Post-merge gates green (tsc, `next build`, regression 92/92; identity 279/280 — pre-existing `identity-p2-schema` `student` back-relation naming failure only). Additive migration **`20260603000000_learner_pin_throttle`** applies on next preview/prod deploy.
