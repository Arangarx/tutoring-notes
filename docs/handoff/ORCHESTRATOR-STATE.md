...
## ⏩ HEAD — 2026-06-17 P2 student shell SMOKE FAILED; WB stabilization next

| Field | Value |
|---|---|
| **Last action completed (2026-06-17)** | **P2 student-shell hardware smoke FAILED** on `phase2/wb-student-new-shell` @ [`5a04689`](https://github.com/Arangarx/tutoring-notes/commit/5a04689) (includes disconnect-resize [`974fc87`](https://github.com/Arangarx/tutoring-notes/commit/974fc87) + Exit-disconnect [`574fae9`](https://github.com/Arangarx/tutoring-notes/commit/574fae9) — both **ineffective**). Broad regression cluster: many **shared-chrome regressions** of previously-solved behavior (cross-page bleed, eraser-tutor, undo/redo-both, disconnect-resize, laser, theme-dark-bg) plus student A/V (no video) + chrome overflow + mobile gaps. Raw smoke notes committed [`f138dab`](https://github.com/Arangarx/tutoring-notes/commit/f138dab) on phase2. Full triage: [`wb-student-shell-smoke-triage-2026-06-17.md`](wb-student-shell-smoke-triage-2026-06-17.md). Smokebook template gained **PARTIAL** per-item state. BACKLOG escalations + new items captured on `v1-redesign`. |
| **Next action(s)** | **WB stabilization (UNIFY direction — Andrew 2026-06-17).** 3 root-cause explores in flight → synthesize a **unification-framed** sequenced plan (one shared canvas/chrome core, role-gated; session-ownership stays tutor-only; additive extraction, tutor smoke-protected) and bring to Andrew to approve. Confirm monolith-parameterization feasibility from the investigations FIRST; flag if it resists. **`phase2/wb-student-new-shell` NOT mergeable** (will likely be superseded by the unified core rather than patched). Then **(1b) P3 PLAN dispatch** ([`58ca872`](https://github.com/Arangarx/tutoring-notes/commit/58ca872)/[`0c92644`](https://github.com/Arangarx/tutoring-notes/commit/0c92644)) after greenlight + stabilization. **(2)** `phase1/wb-reliability-floor` @ `d63ac22` — merge after Andrew **DESKTOP** smoke. **(3)** Ship-to-Sarah gate (SSG-1/2/3, F1). **Near-immediate post-master:** full product usage instrumentation (Andrew 2026-06-17; reclassified from pre-master gate — [`BACKLOG.md`](../BACKLOG.md) § Near-immediate post-master priorities). **Deferred post-master (BACKLOG):** WB-DRY-VIEWPORT, WB-REVIEW-TRANSITION, WB-IMPERSONATION-SESSION, WB-ERASE-RELIABILITY, WB-NEW-SESSION-FALSE-RECOVERY, WB-COMPONENTS-PASS, WB-AV-VIDEO-PAINT-REAL-FIX, WB / site composition + de-duplication audit, **WB-D6** student asset inserts. |
| **Open Andrew-confirms** | **WB stabilization sequencing — RESOLVED (Andrew 2026-06-17): investigate root-cause scope first, then bring a sequenced plan to approve.** **Architecture direction — RESOLVED (Andrew 2026-06-17): UNIFY the surfaces now (pull the WB composition/de-dup consolidation forward) instead of patching the divergent student copy.** Plan around ONE shared canvas/chrome core (capability-gated by role); session-ownership layer (recording FSM, end-session, outbox, share link, asset inserts, page authority) stays tutor-only; done as **additive extraction with the working tutor path smoke-protected** (NOT a big-bang rewrite — `a150d4f` baseline; "extend don't rewrite"). Orchestrator to CONFIRM monolith-parameterization feasibility from the in-flight investigations before any code, and flag immediately if it resists. **Next open-confirm = approve the synthesized UNIFY stabilization plan.** *(Prior resolved 2026-06-14/16 unchanged — see body.)* |
| **In-flight subagents** | **3 read-only root-cause explores (composer-2.5)** for WB stabilization: cross-page bleed [`337ebb67`], A/V video+cluster [`1dffc020`], tool wiring [`7913f014`]. Each determines shared-chrome vs student-specific + recovers prior-fix logic (baseline `a150d4f`). Synthesize into UNIFY-framed plan when they land. |
| **Uncommitted / unmerged** | **`phase2/wb-student-new-shell`** — smoke **FAILED** @ `5a04689`; **not mergeable** until WB stabilization. **ONE other unmerged feature branch:** `phase1/wb-reliability-floor` @ `d63ac22`. |
