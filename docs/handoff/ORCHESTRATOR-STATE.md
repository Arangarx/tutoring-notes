...
| **In-flight subagents** | **None.** Tonight's login-friction thread shipped two branches (both await Andrew smoke): **2FA "remember this device"** (`auth/2fa-remember-device` tip [`4e18de5`](https://github.com/Arangarx/tutoring-notes/commit/4e18de5) ← impl [`7e23f88`](https://github.com/Arangarx/tutoring-notes/commit/7e23f88), 23 files; chain plan [`b079a1d`](https://github.com/Arangarx/tutoring-notes/commit/b079a1d) → 5-axis [`588f732`](https://github.com/Arangarx/tutoring-notes/commit/588f732) 3BLOCKER+6SHOULD-FIX → Rev 2 [`f0bcbdf`](https://github.com/Arangarx/tutoring-notes/commit/f0bcbdf) → build; B1 impersonation step-up, B2 fail-closed skip mint, B3 step-up shares `check2faVerifyRateLimit`; `@ts-nocheck` on 2 new TEST files only) and **preview-only branch badge** (`feat/preview-branch-badge` tip [`7f7a982`](https://github.com/Arangarx/tutoring-notes/commit/7f7a982) ← impl [`6bf1c4f`](https://github.com/Arangarx/tutoring-notes/commit/6bf1c4f); server-side `VERCEL_ENV==='preview'` gate → bottom-right dismissible pill, 5 unit tests prove no prod render). Both: tsc/eslint/jest/`next build` green. |
| **Uncommitted / unmerged** | **FOUR unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (VERIFIED-CLEAN; merge after Andrew DESKTOP smoke). (2) `phase1/wb-replay-in-frame` @ `52a9dae` — in-frame two-state replay (awaits Andrew smoke; orchestrator owes `test:wb-sync` pre-merge). (3) `auth/2fa-remember-device` @ `4e18de5` — 2FA remember-device, gates green, awaits Andrew smoke. Migration auto-applies on push via deploy pipeline `prisma migrate deploy` (Andrew confirmed 2026-06-14) → already live on preview DB. Not whiteboard → no `test:wb-sync` gate. (4) `feat/preview-branch-badge` @ `7f7a982` — preview-only branch badge, gates green, awaits Andrew smoke; not whiteboard → no `test:wb-sync` gate. `v1-redesign` tree clean, in sync with origin. **⚠️ Dispatch-safety near-miss (2026-06-14):** the badge executor hit the orchestrator's uncommitted `ORCHESTRATOR-STATE.md` edits and `git stash`+reset instead of STOPPING per the dispatch rule — work was preserved (stash, not dropped) + recovered, but reinforce STOP-don't-stash in future branch-switching dispatch prompts; orchestrator now commits state promptly rather than leaving it uncommitted. |
