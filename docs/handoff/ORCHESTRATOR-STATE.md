...
**NEXT:** (a) **IN FLIGHT** — Composer realigns the 2 `audio-upload` specs (test-only; :86 pane-visible wait + Notes scroll; :101 harness-aware upload stub + `toBeEnabled()`), verify red→green per-spec, commit/push. (b) **F3** apply-remote spec realignment (test-only, Composer — but WS-X careful per A4 row). (c) Then a **confirming attended relay run** (`test:wb-sync`, ~50min) once the test-only reds are realigned → merge-gate #1 GREEN. (d) **Bug A** `wb-tutor-recording-mute` — NOTE non-repro (didn't appear in runs #2/#3); **re-confirm reproduction first**, then (if real) fragile: plan-mode → Sonnet 5-axis → fix; green spec = merge bar. (e) Then **Tranche E** (P2-WB-2 → P3).
