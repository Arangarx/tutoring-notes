...
**DISPATCH QUEUE (serialize whiteboard-gated writers — shared relay + test DB):** (1) **replay-v2** (Sonnet, fix-forward on `fix/replay-multi-segment`): S2 fixed-width Play/Pause button (kills layout shift behind S2/S6/abruptness) + scrubber true t=0; S3 end-of-timeline replay-segment-2 jump; seek-pauses-but-says-"Pause"; initial final-state flash; **no-audio sessions replay in REAL TIME (synthetic clock)**; unify single-segment onto custom player. (2) **join-reliability** (Sonnet + wb-sync + hardware): unconditional welcome push + UI honesty. (3) **Problem-1 warn-then-end** (Composer). (4) **Problem-2 viewport-lock** (Sonnet + hardware). (5) **landing-B + `/login` home-nav** (Composer, non-whiteboard). Items 1,2,4 all need the wb-sync gate → run serially.
