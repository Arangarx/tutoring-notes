...
| **In-flight subagents** | **Workstream C slice 3 — routing e2e** (Composer 2.5, code-writing): student-vs-parent routing — `/students/login` (learner) vs `/account/login` (AH/parent); root `/` persona routing (operator→`/admin`, learner→`/join`, AH→`/account/dashboard`); `JoinAuthGate` `/join/[sessionId]` redirect by persona (child→students login, self-learner→account login); parent lands on `/account/dashboard`. Adds `data-testid`s to login forms/dashboard as needed. Code-writing serialized (single writer). **After this → checkpoint re-smoke (NO merge).** **Continuation bootstrapper:** [`session-experience-arc-continuation-bootstrapper.md`](session-experience-arc-continuation-bootstrapper.md). |
