...
| Integrated gate — relay `test:wb-sync` on tip 912c886 | ❌ RED | [relay run](1166f932-c93c-48ae-bcb3-5a20b2582d9a): `relay:build` ok; jest 812/812; Playwright **107 pass / 2 fail / 2 skip** (both fails also failed retry#1). **(A) HARD BLOCKER:** `wb-wave5-polish.spec.ts` item 13 (`@wb-chrome @wb-graph` review thumbnail renders JSXGraph) — after `wb-end-session` the `toolbar[name="Session review"]` NEVER appears (180s×2). **Directly on WS-C surface** (End→straight-to-review rewire, dadc01e). Investigating. **(B) SOFT:** `wb-session-lifecycle` "Session has ended" — known pre-existing flake (2/2), not blocking. **(C) COVERAGE GAP:** the 8 new wave teeth specs are NOT in `playwright.config.ts` `wb-regression` testMatch (lines 144-153) — relay never ran them (only `wb-student-mic-persistence`, PASS). Per-WS "teeth green" was targeted dev runs, not the standing gate. MUST wire in + re-run. |
