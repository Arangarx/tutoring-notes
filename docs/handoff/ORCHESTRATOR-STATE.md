...
### 🟩 2026-07-05 OVERNIGHT UPDATE — P1 fix train + P2 UX train COMPLETE (authoritative status = [`wb-wave5-execution-queue.md`](wb-wave5-execution-queue.md))

> **Branch tip: `28acea9`** on `wb-wave5-polish` (pushed). Still **NOT merged** (parked at merge gate for Andrew's hardware re-smoke). Everything below is committed on the branch.
>
> **SHIPPED this overnight run (all with teeth; fragile items 5-axis-reviewed + fix-passed):**
> - **P1 train:** WS-W replay-entry-autoplay-at-t=0 (`610ee90`, Sonnet CLEAN) · WS-K live map+reduce notes ≤2–3s + Race-A guard (`859f695`) · WS-G seamless replay concat + erasure-wire + race fixes (`d20ea9a`) · WS-N4 gate/roster orphan-segments (`0df4bf3`, Sonnet CLEAN) · WS-P core deliverables 1/3/4 build-identity/ChunkLoadError/footer-SHA (`b386ef6`).
> - **P2 UX train — COMPLETE:** WS-U-COPY copy/nav wave (`dfe1bf4`) · WS-F waiting-room exit (`7bff936`) · WS-H mic persistence (`c26f7ce`, Sonnet CLEAN) · WS-Q time-alert copy (`4ac69d4`) · WS-R roughness-no-op chrome gate (`8e23324`) · WS-M student mic boost (`3947728`, Sonnet + fixes) · **WS-J billable rounding (`1d23fc6`, Sonnet SHIP-WITH-FIXES, B1 blocker fixed in-commit).**
>
> **🛑 HEADLINE FINDING (2026-07-05, CONFIRMED P1 — needs Andrew):** **IN_PERSON sessions with full audio consent record NO audio on prod defaults** ([verify](52816713-0d5b-4085-aba3-6ddd83405575), 93%). The recorder FSM (`src/lib/recording/lifecycle-machine.ts`) has no `sessionMode` wiring → in-person sits in `armed/awaiting_first_participant` (`recordingActive===false`) waiting for a remote peer that never joins → `handleStartRecording` never fires → 0 `SessionRecording` rows. **= BACKLOG SMOKE-BLOCK-5.** Full-consent in-person loses strokes too (same armed gate; "strokes recover" was the denied-audio path). FRAGILE recorder-FSM fix → **parked for Andrew's product call (treat in-person like solo?) + own 5-axis; NOT touched overnight.** CI masks it (`playwright.config.ts:48` sets the solo env flag). Detail: queue WS-T #6 + FOR-ANDREW-U13-b.
>
> **PARKED / gated (need Andrew):** WS-X BUG-3 PDF stroke-leak (2nd attempt exhausted → real cause = v3-broadcast tombstone rebroadcast; WIP on `wb-wave5-ws-x-wip` `5d80ea8`; no 3rd blind attempt) · WS-P deliverable 2 (version-poll defer-during-live-session — WS-P-A/WS-P-B acks) · **migrations authored NOT applied** (WS-K/WS-G/WS-J additive nullable; `.env` targets Neon preview-dev; prod verified clean; prod apply = HARD STOP).
>
...
### 🔵 2026-07-05 PM — FRESH ORCHESTRATOR CHAT picked up (swap) — autonomy=LIMITED; in-person P1 RESOLVED

> New chat bootstrapped from §1 docs; branch tip `41a5bcd` clean + pushed, unmerged. **Autonomy posture this session = LIMITED** (Andrew): proceed unattended ONLY on safe non-fragile work (jest-isolation pass, state restructure, pure-jest batches); **PARK anything fragile/gated** for Andrew. Prior "keep going" grant expired.
>
> **✅ HEADLINE PRODUCT CALL RESOLVED (FOR-ANDREW-U13-b / SMOKE-BLOCK-5 / WS-T #6):** Andrew = **treat IN_PERSON like solo — start recording the tutor's mic on Start, no remote peer required.** The fix is a FRAGILE recorder-FSM change (thread `sessionMode` into `evaluateLifecycle`, `src/lib/recording/lifecycle-machine.ts`). Execution plan: **read-only design pass first → Sonnet 5-axis → Andrew go → gated additive FSM edit + in-person-audio teeth (unit + Playwright P1-WB-10, `SessionRecording`≥1 with no student join).** NOT unattended (fragile + limited autonomy). This partially moots FOR-ANDREW-U13-a (in-person `awaiting_first_participant` waiting copy) — in-person will no longer sit in that state.
>
> **FOR-ANDREW batch RESOLVED (2026-07-05 PM):** WS-K/WS-G tuning defaults confirmed (no pre-flush; 5-chunk/2min debounce; full reduce; libopus re-encode; cap 400; duration free-ride). **WS-G-A = POLL** ("preparing seamless replay…" swap-in when concat ready — new replay-side follow-up). WS-N4 defaults ratified, NO concurrent-tab End-block. WS-J defaults confirmed (nearest/5 + America/Denver; in-person wall-elapsed incl pauses; ≥1-increment min); prod-apply still merge HARD STOP. **WS-P-A = tutor-only defer**; **WS-P-B = YES** (read-only-of-FSM `useEffect` in WWC approved) → deliverable 2 unblocked (Skew-Protection toggle = Andrew dashboard action). **WS-X = SCHEDULE** (read-only design + 5-axis). Known-issues page = IN-APP (Help/Settings). `.env`→preview-dev = informational, no action.
>
> **✅ KNOWN-ISSUES IN-APP PAGE SHIPPED** `89d8d02` (`/admin/settings/known-issues`, tutor-facing, 3 Sarah-facing sections only; internal WS-* appendix excluded; tsc clean + 5/5 DOM test). **FOR ANDREW:** tone/copy sign-off + 3 content calls (WS-I/WS-N inclusion, WS-G concat-lag omission, WS-O framing).
>
> **✅ TWO FRAGILE READ-ONLY DESIGN PASSES DONE — both GATED on Andrew go + Sonnet 5-axis before any edit (serialize on `WhiteboardWorkspaceClient.tsx` — in-person, WS-X, and WS-P deliverable-2 all touch WWC → NEVER 2 code-writers at once):**
> - **In-person FSM fix** ([design](74fc7510-5309-4e39-87a0-30b4a13f3805)): additive — `inPersonMode?:boolean` on `LifecycleInputs` + dedicated **step-3b** branch (`if(inPersonMode) return finalize("recording",…)` — chosen over folding into the `soloEnabled` guard so a LIVE→IN_PERSON mid-session toggle after a peer connected doesn't wrongly pause) + `derivePresentation` guard (clean red "Recording" pill, not amber "Solo rehearsal") + one line at 2 WWC call sites. Root cause confirmed `lifecycle-machine.ts` L467 (both `!syncEnabled` L426 + `soloEnabled` L459 bypasses miss on prod defaults). Billing unaffected (`billable-clock.ts` IN_PERSON already wall-elapsed from `activatedAtMs`). Teeth: **4 jest units = authoritative red-before-green** + 1 Playwright BUT CI masks it (`playwright.config.ts:48` sets solo env flag) → jest is the true gate; recommends a `wb-in-person-unmasked` playwright project (backlog vs in-scope = Andrew). **3 open Qs for Andrew:** LIVE→IN_PERSON mid-session toggle intent; unmasked playwright project now vs backlog; `inPersonMode` boolean vs `sessionMode` enum style.
> - **WS-X BUG-3 stroke-leak** ([design](1c811ac4-8246-416f-86a0-f8748068d8f7)): **🔁 REVERSES the queue's recommended fix (b).** Filtering `isDeleted` from the v3 broadcast **WOULD BREAK tutor-erasure propagation** — tombstones are the student's sole deletion signal (`reconcileElements` needs `{isDeleted:true,version:V2}` to override local `{isDeleted:false,V1}`); the WBElement adapter filter is recording-only, NOT a live-sync precedent. **Recommended = fix (a):** additive ~3 LOC — add `&& !pageSceneSetFingerprintRef.current.has(targetId)` to the `onTargetReadTime` condition in `applyRemoteToCanvas` (~L1015 WWC) → during the post-page-switch fingerprint window falls back to clean `pageDataRef[targetId]` (set by `commitPdfBatch`); tombstone-baseline reconcile + erase/undo sync preserved. Needs a NEW `__WBX_INJECT_APPLY_REMOTE__` test seam (existing WIP seam on `wb-wave5-ws-x-wip` covers the OTHER, already-guarded bypass). Sonnet + 5-axis mandatory.
>
> **BOTH FRAGILE FIXES APPROVED (Andrew go).** In-person: unmasked Playwright project = IN SCOPE (Andrew); `inPersonMode` boolean; LIVE→IN_PERSON mid-session toggle = NOT a real scenario (Andrew: mode fixed at creation) → no toggle handling. WS-X: fix (a) fingerprint-guard approved (filter-isDeleted reversal accepted).
>
> **✅ IN-PERSON FSM FIX IMPLEMENTED — `9740b1b` (pushed).** Additive: `LifecycleInputs.inPersonMode?` + step-3b (`if(inPersonMode) finalize("recording",…)`, placed AFTER `!syncEnabled` step 3, BEFORE `!networkOk` step 4a → in-person does NOT pause on network loss, correct: no remote peer) + `derivePresentation` guard (`participants.size===0 && !ctx.inPersonMode` → red Recording pill not amber Solo-rehearsal) + 2 WWC call-site lines. Teeth: 4 jest units red→green (LIVE path explicitly asserted unchanged); Playwright `wb-in-person-audio-start.spec.ts` (`@wb-recording`) enrolled in `wb-regression` + NEW `wb-in-person-unmasked` project (port 3101 webServer WITHOUT the solo env flag — the true prod-default CI gate). jest 46/46, tsc clean, `--list` enrolled. **RELAY RUN OWED at attended merge boundary** (both projects; Docker unavailable in impl worktree). **✅ Sonnet 5-axis = SHIP-WITH-FIXES, NO BLOCKERs** (core FSM/billing/blast-radius/replay-mix clean). 2 SHOULD-FIXes ✅ FOLDED — `3bf3a7e` (pushed): (1) `clock_start` log now carries `mode=in_person|solo|live`; (2) spec removed from `wb-regression` testMatch (soloEnabled masks → synthetic-green; `wb-in-person-unmasked` is its real gate — documented exception to the "enroll every teeth spec in wb-regression" rule). jest 46/46, tsc clean. **IN-PERSON FIX COMPLETE @ `3bf3a7e`** (relay run for both projects still owed at attended merge boundary).
>
> **✅ WS-X FIX (a) IMPLEMENTED — `ef5fb1a` (pushed).** Additive guard on `applyRemoteToCanvas`: `onTargetReadTime` now also requires `!pageSceneSetFingerprintRef.current.has(targetId)` → during the post-page-switch fingerprint window the merge READ baseline falls back to the clean `pageDataRef[targetId]` (set by `commitPdfBatch`) instead of transitional `getSceneElementsIncludingDeleted()`; `stillOnTargetWriteTime` unchanged. **Diff LARGER than 3 LOC:** also pulled in prerequisite infra that lived only on `wb-wave5-ws-x-wip` (`pageSceneSetFingerprintRef` + its set in `selectTutorPage`, + the attempt-#2 stale-onChange rejection in `handleExcalidrawChange`) → 5-axis must scrutinize this. New prod-safe test seams (`__WBX_INJECT_APPLY_REMOTE__`/`__WBX_FORCE_LIVE_SCENE__`/etc., gated on `NEXT_PUBLIC_WB_E2E_SCENE_HOOK===1` + tutor). Playwright `wb-e2-apply-remote-pdf-stroke-leak.spec.ts` (`@wb-strokes @wb-sync @wb-assets`) enrolled + `--list`-verified. tsc clean; whiteboard jest 645/645. **v3 broadcast filter NOT touched** (erasure propagation preserved). **RELAY red/green proof OWED at attended `test:wb-sync`** (Docker unavailable in worktree — cannot execute the real-browser oracle here). 3 full-jest failures = known parallel-DB flake class (`--workers=1` green; jest-isolation debt). **✅ Sonnet 5-axis = CLEAN** (`ef5fb1a`, [review](64f308b1-9b3d-4815-8654-132c9e8aac34)): `pageDataRef[pdfPageId]` proven the correct authoritative baseline during the window (no over-suppression — tutor has no uncommitted work on a freshly-navigated page); tombstone/erase-undo path unchanged; multi-page apply unchanged; fingerprint reliably clears; `__WBX_*` seams inert in prod (double-gate); red/green oracle sound. Reviewer confident fix (a) closes the leak with NO new sync regression. **WS-X IMPLEMENTED + 5-axis CLEAN @ `ef5fb1a`; relay red/green proof OWED at attended `test:wb-sync`** (same status as the existing BUG-3 spec).
>
> **✅ MILESTONE — BOTH FRAGILE FIXES DONE + REVIEWED:** in-person audio `3bf3a7e` (5-axis SHIP-WITH-FIXES→folded) + WS-X BUG-3 `ef5fb1a` (5-axis CLEAN). Both additive, no behavioral engine rewrite, relay proofs owed at attended merge boundary.
>
> **NEXT (awaiting Andrew steer):** WS-P deliverable 2 (fragile-adjacent WWC — P-B ack given; gated), WS-G-A "preparing seamless replay…" poll (replay-side), jest-isolation pass (broad — attended-preferred, queue warns vs rushing), heavy `ORCHESTRATOR-STATE` restructure (Composer, due at milestone), PART-2 relay/identity serial burndown (attended + Docker — incl. WS-X + in-person + `wb-in-person-unmasked` relay proofs). Known-issues page copy/tone + 3 content calls still FOR ANDREW.
...
| **Next action(s)** | P2 shipped: WS-U-COPY (`dfe1bf4`), WS-F (`7bff936`), WS-H (`c26f7ce`, 5-axis CLEAN), WS-Q copy-slice (`4ac69d4`), WS-R (`8e23324`), **WS-M (`3947728`, 5-axis SHIP-WITH-FIXES → both BLOCKERs fixed)**. P2 order **WS-F✅ WS-H✅ WS-Q✅ WS-R✅ WS-M✅ → WS-J** (mini-plans [P2 design](775aca66-e219-4c84-b6fc-5db1783da9f1); Andrew-gates in queue FOR-ANDREW). Next: dispatch **WS-J** (billable rounding — FRAGILE-ADJACENT `endWhiteboardSession` + ADDITIVE nullable migration LOCAL/TEST DB ONLY + `/settings/billing` UI → mandatory Sonnet 5-axis + Andrew defaults for increment/direction/timezone/clock-source). Then WS-U-FRAGILE (Sonnet+FSM gating), known-issues page, Part-2 test buildout (WS-V audit), WS-A F-1 outbox cap. **Merge-gate notes before master:** (1) branch cumulatively touches `src/components/whiteboard/` → `npm run test:wb-sync` ONCE on integrated tip; (2) WS-M needs a two-device real-hardware A/V smoke; (3) WS-J migration needs prod-apply greenlight. Merge = Andrew hard stop. |
...
| **Uncommitted / unmerged** | Branch `wb-wave5-polish` at [`3947728`](https://github.com/Arangarx/tutoring-notes/commit/3947728) (WS-M) + this docs commit on top, all pushed, unmerged (merge = Andrew hard stop). No code uncommitted at time of this write. |
