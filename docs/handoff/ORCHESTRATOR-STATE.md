...
## ⏩ HEAD — 2026-06-09 (wb-chrome-redo MERGED to v1-redesign; JSXGraph swap next)

| Field | Value |
|---|---|
| **Last action completed** | **wb-chrome-redo MERGED `--no-ff` → `v1-redesign`** @ [`f73f5ee`](https://github.com/Arangarx/tutoring-notes/commit/f73f5ee) (pushed). Andrew smoked all chrome batches GREEN through [`415865f`](https://github.com/Arangarx/tutoring-notes/commit/415865f); pre-merge gate GREEN (`npx tsc --noEmit` clean after spec fix `caa6685`; `npm run test:wb-sync` 566 jest + 12 playwright pass). Includes: chrome redo (no engine rewrite), MathLive 2nd-open portal-race fix, Desmos CSP (now superseded — see below), AV split-brain A4 fix, ink-inversion, shapes split-button, hover, board tabs, AV state cohesion. |
| **Next action(s)** | **Kick off JSXGraph graphing swap** on a fresh branch off `v1-redesign` (decision + full scope in [`BACKLOG.md`](../BACKLOG.md) § Graphing tool swap — Desmos → self-hosted JSXGraph via `renderEmbeddable`; removes desmos.com CSP origins; accept breakage of old Desmos graphs). **Then** land the **3 deferred-but-v1 items** (Gate A3 notes-primary end-session + A3a PDF tab indicator + A3b SR-04a video-tile sizing). **Do NOT cut `master` until ALL Gate A items below are done.** |
...
| **In-flight subagents** | **JSXGraph graphing swap** dispatching on a fresh branch off `v1-redesign`. |
...
| **Uncommitted / unmerged** | **`feat/wb-chrome-redo` MERGED → `v1-redesign` @ `f73f5ee` (pushed).** JSXGraph swap branch dispatching next off `v1-redesign`. **Deferred-but-v1 (post-graph-swap):** A3 notes-primary end-session, A3a PDF tab indicator (`isPdf` on `PageStripRow`), A3b SR-04a video-tile sizing (**note: tile DOES scale slightly with the panel but far too little — scale-ratio bug, wiring exists**). Equation-over-white-PDF-in-dark-mode legibility deferred (BACKLOG; white backing-plate candidate). **Unmerged → master:** entire `v1-redesign` epic. **Parked:** `feature/sarah-forward-migration-q6` @ `a396ab5`. |
