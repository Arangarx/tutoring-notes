...
**2026-05-30 — SEC-1 Dispatch C SMOKED + MERGED** (`--no-ff` merge `8bb7449`): real-admin `/admin` dashboard landing + routing (tutor paths impersonation-only; exit → dashboard) + `AdminTestAccountsPanel` replacing interim `TestAccountsSection` + login default callback `/admin` + 39 SEC-1 tests green. Andrew real-hardware smoke **PASS** (dashboard landing, guard redirects, impersonate round-trip, exit-to-dashboard, test-account credential rejection). **SEC-1 (A+B+C) is DONE.** Open nit: banner not amber (cosmetic, defer to UI redesign). **preview-dev account cleanup done 2026-05-30:** `+test1` flipped to `isTestAccount=true` (now the sole impersonation target; owns smoke student `48e3c900…`; password login disabled by gate); `+test2/3/4` + the `+sec1smoke` throwaway DELETED (cascade took their test students/sessions/imp-logs). Remaining on preview-dev: `arangarx@gmail.com` (real admin, password→dashboard), `arangarx+test1@gmail.com` (impersonation target), `malmesae@gmail.com` + `playwright@test.local` (untouched). **Production AdminUser untouched** (all cleanup on `br-crimson-mode-amape02v` only).
