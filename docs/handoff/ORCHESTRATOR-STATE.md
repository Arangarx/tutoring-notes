...
## ⏩ HEAD — 2026-06-09 (wb-chrome-redo awaiting smoke; JSXGraph swap decision logged)

| Field | Value |
|---|---|
| **Last action completed** | **JSXGraph graphing swap decision logged** in [`BACKLOG.md`](../BACKLOG.md) § Graphing tool swap (Desmos → self-hosted JSXGraph via `renderEmbeddable`; own branch after chrome-redo merge). **wb-chrome-redo** on `feat/wb-chrome-redo` @ [`e78a5e7`](https://github.com/Arangarx/tutoring-notes/commit/e78a5e7) — awaiting Andrew smoke, then `npm run test:wb-sync` gate + `merge --no-ff` → `v1-redesign`. |
| **Next action(s)** | **Andrew smoke** `feat/wb-chrome-redo` @ `e78a5e7` → on pass: `test:wb-sync` gate → `merge --no-ff` → `v1-redesign`. **Next branch after merge:** JSXGraph graph swap (decision + scope in [`BACKLOG.md`](../BACKLOG.md) § Graphing tool swap). **Then** land the **3 deferred-but-v1 items** (Gate A3 + PDF tab indicator + SR-04a). **Do NOT cut `master` until ALL Gate A items below are done.** |
...
| **Uncommitted / unmerged** | **`feat/wb-chrome-redo` @ [`e78a5e7`](https://github.com/Arangarx/tutoring-notes/commit/e78a5e7)** — awaiting Andrew smoke + `test:wb-sync` gate + merge to `v1-redesign`. JSXGraph swap decision logged in BACKLOG (docs commit pending). **Unmerged → v1-redesign:** `feat/wb-chrome-redo` (in flight); `feature/sarah-forward-migration-q6` @ `a396ab5` (parked). |
