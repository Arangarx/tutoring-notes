...
## ⏩ HEAD — 2026-06-17 Phase 1 merged @ [`f68053c`](https://github.com/Arangarx/tutoring-notes/commit/f68053c); Phase 2 route-gap found in smoke → HARD-SWITCHED (flag removed) @ [`23e1cba`](https://github.com/Arangarx/tutoring-notes/commit/23e1cba), head [`a644a64`](https://github.com/Arangarx/tutoring-notes/commit/a644a64) — awaits RE-smoke; P3 plan+5axis execute-ready
...
| **Next action(s)** | **(1) ⏸ P2 student-on-new-shell — ROUTE-GAP FOUND IN SMOKE → HARD-SWITCHED, AWAITS RE-SMOKE.** Branch `phase2/wb-student-new-shell` head [`a644a64`](https://github.com/Arangarx/tutoring-notes/commit/a644a64). **Andrew smoke 2026-06-17 caught:** `/w/[joinToken]/page.tsx` rendered LEGACY `StudentWhiteboardClient` unconditionally — new shell was built but **never wired into the real route** (gated on `NEXT_PUBLIC_WB_STUDENT_NEW_SHELL`, unset on Preview; `test:wb-sync` passed ONLY because Playwright `webServer` injected the flag — synthetic green, not the real default route). **Andrew decision: HARD SWITCH, no flag, "not going back."** Fix @ [`23e1cba`](https://github.com/Arangarx/tutoring-notes/commit/23e1cba): `/w/[joinToken]` → `StudentWhiteboardSessionShell` → `WhiteboardSessionShell role=student` → `StudentLiveWorkspaceClient` UNCONDITIONALLY (prop-parity preserved); `NEXT_PUBLIC_WB_STUDENT_NEW_SHELL` removed everywhere (code/playwright/docs/smokebook); legacy `StudentWhiteboardClient.tsx` orphaned (on disk, unreferenced) → delete after Sarah live-smoke (backlog **WB-LEGACY-STUDENT-CLIENT-DELETE**). Gates: tsc/eslint clean, jest 660, `test:wb-sync` 13/1-skip now through REAL route (no flag), `next build` 0. Flag-free smokebook [`phase-2-student-new-shell-smokebook-2026-06-16.md`](phase-2-student-new-shell-smokebook-2026-06-16.md) (also on `v1-redesign`). **Andrew action: re-smoke on Preview — NO env setup; student should land on `mynk-wb-chrome` `data-role=student`. On PASS → `merge --no-ff` into `v1-redesign`.** **(1b) P3 PLAN + 5-AXIS execute-ready** ([`58ca872`](https://github.com/Arangarx/tutoring-notes/commit/58ca872)/[`0c92644`](https://github.com/Arangarx/tutoring-notes/commit/0c92644), 5 blockers folded P3-A1..A5) — dispatch after P2 merge + greenlight. **(2)** `phase1/wb-reliability-floor` @ `d63ac22` — merge after Andrew **DESKTOP** smoke (1b audio-clock; iOS tighten-pass deferred-safe, not merge gate). **(3)** Ship-to-Sarah gate (SSG-1/2/3, F1) — folds into P2/P3 + backend pass. **(4)** `Experience-Driven Wedge` rolling-wave + consent v2 when Andrew prioritizes. **Decision pending:** WB-NEW-SESSION-FALSE-RECOVERY — pre-master vs post-master. **Deferred post-master (BACKLOG):** WB-DRY-VIEWPORT, WB-REVIEW-TRANSITION, WB-IMPERSONATION-SESSION, WB-ERASE-RELIABILITY, WB-NEW-SESSION-FALSE-RECOVERY, WB-COMPONENTS-PASS, WB-AV-VIDEO-PAINT-REAL-FIX. |
...
| **In-flight subagents** | **1 RUNNING (Composer):** A/V cluster shrink-on-peer-leave fix on `phase2/wb-student-new-shell` — surfaced during Andrew's P2 re-smoke 2026-06-17 (student disconnect → tutor cluster stayed 2-up, local video filled the frame). Root cause CONFIRMED: Mechanism A (`3b996ae`/`1cc268d`) sets `userResized=true` on remote-video arrival, never cleared on leave → `useAutoGrow=false` freezes `displayHeight` at 2-up (~497px); CSS `flex:1 1 0`+`object-fit:cover` stretches the lone tile. Tutor-side `WbAVCluster` only (student shell doesn't mount it); **predates P2, lives on `v1-redesign` since `3b996ae`** — fix folded into `phase2` so Andrew's live Preview updates in place. Fix = split paint-reflow lock from manual-drag `userResized` + symmetric shrink on tile-count decrease (additive, no engine touch) + jsdom departure test. **DONE:** P2 executor [`93d71ca`](https://github.com/Arangarx/tutoring-notes/commit/93d71ca); hard-switch [`23e1cba`](https://github.com/Arangarx/tutoring-notes/commit/23e1cba); P3 plan [`58ca872`](https://github.com/Arangarx/tutoring-notes/commit/58ca872) + 5-axis [`0c92644`](https://github.com/Arangarx/tutoring-notes/commit/0c92644). Login-friction/auth CLOSED @ [`8ffd344`](https://github.com/Arangarx/tutoring-notes/commit/8ffd344). |
...
| **P2 student-on-new-shell (HARD-SWITCHED 2026-06-17 — awaits re-smoke)** | Executor DONE @ [`93d71ca`](https://github.com/Arangarx/tutoring-notes/commit/93d71ca); **Andrew smoke caught the new shell was never wired into the real route** (flag-gated `/w/[joinToken]`, Preview flag unset; `test:wb-sync` green only via Playwright `webServer` flag injection — synthetic). Hard-switched @ [`23e1cba`](https://github.com/Arangarx/tutoring-notes/commit/23e1cba) (head [`a644a64`](https://github.com/Arangarx/tutoring-notes/commit/a644a64)): `/w/[joinToken]` renders new shell UNCONDITIONALLY, flag removed everywhere, legacy orphaned. All gates green; wb-sync now exercises new path via REAL route. Plan/5-axis: [`phase-2-student-on-new-shell-plan-2026-06-16.md`](phase-2-student-on-new-shell-plan-2026-06-16.md) + [`phase-2-student-on-new-shell-5axis-2026-06-16.md`](phase-2-student-on-new-shell-5axis-2026-06-16.md). Flag-free smokebook [`phase-2-student-new-shell-smokebook-2026-06-16.md`](phase-2-student-new-shell-smokebook-2026-06-16.md). **On re-smoke PASS → `merge --no-ff` into `v1-redesign`.** |
...
| **Uncommitted / unmerged** | **ONE unmerged feature branch off `v1-redesign`:** `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (merge after Andrew **DESKTOP** smoke; iOS tighten-pass deferred-safe). **DONE/MERGED:** `phase1/wb-review-correct` → `v1-redesign` @ [`f68053c`](https://github.com/Arangarx/tutoring-notes/commit/f68053c); `auth/2fa-remember-device` @ [`8ffd344`](https://github.com/Arangarx/tutoring-notes/commit/8ffd344); `fix/preview-badge-copy-commit` @ [`feccae9`](https://github.com/Arangarx/tutoring-notes/commit/feccae9). **SMOKEABLE (awaits re-smoke):** `phase2/wb-student-new-shell` head [`a644a64`](https://github.com/Arangarx/tutoring-notes/commit/a644a64) — hard-switch @ [`23e1cba`](https://github.com/Arangarx/tutoring-notes/commit/23e1cba), flag removed, all gates green. **Known pre-existing:** `identity-2fa-management` stale jest assertion; `test:wb-sync` broadcast-throttle half fail (acceptable). **Reset-screen generate-offer → BL-RESET-GENERATE** (backlog). |
