...
| **Next action(s)** | **PRE-MASTER gate = ONE re-smoke on `phase1/wb-review-correct` @ [`a3a94f7`](https://github.com/Arangarx/tutoring-notes/commit/a3a94f7):** LV-2 two-device — student enables cam after joining → tutor sees student video **paint on arrival without** manual cluster resize; resize still scales both tiles smoothly (no squish). Remaining failure is **purely video PAINT-on-arrival** (Andrew confirmed: hard-refresh scrub works; tile space allocated — track arrival + tile render OK). If LV-2 fails → `ResizeObserver` + `play()`/reflow on non-zero dimensions (see Last action successor note). *(Scrub re-smoke [`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08) — **CONFIRMED**.)* Smokebook [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — Andrew actively entering results (**do NOT edit**). **Once LV-2 passes →** `npm run test:wb-sync` + merge `--no-ff` to `v1-redesign`/`master`. Everything else confirmed or deferred. **Decision pending:** WB-NEW-SESSION-FALSE-RECOVERY (stale IndexedDB recovery banner on new sessions) — pull forward pre-master vs leave post-master (Andrew asked). **(2) P2 student-shell** execute-ready @ [`80ac571`](https://github.com/Arangarx/tutoring-notes/commit/80ac571) — awaiting greenlight (unchanged). **(3) Prior threads:** `phase1/wb-reliability-floor` @ `d63ac22` merge after Andrew desktop smoke; Gate A→A6 unchanged; `Experience-Driven Wedge` program + consent v2 when Andrew prioritizes. **Deferred post-master (Andrew 2026-06-16, captured in BACKLOG):** DRY viewport consolidation (**WB-DRY-VIEWPORT**), review transition animation polish (**WB-REVIEW-TRANSITION**), impersonation session-recovery bypass (**WB-IMPERSONATION-SESSION**), eraser reliability + partial erase (**WB-ERASE-RELIABILITY**), new-session false recovery banner (**WB-NEW-SESSION-FALSE-RECOVERY**). |
| **Open Andrew-confirms** | **P1 replay (2026-06-16) — confirmed:** replay first-play starts at 0:00 ✓; live+replay center tracking ✓; thumbnail-pin + button layout ([`8380bf8`](https://github.com/Arangarx/tutoring-notes/commit/8380bf8)) ✓; #5 audio pauses when leaving replay to notes-hero ✓; hard-refresh stored-recording scrub ([`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08)) ✓; in-page delete warning (no `alert()`) ✓; notes/form across prominent↔docked ✓; LV-1 self-view both sides ✓; idempotency #13 tentative (will reconfirm); legacy standalone replay D1-D3 skipped. **Pending re-smoke (pre-master gate — 1 item):** LV-2 two-device @ [`a3a94f7`](https://github.com/Arangarx/tutoring-notes/commit/a3a94f7) — student enables cam after join → tutor sees video **paint on arrival** without manual cluster resize; resize scales both tiles smoothly (no squish). **Clarified (Andrew 2026-06-16):** earlier "impersonation hid the sessions" was an **UNCONFIRMED hypothesis** — NOT the real cause (Continue/open-sessions list shows normally). Real recurring annoyance = **WB-NEW-SESSION-FALSE-RECOVERY** (stale IndexedDB recovery banner on new sessions) — already in BACKLOG; **decision still pending** pre-master vs post-master. **Resolved — deferred post-master:** DRY consolidation, transition animation #2, impersonation bypass, eraser reliability → [`BACKLOG.md`](../BACKLOG.md) § P1 replay-in-frame post-master follow-ups. **Standing:** (c) student URL · (d) learner-swap · (e) student camera default · (f) C1–C4 · (g) env scoping · B2 D-1/D-2/D-5 · B1 deferred TODOs · N-2. **P2 Q1/Q7** branch strategy + `defaultShowLocalVideo`. **Ratified 2026-06-11:** wall-clock metering. **Lawyer-needed:** VPC, retention, FERPA/SOPIPA. |
