...
> Integration base is `v1-redesign` @ [`a166f6c`](https://github.com/Arangarx/tutoring-notes/commit/a166f6c). Wave 4 done. **Wave 5 = LAST wave of the unify/stabilization plan (polish only — NOT "WB finished"; W1-3 backlog + Gate A5/A6 sync+replay + reliability-floor smoke + consent merge all still open after it).** Wave 5 split into 2 slices on branch `wb-wave5-polish`: **slice 1 (visual polish) DONE @ `f5de77e`** (code `3415f3a`) — coral student Exit + icon, follow/match icons, ⋯ overflow drops down + scroll affordance, grid icon toggle, More-sheet alignment; tsc/eslint/jest/next-build all green; tutor CSS append-only (pristine). **Slice 2 dispatched** (tab-highlight + view-lock + graph-entry). **Andrew design decisions (2026-06-21):** (a) student board tabs → **highlight active tab read-only**; (b) graph embeds → **interactive for students** (can enter expressions). **Deferred:** tutor phone-landscape over-compaction (R6-1) → NOT a bespoke tweak; needs the proper tutor-responsive/component pass since it requires editing tutor baseline CSS (the exact regression-prone surface). Sequencing answer to Andrew: Wave 5 → backlog burndown + consent merge + reliability-floor smoke → **A5/A6 sync+replay** → waiting-room wiring (#7, visual shell already merged) → master cut / ship-to-Sarah.
