...
**NEXT:** (a) **F3 — apply-remote:83 (Bug B / WS-X) realignment** = the sole remaining test-only relay red. **CAREFUL/ATTENDED per A4 WS-X row (Andrew deferred 2026-07-05):** WS-X is the most-fragile surface + multi-attempt history → settle the L146 onChange-leak question first, THEN realign the spec (test-only). Not a blind Composer fix — needs the attended design pass. (b) **Confirming attended relay run** (`test:wb-sync`, ~50min) — best run *after* F3 so it can go fully green in one shot; watch the :101 flake. Alternatively run now to lock the audio-upload fixes (expect only apply-remote:83 red). (c) **Bug A** `wb-tutor-recording-mute` (WS-I-PRESTART-MUTE, BACKLOG reliability-gap) — NOTE non-repro in relay runs #2/#3; **re-confirm reproduction first**, then (if real) fragile: plan-mode → Sonnet 5-axis → fix; green spec = merge bar before WS-I→master. (d) Then **Tranche E** (P2-WB-2 → P3).
