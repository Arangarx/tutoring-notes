...
| **Next action(s)** | (1) **⏳ AWAITING ANDREW WAVE 4 ROUND-6 RE-SMOKE.** Rounds 1-5 were bespoke-patch attempts that kept regressing (global-CSS churn); **STUDENT TOP BAR REBUILT correctly @ `64108cf`** — student now reuses the tutor's exact zone structure + same `desktop-only`/overflow-⋯ responsive mechanism with only 6 documented deltas; ALL tutor-affecting shared CSS reverted to `5d56f49` baseline (acceptance gate: `git diff 5d56f49 -- whiteboard-chrome.css` = pure additions, zero deletions — tutor provably restored); bespoke student override pile + bespoke JS removed. Round-6 re-smoke **R6-1..R6-5** added (tutor no-regression + student parity x3 sizes) to smokebook [`wb-wave4-responsive-smokebook-2026-06-19.md`](wb-wave4-responsive-smokebook-2026-06-19.md) (on `v1-redesign`; preview = same branch alias `tutoring-notes-git-wb-wave4-responsive-arangarx-5209s-projects.vercel.app` now serving `64108cf`). Investigation (2026-06-20) confirmed WB layout mode is **viewport-dimension-only** (`useWbLayoutMode.ts` width/height buckets), NOT touch/pointer — desktop-shrunk ≠ phone unless height&lt;500px enters `phone-landscape`. **Hard-won lesson:** when unifying surfaces, the secondary role must REUSE the primary's working component + layer deltas — NOT get bespoke markup + a global-override pile; the latter caused 5 rounds of jsdom-blind regressions. **Smokebook canonical copy = `v1-redesign`** (Andrew's round-1/2/3/4/5 results live here) → at `wb-wave4-responsive` merge, take `v1-redesign`'s version. On round-6 PASS → `merge --no-ff` `wb-wave4-responsive` into `v1-redesign`; on tutor regression → that's priority (merge-introduced). **Do NOT stack Wave 5 on this branch until smoked.** (2) **Wave 5** polish (post-Wave-4-smoke) — coral Exit btn, smaller Match-tutor's-view control, 2 design Qs (highlight non-clickable student tab? / allow student graph entry?), student view-lock-while-synced — see [`wb-unify-stabilization-plan-2026-06-17.md`](wb-unify-stabilization-plan-2026-06-17.md) § Waves 4–5 + W1-3 backlog IDs. (3) **`docs/phase3-consent-model` @ `4f9dbcd`** → union-merge to `v1-redesign` (conflict risk — see lesson below). (4) `phase1/wb-reliability-floor` @ `d63ac22` — awaits DESKTOP smoke. (5) Gate A→A6 + Ship-to-Sarah gate burndown per table below. |
