...
| **In-flight subagents** | **None.** Replay-in-frame chain complete (pivot→recon→plan Rev1-3→5-axis→build→smokebook reconcile). Awaiting Andrew's smoke. |
| **Uncommitted / unmerged** | **TWO unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `70b78eb` — Phase 1 plan/5-axis/1b-diff-review/smokebook + **1b audio-clock + 3-tier iOS-safe fallback + CSP fix (worklet now same-origin static file)**, VERIFIED-CLEAN. **1b decision (2026-06-13):** Andrew questioned re-architecting a working-on-desktop clock; agreed to a **one-cycle** finish (CSP fix was that cycle). **1b is KEPT but NOT smoked in isolation** — replay (its only consumer of replay-sync) is being rebuilt, and recon confirmed **replay runtime is independent of the 1b clock** (1b only fixes NEW recordings' event `t`). 1b merges with later WB work, no standalone smoke. (2) `phase1/wb-replay-in-frame` @ `52a9dae` (pushed) — **in-frame two-state replay BUILT + smokebook reconciled** (Rev 3): tsc clean, jest 91/91 replay, `next build` 0, **live `WhiteboardWorkspaceClient.tsx` engine UNTOUCHED** (empty diff — additive held). Smokebook [`...-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) = 13 in-frame items + D1–D3 deferred standalone (regression-only/SKIP). Preview alias [`...phase1-wb-rep-7d986c...`](https://tutoring-notes-git-phase1-wb-rep-7d986c-arangarx-5209s-projects.vercel.app) — prior deploy `40cbcfb` READY; tip `030a3c2` docs-only delta was INITIALIZING (wait for READY or smoke the READY one — feature code identical). **Known follow-up:** standalone admin/share `WhiteboardReplay` parity refactor DEFERRED (only restore-cache shared; legacy inline timeline retained) — in-frame path got the B1–B3 scrubber fixes; standalone unchanged (no regression, consistent with "stop patching the obsolete page"). **Orchestrator owes `npm run test:wb-sync` (relay/Docker) before merge.** **Next: Andrew smokes → if clean, run wb-sync → merge `--no-ff` → decide standalone-parity follow-up + B2/B3 → 1c.** `v1-redesign` tree clean. |

**🔁 Replay-in-frame pivot (2026-06-13, evening):** Andrew flagged the standalone WB replay page as **obsolete** — scrubber/scrubbing broken across multiple sessions; patching it ≈ same effort as doing replay right **in the WB frame**. **Decision:** abandon the standalone player; build replay **inside the WB frame** as a **two-state review surface**. Refined live with Andrew into: **State 1 = notes "hero"** (default on End Session; zero-typing goal; **reserved slot** for the Phase-2 "would you agree?" confirm section — layout-only now); **State 2 = full-viewport replay** that **visually mirrors the live board** (read-only style-mirror of the chrome, NOT live-engine reuse — additive per the twice-burned "extend don't rewrite" lesson) + an unobtrusive scrubber. Default Play → full replay; back → hero. **Notes drawer over read-only replay** (in scope) gives "type while watching" without a separate PiP/split (deferred). Architecture = **Option B isolated overlay** (`WhiteboardReplayInFrame`/`ReplayBoardChrome`/`ReplayCanvasSurface`/`ReplayTimelineScrubber` + `useReplayTimelineController` single-`globalMs` truth). Plan [`phase-1-wb-floor-replay-in-frame-plan-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-plan-2026-06-14.md) (Rev 3) + 5-axis [`...-5axis-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-5axis-2026-06-14.md) — 5-axis caught **2 BLOCKERs** (notes-state lost on hero↔replay unmount → **Path A lift state to `SessionReviewMode`** w/ controlled `TutorNotesSection`; replay controller persist unspecified → **lazy-mount-once + CSS-hide, never unmount, pause-on-hide**) + 5 SHOULD-FIX, all folded into Rev 3. Orchestrator decided NOTE-1 (empty session → gate on `hasAudio||eventCount>0`, else "No recording available") + NOTE-2 (shared hook for `replayCachedRestoreElements`). Deferred: "would you agree?" engine (Phase 2), PiP/split, A6-1 multi-segment, A6-6 multi-page, laser, scratchy tutor audio. **Mockup ref:** [`whiteboard-session-shell-design-2026-06-08.md`](whiteboard-session-shell-design-2026-06-08.md) §6 + mock `docs/brand-previews/whiteboard-session-shell-mock-2026-06-08.html` (#page-review). **wb-sync pre-merge gate** (`npm run test:wb-sync`) still owed by orchestrator before merge (touches `src/components/whiteboard`+`src/lib/whiteboard`, though read-only).
