...
- **SMOKE (round 2):** `security/durable-auth-2fa-limiters` @ **`2304f9e`** — Smoke-1 FAILED on UX only (raw-JSON black screen, no on-screen msg/timer); **diagnosis: the new limiters are correctly scoped + return inline errors — the JSON screen came from the PRE-EXISTING middleware `auth:<ip>` limiter (in-memory, 10/min) returning JSON for full-page nav.** Fix @ `2304f9e`: `rateLimitResponse()` returns HTML for non-API paths; inline 429 handling on `/login`, `/account/login`, claim login; password reveal-icon suppression (`::-ms-reveal`/`::-webkit-credentials-auto-fill-button` hidden + `pr-20`) for the eye/Show overlap. **Re-smoke: wait 60s after burst-testing (in-memory window), then expect friendly HTML/inline "Too many attempts — wait N seconds" instead of raw JSON.** NOTE: this branch predates the schema-test fix (`b61d639`), so it still shows the 1 pre-existing `identity-p2-schema` RED — resolves on merge to `v1-redesign`. Then merge if pass.
...
**⚠️ RELIABILITY-CRITICAL — Pause/Resume recording DATA LOSS (NEW, likely pre-existing on master):** Andrew H-test: using **"Pause recording"** (the button in the WB-session block w/ the pills) then resuming → **only the chunk up to the pause survives; ALL post-resume audio AND strokes are LOST; recording stops at pause.** Pause shows a "Recording saved" message + starts a NEW timer with no indication of the original. **→ Investigation dispatched:** is post-resume content truly never captured, or captured as a 2nd `SessionRecording` segment that replay ignores (analysis flagged "replay uses first SessionRecording only")? Either way user-observable = data loss → reliability-bar breach; needs a targeted fix, not just the lifecycle redesign.
