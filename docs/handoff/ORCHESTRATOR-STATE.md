...
## ⏩ HEAD — 2026-06-07 (whiteboard chrome design pass complete; Phase 0 POC in flight)

| Field | Value |
|---|---|
| **Last action completed** | **Whiteboard chrome design pass complete** — forks ratified (hybrid tutor layout, tutor-mobile defer+TM-09, fail-fast Phase 0 POC gate), durable design doc [`docs/handoff/whiteboard-chrome-design-2026-06-07.md`](whiteboard-chrome-design-2026-06-07.md), requirements/BACKLOG/INDEX updated (63 reqs incl. TM-09). **Phase 0 feasibility POC dispatched** (spike branch, sync-free throwaway runtime). Prior: IAC-13 disconnect-copy FIXED @ [`05af41f`](https://github.com/Arangarx/tutoring-notes/commit/05af41f); legacy JWT role-fallback DEFERRED @ [`4eb355f`](https://github.com/Arangarx/tutoring-notes/commit/4eb355f). **Exposure model:** `v1-redesign → master` = **Sarah reveal** — no UI-skin feature flag. **Operating model:** **`master` stays frozen**; whole redesign cooks on **`v1-redesign`**; isolated **cherry-pick to `master`** is the lever for any urgent Sarah fix. |
| **Next action(s)** | **Await Phase 0 POC result** (Andrew smoke on Vercel preview) → on green, dispatch **Phase 1 tutor-desktop chrome** per design doc §6. **Pre-master gates still open:** (1) component-pass cohesive visual review; (2) whiteboard chrome **build** (design done; build gated on POC). Cut `master` only when whole site (incl. whiteboard chrome) is one cohesive reveal. |
| **Open Andrew-confirms** | (1) **IAC-13 corrected-copy smoke** — Disconnect dialog copy on claimed student. (2) Component-pass **cohesive visual review** (pre-master). (3) **Phase 0 POC smoke** on spike preview — hide/tools/undo/pen/viewport. (4) When POC + visual review + chrome build pass: green-light `v1-redesign → master` (= Sarah reveal). |
| **Open v1 requirements** | **Light/dark = per-component build standard (no separate pass)** — theme plumbing + toggle slot into component-pass **first slice** (A′); enforced via `V1-COMPONENT-LIBRARY.md` §2.11 HARD gate + `.cursor/rules/both-theme-components.mdc` (tracked: `BACKLOG.md` § V1 redesign, whiteboard **TU-12**). |
| **In-flight subagents** | **Phase 0 whiteboard chrome feasibility POC** (spike branch). |
