...
## ⏩ HEAD — 2026-06-11 student-WB migration steps 1-2 merged; trust model decided

| Field | Value |
|---|---|
| **Last action completed** | **Student-WB migration steps 1-2 merged to `v1-redesign`** @ [`dfb221c`](https://github.com/Arangarx/tutoring-notes/commit/dfb221c) (no-behavior-change tutor-only refactor: `wb-role.tsx` role model + `deriveWbCapabilities` per design §7.2/§7.5.1; `LiveBoardChrome.tsx` presentational shell extraction; tutor delegates into slots). Green on `next build` + `test:wb-sync` (13/1-skip) + 115 regression. Andrew merged on gate strength (pure parity change). **Prior context:** wb-sync gate RED→GREEN earlier (B1 harness seed gap, not a WB regression; fixed `bd0894c`, merged). **Trust-model + COPPA threads resolved** (see below). |
| **Next action(s)** | **X2 = finish v1 design across ALL user-facing surfaces**, shared-components-first; design-gap inventory committed @ [`d722495`](https://github.com/Arangarx/tutoring-notes/commit/d722495) (`docs/handoff/v1-design-gap-inventory-2026-06-11.md` — re-find exact filename if needed). **Next concrete item: student-WB migration steps 3-9** (flag-gated `StudentLiveBoardShell` wiring + cutover) — needs Andrew confirms (c) URL keep-vs-retire + (e) camera default before step 4; cutover (step 9) requires real 2-device tutor+student hardware smoke. COMPREHENSIVE re-smoke deferred until v1 design complete. |
| **Open Andrew-confirms** | **RESOLVED:** (a) Encryption trust model = **app-server-brokered session key** (relay stays blind; portal-join works) — confirmed; legally clear per COPPA spike. (b) Stored-content operator-blind encryption = **backlogged** as app-managed envelope-encryption + KMS-separated custody, triggered pre-world-release alongside Andrew's end-of-summer org-readiness project (`docs/BACKLOG.md` Security; rationale @ [`982e78b`](https://github.com/Arangarx/tutoring-notes/commit/982e78b); COPPA spike @ [`22d2886`](https://github.com/Arangarx/tutoring-notes/commit/22d2886) — **no operator-blind legal mandate**). **STILL OPEN:** (c) student URL keep `/w/[joinToken]#k=` + login (rec) vs retire sent-link (Andrew leaning retire); (d/learner-swap) draft offered, awaiting go; (e) student camera default = self-view OFF working default, confirm w/ Sarah; (f) C1–C4 before `CONSENT_ENFORCEMENT`; (g) `NOTES_AUTH_WALL`+`CONSENT_ENFORCEMENT` env scoping at master cut. Standing: B2 D-1/D-2/D-5, B1 deferred TODOs, N-2. Lawyer-needed (COPPA spike): VPC method/copy, retention timeframe in /privacy, FERPA/SOPIPA if pitching schools. |
