...
| **Uncommitted / unmerged** | **`v1-redesign` @ [`b57dfa6`](https://github.com/Arangarx/tutoring-notes/commit/b57dfa6)** (slice-3 auto-notes + auth-role-refresh + 404-consistency + Target-B smoke notes all IN, on top of component + IAC-13 + notes-UX + whiteboard reqs/TU-11 + docs-policy + archive sweep). **Main tree now ON `v1-redesign`, working tree CLEAN.** **Unmerged branches → v1-redesign:** `feature/sarah-forward-migration-q6` @ `a396ab5` (parked). **MERGED (retained for branch-sweep cleanup):** `feat/recording-p1-slice3-autonotes`, `harden/auth-role-refresh` + assorted stale worktrees (`tn-auth-harden`, `tn-docs-notes`, `.cursor/worktrees/*`). |
