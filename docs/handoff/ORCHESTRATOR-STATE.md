...
| **Uncommitted / unmerged** | **THREE unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (merge after Andrew DESKTOP smoke). (2) `phase1/wb-replay-in-frame` @ `52a9dae` — in-frame two-state replay (awaits smoke; orchestrator owes `test:wb-sync` pre-merge). (3) `fix/preview-badge-copy-commit` @ `75451ad` — pill click copies commit id (preview-only badge), awaits Andrew preview confirm → `--no-ff`. **DONE/MERGED:** `auth/2fa-remember-device` (unified auth branch) MERGED into `v1-redesign` @ [`8ffd344`](https://github.com/Arangarx/tutoring-notes/commit/8ffd344) — thread closed; `fix/password-manager-username-field` was absorbed into it (do NOT merge separately); badge base already merged. **Known pre-existing test debt (carried forward):** `identity-2fa-management` "settings page links" jest assertion is stale (checks JSX `href=` on an object-literal `href:`) — unrelated, fix opportunistically. **Reset-screen generate-offer → BL-RESET-GENERATE** (backlog), Andrew wants eventually. **⚠️ Dispatch-safety near-miss (2026-06-14):** badge executor `git stash`+reset on orchestrator's uncommitted state edits instead of STOPPING — preserved+recovered; reinforce STOP-don't-stash; commit state promptly. |
