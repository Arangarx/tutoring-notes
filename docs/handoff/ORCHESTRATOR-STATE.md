...
## ⏩ HEAD — 2026-07-01 Erasure Steps 1–3 shipped (reversible tombstone, Option A) @ 11a812c; Sonnet 5-axis review PASS

> **Active branch:** [`wb-wave5-polish`](https://github.com/Arangarx/tutoring-notes/tree/wb-wave5-polish) @ [`11a812c`](https://github.com/Arangarx/tutoring-notes/commit/11a812c) (+ state-doc commit on top). **Worktree:** `tutoring-notes-polishwt` (default `tutoring-notes` checkout is on `v1-redesign` — NOT current). **All remaining Sarah-gate work lands here; single `merge --no-ff` to `v1-redesign` at the FINAL full-arc gate only (no interim merge — Andrew reaffirmed 2026-07-01).** Integration base remains **`v1-redesign` @ [`7397abc`](https://github.com/Arangarx/tutoring-notes/commit/7397abc)**.
>
...
| Field | Value |
|---|---|
| **Last action completed** | **Erasure Step 3 shipped @ [`11a812c`](https://github.com/Arangarx/tutoring-notes/commit/11a812c)** (ER-2/ER-5/ER-7; BLOCKERs A/C/D/E/F) — reversible tombstone (Option A): tombstone soft-disables credentials + preserves ALL AH/LP PII through grace; `scrubDbContent` hard-redacts identity PII + deletes credentials at grace expiry (**PII dies at purge, not tombstone**); cancel = single `$transaction` **true restore** (untombstone + re-enable, full-family recursive); `advancePhase` conditional `updateMany` + abort-on-count-0 (TOCTOU D/F); `pg_advisory_xact_lock(int,int)` on cancel+re-request (E). 58 erasure/admin tests green vs local `tutoring_notes_test`; `next build` 0. **Sonnet 5-axis review PASS — no code BLOCKERs** (worker-vs-cancel race F correctly handled by D's row-level conditional update); 2 HIGH (test-only, pre-merge) + 3 MEDIUM to fold in Step 3.5. — — — **Prior:** Steps 1+2 (BLOCKER B @ [`d3458f9`](https://github.com/Arangarx/tutoring-notes/commit/d3458f9), I @ [`ac963a3`](https://github.com/Arangarx/tutoring-notes/commit/ac963a3)); migration `20260701120000_learner_credential_disabled` live on preview-dev; 3 orphan jobs canceled; `.env`→preview-dev (not prod). |
| **Next action(s)** | **Step 3.5 hardening** (fold 5-axis findings): MEDIUM — fix duplicate `action=` log key in `tombstoneLearnerProfile`, add `advancePhase`-abort unit test, emit `soft_disable_credential` from `tombstoneAccountHolder`; HIGH (pre-merge) — AH-PII purge-oracle assertions in full-family test + `account_holder`-scope cancel-restore integration test. Then **Step 4** (ER-3, BLOCKERs G/H): erasure guards on `createWhiteboardSession`+`startWhiteboardSession` + extend `assertStudentNotErased`(+Api) to active `ErasureJob` via shared util + **flip the deferred grace assertion** (~L429-433) + apply to ALL content routes (events/snapshot/audio/tutor-asset). One subagent, **two serialized commits**. → **Sonnet review of Step 4 diff (auth boundary)** → **Step 5/6** (ER-4 UI + ER-6 copy) → **CF-2** (wb-sync-gated) + **CF-4** → **Workstream C** Playwright e2e → **checkpoint re-smoke (NO merge)** → **Part 3** spine → **full-arc both-themes hardware smoke** → single `merge --no-ff → v1-redesign`. **All DB-test runs override `DATABASE_URL`/`DIRECT_URL` to local `tutoring_notes_test` (never Neon).** |
