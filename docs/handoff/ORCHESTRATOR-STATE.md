...
**🎯 F2b attended sharded relay run DONE (2026-07-07) — core sharding WIN validated; 2 more runner bugs + 1 product data-point surfaced.** Run #1 aborted instantly: F2a runner passed spec paths where Playwright expected `--project=wb-regression` (Windows `spawnSync shell:true` arg-concat) → **FIXED [`38ce3c3`](https://github.com/Arangarx/tutoring-notes/commit/38ce3c3)** (combined `--project=wb-regression` + positional file filters; `--list` proof 24 tests/5 files). Run #2 (post-fix, jest pre-gate **856/856**, auth.setup green): **shards 1–5 = 135 passed / 0 failed on fresh per-shard :3100 dev servers** — lifecycle 49✅ (WS-T #7 confirmed green — was UNVERIFIED in the marathon), shard-2 22✅, shard-3 22✅, shard-4 23✅, shard-5 19✅. **The anti-exhaustion sharding works** (vs marathon's 22 reds). **BUT two NEW runner defects + one product data-point:** **(1) shard-6 collapsed** — `expected:0, unexpected:1, skipped:22`; first test failed on environmental exhaustion symptoms (login `toHaveURL(/\/workspace$/)` 45s timeout, `micSelect` 30s visibility timeout) → **cross-shard machine-level resource accumulation** by the 6th server (fresh-per-shard fixes within-shard marathon exhaustion, NOT whole-machine accretion). **(2) Isolation re-run OVER-COLLECTS** — claimed "99 failures" and its first re-run target (`wb-chrome-interactions:39`) *passed* in shard-2 → classifier is re-running passed/skipped specs; a ~2.5h runaway. **Orchestrator STOPPED the runaway** (killed runner PID tree; freed :3100/:3101 via safe helper; **:3002 + Postgres untouched, still up 27h/28h**). **(3) Bug A `wb-tutor-recording-mute` #19 did NOT reproduce** in shard-4 (23/23 green) — contradicts the earlier "CONFIRMED REAL / deterministic fail initial+retry" isolated finding → mute-before-graph-ready race is timing/environment-sensitive; re-confirm before the Bug A fix thread. **NEXT (awaiting Andrew steer):** dispatch Composer to (a) fix isolation collection to re-run ONLY genuine `unexpected` failures (not passed/skipped), (b) harden cross-shard (kill lingering node + inter-shard cooldown, and/or split shard-6) → then re-run F2b. Full-run terminal log: `terminals/634468.txt`.
