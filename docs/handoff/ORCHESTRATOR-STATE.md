...
| **In-flight subagents** | **Sonnet follow-sync fix** (claude-4.6-sonnet, background) — fixing the Wave 5 view-lock regression on `wb-wave5-polish`. **REGRESSION FOUND in Andrew's Wave 5 smoke (2026-06-21):** student stops following tutor once tutor zooms-out/pans far ("can only follow so far / old center leaves top-left quadrant"). Root cause (investigation [`a7ebdbfc`](a7ebdbfc-7917-446b-8084-306160665801)): the slice-2 view-lock `d1f770e` `onCanvasChange` reverts ANY viewport ≠ `followLockedViewportRef` — including tutor-driven applies Excalidraw clamps at extreme pan/zoom. Pre-existing amplifiers: `runV3Apply` clears `applyingRemoteRef` before the rAF viewport write; `pageViewState`/`wire-recv` uses stale `centerScene`. NOT on `a166f6c` base (base `onCanvasChange` ignored appState). Keep the lock (Andrew likes it); fix = lock from actual `getAppState`, compare scene-center, revert only on confirmed user input, extend remote-apply guard across rAF. **jsdom-blind class → real gate = `test:wb-sync` + Andrew hardware re-smoke, not jest alone.** Andrew confirmed working in same smoke: active-tab highlight (#6) + view-lock-blocks-student-pan (#7). |
