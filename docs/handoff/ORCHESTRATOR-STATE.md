...
| **In-flight subagents** | **E3 (erasure blob inventory)** — Composer 2.5 `generalPurpose`, dispatched 2026-07-01. Scope: `src/lib/erasure/blob-inventory.ts` (`enumerateLearnerFamilyBlobs`) reusing `collectReplayAssetUrls`; all blob sources incl. `chunkBlobUrl` + checkpoint prefix; H-2 re-callable; tests with mocked Blob fetch. (E2 complete @ `7792f7d`, pushed.) |
