...
| **CC-1 — no session without claimed student** | ✅ **RATIFIED Sarah-merge BLOCKER (Andrew 2026-06-30)** — tutor cannot start/create a whiteboard session unless `Student` is claimed (linked to `LearnerProfile`). Closes hole (3) unclaimed tutor-created Student; tutor gets "invite the parent to claim before starting a session" affordance. Implications: existing unclaimed students must be claimed at cutover or can't start sessions; admin/test accounts need claimed test students. [`BACKLOG.md`](../BACKLOG.md) `CONSENT-COLLECTION-COMPLETENESS`. |
| **CC-2 — claim requires explicit consent choice** | ✅ **RATIFIED Sarah-merge BLOCKER (Andrew 2026-06-30)** — parent cannot finish learner claim without choosing "Save preferences" OR explicit "No consent now, I'll review later"; **either** choice writes a `ConsentRecord` (decline → ALL-OFF record). Closes hole (1) claim-complete-without-consent. Warning copy when claim leads to active session invite (DRAFT — Andrew approval pending). Claim flow: `app/claim/[token]/setup` + complete route. |
