...
| WS-E E4 (replay active board tab) + E5 (roughness) | ⬜ IN PROGRESS | bundled (both touch WhiteboardWorkspaceClient); E5 first (trivial captureUpdate), then E4 (page-switch WBEvent + replay tab strip). Relay teeth. |
| WS-E E6 (student mic persistence) | ⬜ queued (serial) | A/V surface (useLiveAV/storage — additive persist, NOT reconnect logic); after E4/E5 |
