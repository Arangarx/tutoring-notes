...
> **Wave 5 agent DONE checklist:** All 13 smokebook items mapped to **wb-regression** Playwright. **New:** `tests/integration/wb-wave5-polish.spec.ts` (items 1–7, 10, 11, 13). **Extended:** invariants **12d** (equal vertical inset + midline), **12e** (student→tutor graph UI persist); **12b/12c** + `wb-student-exit-rejoin.spec.ts` cover items 8/12 + rejoin/banner. **Product fixes:** `ReviewBoardThumbnail` graph detection/rebuild; E2E tutor scene hook calls `recorderOnCanvasChange` for review thumbnail; prior fix wave (graph persist, rejoin, banner, follow CSS, loading guard) retained. **Gates:** `test:wb-affected:run --base origin/v1-redesign` green (1 flaky invariant 7 retry); `test:wb-sync` in flight this turn. **Next:** Andrew **one** hardware smoke per [`smoke-when-done.mdc`](../../.cursor/rules/smoke-when-done.mdc) — human judgment only ([`wb-wave5-polish-smokebook-2026-06-21.md`](wb-wave5-polish-smokebook-2026-06-21.md)); merge blocked until PASS.
...
| **Wave 5 status** | **Agent DONE** — all 13 smokebook items in wb-regression; gates green (affected + wb-sync pending). Awaiting Andrew single smoke. Prior smoke OVERALL FAIL @ `32ef3f5`. |
| **Uncommitted / unmerged** | **`wb-wave5-polish`** — Playwright + thumbnail fixes committing this turn; push to origin. |
