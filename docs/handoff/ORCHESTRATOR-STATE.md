...
| **In-flight subagents** | **None.** **BLOCKER open:** graph equation plot + bidirectional sync broken on hardware (Andrew 2026-06-23). Prior: follow-lag fix (`c4fff44`), styles portal (`6652522`), invariant 13 (`32ef3f5`), wb-sync GREEN @ `32ef3f5` — **does not cover interactive graph UI**. |
...
| **Uncommitted / unmerged** | **`wb-wave5-polish` @ `32ef3f5`** (pushed) — **merge BLOCKED** by graph regression (Andrew 2026-06-23). Awaits fix + re-smoke #8/#12. Other gates: wb-sync GREEN; styles pop-out + follow-lag fixes landed; Andrew re-smoke items 7/7a/9–13 + regression fixes still in progress. **QUEUED:** `chore/test-db-safety` after graph fix. Smokebook [`wb-wave5-polish-smokebook-2026-06-21.md`](wb-wave5-polish-smokebook-2026-06-21.md). |
