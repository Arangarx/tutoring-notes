...
**Branch tip: `2f105da`** (pushed). **NOT merged** — still parked at the merge gate for Andrew's hardware re-smoke after this wave lands.
...
| 🔑 KEYSTONE — Blob-gate skip fix | 🔵 in-flight (readonly investigation) | `webServer` never sets `BLOB_READ_WRITE_TOKEN` → ~20 durability specs (incl. WS-L scrub, VAD/tab-kill, notes-shimmer, PDF) `test.skip` at the gate. Investigating hermetic-stub vs token-passthrough before implementing (`playwright.config.ts` hot file → serialize all code-writers). **#1 next task.** |
