...
**Branch tip: `6799aa4`** (pushed). **NOT merged** — still parked at the merge gate for Andrew's hardware re-smoke after this wave lands.
...
| WS-S review-overlay hydration verify | 🔵 in-flight (readonly) | subagent investigating whether the review overlay reliably loads strokes+audio+notes on all 3 End paths (may be WS-N symptom now fixed, or WS-N4 gap, or distinct). Fragile (hydration/finalize) → flag before any edit. |
| PART 2a site-wide coverage audit | 🔵 in-flight (readonly, background) | building `docs/handoff/site-wide-coverage-audit.md` — enumerate every mechanical surface + map coverage + prioritized behavior-test gap list; feeds the PART 2 build-out. |
