...
## ⏩ HEAD — 2026-06-27 **Plan #1 — Andrew smoke IN PROGRESS** on `wb-wave5-polish` (tip [`60ec652`](https://github.com/Arangarx/tutoring-notes/commit/60ec652)) — 2 smoke fixes landed; Plan #2 next

> **SMOKE IN PROGRESS (2026-06-27, Andrew live).** Plan #1 checkpoint was green @ `6b05e05`; Andrew's hardware smoke then surfaced student-side gaps, now FIXED on branch: **(1)** copy-student-link was missing from the PENDING waiting room — chicken-and-egg (tutor couldn't share the `/join` link, so the student could never connect to enable LIVE Start) → [`fa7ffe0`](https://github.com/Arangarx/tutoring-notes/commit/fa7ffe0). **(2)** student was built on the WRONG paradigm — a separate dead-end "your tutor will let you in" placeholder (`LearnerWaitingRoom` + fake `StudentDevicePreview`, no getUserMedia, action-framed mic/cam, no path into the real room). Retired the placeholder + converged the student onto the SAME `WaitingRoomOverlay` as the tutor: mutual-presence copy, chip-toggle mic/cam (reuse `mynk-wb-follow-toggle`), real self-camera tile, honest `/join` no-session card → [`60ec652`](https://github.com/Arangarx/tutoring-notes/commit/60ec652). Both fixes Playwright-locked (`wb-session-lifecycle.spec.ts`, `@wb-presence`/`@wb-chrome`), `next build` + tsc + lint green. **Deferred (Andrew):** logged-in "Join session" learner surface (tutor link = pilot entry path); consent-dialog removal stays **Plan #2**. **⚠️ Andrew's smoke notes are UNCOMMITTED in [`plan1-lifecycle-authed-join-smokebook.md`](plan1-lifecycle-authed-join-smokebook.md) — do NOT clobber / git restore / checkout that file.** Awaiting re-smoke on preview @ `60ec652`.
...
| **In-flight subagents** | **None.** Final checkpoint re-confirm completed GREEN @ `6b05e05`. |
...
| **Uncommitted / unmerged** | **`wb-wave5-polish`** pushed @ `6b05e05`; **NOT merged** — per Plan #1, no merge until final Sarah end-to-end gate. Integration base remains `v1-redesign`. |
