...
## ⏩ HEAD — 2026-07-01 Erasure FEATURE COMPLETE (backend A–I + tutor UI/copy/docs) @ 51e5bfd; next CF-2/CF-4 (root-cause explore in flight)
...
| **Next action(s)** | **CF-2/CF-4 root-cause explore (in flight, read-only)** — ground the fix before touching code; CF-2 is on the **fragile whiteboard event/apply surface** (wb-sync-gated) so confirm the fix is additive (decouple event logging from audio-capture policy; MB-4 IN_PERSON-no-replay) vs a real engine change (→ escalate). CF-4 = MB-5 tutor_only produces no notes. → dispatch fix(es) per explore findings (serialize on shared worktree; CF-2 wb-sync at merge boundary) + re-confirm MB-1/CF-1 → **Workstream C** Playwright e2e (consent CC-1/CC-2 + erasure cancel-restore/access-suspension) → **checkpoint re-smoke (NO merge)** → **Part 3** spine → **full-arc both-themes hardware smoke** → single `merge --no-ff → v1-redesign`. **All DB-test runs override `DATABASE_URL`/`DIRECT_URL` to local `tutoring_notes_test` (never Neon).** |
...
| **In-flight subagents** | **1 (explore, read-only):** CF-2/CF-4 root-cause. Step 5/6 Composer subagent DONE (pushed @ `51e5bfd`); erasure feature complete. |
