...
## ⏩ HEAD — 2026-06-08 (Phase 0 POC GREEN; Phase 1 started)

| Field | Value |
|---|---|
| **Last action completed** | **Phase 0 whiteboard chrome feasibility POC = GREEN** (Andrew real-device smoke 2026-06-08) — every Mynk toolbar button works; native Excalidraw chrome fully hidden (screenshot-confirmed: no tool rail, hamburger, properties panel, or footer); Ctrl+Z and Ctrl+Y keyboard OK; generic stylus OK. Core assumption proven: hide native chrome + drive Excalidraw via `excalidrawAPI`. Verdict recorded [`docs/handoff/POC-SMOKE-wb-chrome-2026-06-07.md`](POC-SMOKE-wb-chrome-2026-06-07.md). Prior: whiteboard chrome design pass complete; IAC-13 disconnect-copy FIXED @ [`05af41f`](https://github.com/Arangarx/tutoring-notes/commit/05af41f). **Exposure model:** `v1-redesign → master` = **Sarah reveal** — no UI-skin feature flag. **Operating model:** **`master` stays frozen**; whole redesign cooks on **`v1-redesign`**; isolated **cherry-pick to `master`** is the lever for any urgent Sarah fix. |
| **Next action(s)** | **Phase 1 — tutor-desktop hybrid chrome STARTED** — leading with foundational **theme plumbing slice (A′)** per the per-component theme standard; remaining Phase 1 slices: chrome-over-workspace (`test:wb-sync`-gated), toolbar+tool-wiring, properties popover + drawing defaults. Spike worktree `spike/wb-chrome-poc-857e6d4c` **RETAINED** (not deleted yet) — Phase 1 lifts its verified CSS hide-selector list; cleanup after that. **Pre-master gates still open:** (1) component-pass cohesive visual review; (2) whiteboard chrome **build** (POC green; build in flight). Cut `master` only when whole site (incl. whiteboard chrome) is one cohesive reveal. |
...
| **In-flight subagents** | **Phase 1 tutor-desktop hybrid chrome** (theme plumbing A′ slice). |
