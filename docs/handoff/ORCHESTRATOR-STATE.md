...
## ⏩ HEAD — 2026-07-01 Erasure Steps 1–4 shipped @ 6e46e16; Step 4 5-axis review DONE → 1 HIGH (share-link content hole) → Step 4.5 fix in flight

> **Active branch:** [`wb-wave5-polish`](https://github.com/Arangarx/tutoring-notes/tree/wb-wave5-polish) @ [`6e46e16`](https://github.com/Arangarx/tutoring-notes/commit/6e46e16) (+ state-doc commit on top). **Worktree:** `tutoring-notes-polishwt` (default `tutoring-notes` checkout is on `v1-redesign` — NOT current). **All remaining Sarah-gate work lands here; single `merge --no-ff` to `v1-redesign` at the FINAL full-arc gate only (no interim merge — Andrew reaffirmed 2026-07-01).** Integration base remains **`v1-redesign` @ [`7397abc`](https://github.com/Arangarx/tutoring-notes/commit/7397abc)**.
>
...
| Field | Value |
|---|---|
| **Last action completed** | **Erasure Step 3.5 + Step 4 shipped.** **Step 3.5** @ [`6631b5e`](https://github.com/Arangarx/tutoring-notes/commit/6631b5e) — folded all 5-axis findings: duplicate `action=` log-key fix + `soft_disable_credential` event (MEDIUM 1/2), `advancePhase`-abort TOCTOU unit test (MEDIUM 3), **AH-PII purge oracle** assertions + **`account_holder`-scope cancel-restore + login** integration tests (HIGH 1/2). **Step 4 / ER-3** @ [`6e46e16`](https://github.com/Arangarx/tutoring-notes/commit/6e46e16) (BLOCKERs G/H) — new shared `active-erasure-scope.ts` predicates + `ErasureAccessSuspendedError`; `assertStudentNotErased`(+Api) extended to active `ErasureJob`; guards on `createWhiteboardSession`+`startWhiteboardSession` (after ownership, `[ers]` logs); flipped the deferred grace assertion (~L431 → now FAILS during grace); content routes (events/snapshot/audio/tutor-asset) already call `assertStudentNotErasedApi` so extension is centralized (no per-route edits). **70/70 erasure+admin jest green vs local `tutoring_notes_test` @ 127.0.0.1:5432** (never Neon); both `npx next build` exit 0. Touched `.../whiteboard/actions.ts` (session lifecycle — additive guards only); **NOT** `src/lib/whiteboard`/apply paths → `test:wb-sync` deferred to merge boundary. — — — **Prior:** Step 3 @ [`11a812c`](https://github.com/Arangarx/tutoring-notes/commit/11a812c) (5-axis PASS); Steps 1+2 (@ [`d3458f9`](https://github.com/Arangarx/tutoring-notes/commit/d3458f9), @ [`ac963a3`](https://github.com/Arangarx/tutoring-notes/commit/ac963a3)). |
| **Next action(s)** | **Step 4.5 fix (in flight)** — close the review's HIGH: add active-erasure guard to the 6 family-facing SHARE-LINK surfaces (`/api/whiteboard/[id]/public-events`, `/api/whiteboard/[id]/public-snapshot`, `/api/audio/[id]?token=`, `/s/[token]/*` pages) after share-access resolves `studentId` (same centralized `assertStudentNotErased`(+Api) predicate — BLOCKER H "ALL content routes"). Fold LOW (`[ers]` denial logs on content routes) + MEDIUM (guard fail-CLOSED with clean 503/deny on DB error, not 500). Track deferred in BACKLOG: in-flight ACTIVE-session checkpoint-write TOCTOU (fragile surface — separate), admin student-detail/notes metadata pages. Ship guard-lift tests (share access denied during active job, restored after cancel). → **Sonnet re-review IF the fix is non-trivial** → **Step 5/6** (ER-4 pending-erasure UI + ER-6 copy) → **CF-2** (replay/audio decouple — IN_PERSON must replay; wb-sync-gated) + **CF-4** (tutor_only produces notes); re-confirm MB-1/CF-1 → **Workstream C** Playwright e2e → **checkpoint re-smoke (NO merge)** → **Part 3** spine → **full-arc both-themes hardware smoke** → single `merge --no-ff → v1-redesign`. **All DB-test runs override `DATABASE_URL`/`DIRECT_URL` to local `tutoring_notes_test` (never Neon).** |
