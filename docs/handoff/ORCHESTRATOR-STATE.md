...
## ⏩ HEAD — 2026-07-01 Erasure complete + CF-2/CF-4 shipped @ 3c326d9; CF-2 diff under Sonnet review (event-clock divergence risk)
...
| Field | Value |
|---|---|
| **Last action completed** | **CF-2 + CF-4 shipped.** **CF-2/MB-4** @ [`853bba4`](https://github.com/Arangarx/tutoring-notes/commit/853bba4) — decoupled WB event recording from audio policy via additive `wbEventsActive = role!=="student" && phaseActive && userWantsRecording` (NO audio check); passed as `recordingActive` + `getAudioMs=useAudioMsClock(wbEventsActive)` + viewport-anchor at call sites; FSM `tutorWantsRecording` audio gate + audio-bridge policy gates UNCHANGED → IN_PERSON+denied now persists strokes & replays, still no audio/SessionRecording. 27/27 jest (recorder-lifecycle + audio-policy incl. Gate A regression green). **CF-4/MB-5** @ [`3c326d9`](https://github.com/Arangarx/tutoring-notes/commit/3c326d9) — `triggerNotesGenerationAction` now returns `{ok,error?}` (was void, silently `.catch`-swallowed) + `[tnt] action=trigger_notes_failed` log; `handleEndSession` awaits + surfaces failure via `endingState=error` banner AFTER seal; tutor_only→enqueue→reduce→`TutorNote done` integration test (local DB, OpenAI mocked). 34/34 jest. Both `next build` 0; NO FSM/engine/apply/outbox/`src/lib/whiteboard` edits (workspace client at `.../whiteboard/[wbid]/workspace/WhiteboardWorkspaceClient.tsx` — IS a wb-surface → `test:wb-sync` at merge boundary). Local DB @ 127.0.0.1 only. — — — **Prior:** Erasure FEATURE COMPLETE (Step 5/6 @ [`51e5bfd`](https://github.com/Arangarx/tutoring-notes/commit/51e5bfd) + backend A–I). |
| **Next action(s)** | **Sonnet 5-axis review of CF-2 diff [`853bba4`] (in flight, read-only)** — PRIMARY question: `getAudioMs` was repointed from FSM `recordingActive` to `wbEventsActive` for ALL modes; `wbEventsActive` does NOT pause on participant-disconnect (executor's deliberate tradeoff). Does this DESYNC whiteboard-event timestamps from the AUDIO track in audio-recording modes (full / tutor_only LIVE) on pause/resume/disconnect — a stroke↔audio replay-drift regression? (jsdom blind spot — reason about the clock, don't trust green units.) Also: no engine/FSM regression; IN_PERSON replay correct. → per verdict, fix if needed → **re-confirm MB-1/CF-1** (Start regression) → **Workstream C** Playwright e2e (consent CC-1/CC-2 + erasure cancel-restore/access-suspension) → **checkpoint re-smoke (NO merge)**; **run `npm run test:wb-sync` at merge boundary** (WhiteboardWorkspaceClient is a wb-surface) → **Part 3** spine → **full-arc both-themes hardware smoke** → single `merge --no-ff → v1-redesign`. **All DB-test runs override `DATABASE_URL`/`DIRECT_URL` to local `tutoring_notes_test` (never Neon).** |
...
| **In-flight subagents** | **1 (Sonnet, read-only):** 5-axis review of CF-2 diff (`853bba4`) — event-clock divergence risk. CF-2+CF-4 fix subagent DONE (pushed @ `3c326d9`). |
