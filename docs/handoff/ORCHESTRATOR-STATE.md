...
**NEXT — TRIAGE (awaiting Andrew steer):** the 2 `audio-upload` reds need real-regression-vs-stale-oracle triage (dispatch `explore`: do these pass on `master`? when did they last pass? is `audio-upload` in go-to-Sarah scope, and is the dropzone-hidden / transcribe-disabled a product regression on this branch?). Then: **F3** = realign Bug B apply-remote spec (test-only, Composer). Then **Bug A** (fragile; re-confirm repro → plan-mode → Sonnet 5-axis → fix; green `wb-tutor-recording-mute.spec.ts` = merge bar). Then **Tranche E** (P2-WB-2 → P3).
