...
| **Next action(s)** | (1) **⏳ AWAITING ANDREW MORNING SMOKE — Wave 4 responsive** staged on **`wb-wave4-responsive` @ [`bff6a9b`](https://github.com/Arangarx/tutoring-notes/commit/bff6a9b)** (code) + `945de81` (smokebook), pushed. Smokebook [`wb-wave4-responsive-smokebook-2026-06-19.md`](wb-wave4-responsive-smokebook-2026-06-19.md) (preview alias `tutoring-notes-git-wb-wave4-responsive-arangarx-5209s-projects.vercel.app`). **Smokebook ALSO copied onto `v1-redesign` @ `3e09b4e`** for Andrew's local-checkout access (he smokes/edits it on `v1-redesign`); the file therefore exists on BOTH branches → **at `wb-wave4-responsive` merge, take `v1-redesign`'s edited version** (Andrew's notes), discard the branch's blank template copy. **Tutor ×3 sizes FIRST** (confirm no merge-introduced regression — code says shared `data-layout` media queries unchanged vs pre-unify baseline `5d56f49`, but hardware-unverified), then student ×3. On PASS → `merge --no-ff` into `v1-redesign`; on tutor regression → that's priority (merge-introduced). **Do NOT stack Wave 5 on this branch until smoked.** (2) **Wave 5** polish (post-Wave-4-smoke) — coral Exit btn, smaller Match-tutor's-view control, 2 design Qs (highlight non-clickable student tab? / allow student graph entry?), student view-lock-while-synced — see [`wb-unify-stabilization-plan-2026-06-17.md`](wb-unify-stabilization-plan-2026-06-17.md) § Waves 4–5 + W1-3 backlog IDs. (3) **`docs/phase3-consent-model` @ `4f9dbcd`** → union-merge to `v1-redesign` (conflict risk — see lesson below). (4) `phase1/wb-reliability-floor` @ `d63ac22` — awaits DESKTOP smoke. (5) Gate A→A6 + Ship-to-Sarah gate burndown per table below. |
