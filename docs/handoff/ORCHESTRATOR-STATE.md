...
| Gate-greening fix batch | ⬜ IN PROGRESS | fix build TS + real regressions (notes-worker guard, useLiveAV invariant, VAD clamp) + test fixes (erasure orderIndex, claim-setup, EndedUnsaved env); re-run full jest + next build; THEN relay + smokebook. |
