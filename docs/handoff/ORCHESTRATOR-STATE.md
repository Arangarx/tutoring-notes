...
| **In-flight subagents** | **Sonnet design pass — authed session access** → `docs/handoff/authed-session-access-design-2026-06-10.md` (verify crypto/threat model, resolve E2E-key approach + recommendation, phased plan: Phase 1 notes-login / Phase 2 session-login; design-only, no code). Awaiting Andrew sign-off on the doc + E2E trade-off before build. Wiring map done. A3a/A3b + graph-latency merged. |
