...
| **Uncommitted / unmerged** | **TWO unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (merge after Andrew DESKTOP smoke). (2) `phase1/wb-review-correct` @ [`040416b`](https://github.com/Arangarx/tutoring-notes/commit/040416b) — in-frame unified review surface **+ replay A/V cluster removed** (`4c1275d`: no recorded video → empty "No live A/V participants" panel served no purpose on replay; removed from review chrome both states; LIVE-session A/V untouched, 17/17 av-mount tests green) **+ live-A/V tutor video regression fix** (`682fc15`, see row above) **+ WebM Infinity-duration fix** (`2f28792`). **Real-browser console logs (2026-06-16) revealed the true root cause behind BOTH "always seeks 0" and "first-play snaps to end":** these recordings have NO stored duration (`storedTotal=0`), so everything depends on the MediaRecorder-WebM measured duration — but `audio.duration`/`currentTime` reads `Infinity` until a resolve-seek, and the resolved value (~94741ms) was NEVER fed back to the controller (`attachWebmDurationFix` only reset currentTime; `onMetadataLoaded`'s finite-check left `resolvedMaxMs=0` all session). Fix: duration-fix now fires `onDurationResolved` callback → updates `resolvedMaxMs`/scrubberMax (scrub maps proportionally, not localMs=0) + emits `[avx] duration_resolved`; play() guards non-finite currentTime → sets to mapped/0 (no snap-to-end); play-loop `getGlobalTimeMs` returns last-known not Infinity. 7 red/green tests, 118 green. **Earlier seek/first-play "fixes" (`8559ae9`/`f3a525e`) were necessary but insufficient — they relied on a measured duration that was silently 0.** Seek-storm fix + thumbnail coloring + scrub-seek + `duration_resolved` eager resolution **confirmed good by Andrew (real browser, 3d9572b)**. **One residual then fixed @ `cf1eadc`/`08bdba0`:** first-play still snapped to end because the WebM duration-resolve hack seeks to the END to read duration (lands at 94.741) and the snap-back-to-0 was blocked by an over-aggressive `if(audio.seeking)return` guard → element parked at end; `play()`'s Infinity-only guard missed the finite-parked case. Fix (two-pronged): webm-fix captures+restores pre-scan `currentTime` (removed the seeking-guard, since controller seeks already `cancelPendingFix`); `play()` now ALWAYS position-syncs `el.currentTime` to mapped `globalMs` before play (fresh=0, resume/scrub=no-op via 50ms tol) — does NOT reload segment src. The `2× cap` log was a downstream symptom (onEnded incremented offset by full duration), gone with fix. 8 new tests. **Resize-centering ELEVATED to pre-master (Andrew: "needs centered before master, anything in master I show Sarah") + FIXED @ `b7b8d3e`/`596d920`:** scene drifted right on resize because Excalidraw updates `appState.width` to the new size BEFORE the ResizeObserver fires, so the replay play-loop overwrote the centering snapshot with the already-resized width each frame → correction collapsed to ~0. Fix: freeze pre-resize snapshot (`resizeActiveRef`/`frozenSnapshotRef`, applySceneAt skips snapshot update during resize) + continuous re-center via new pure `computeResizeScroll` (offset-invariance tested). **Awaits Andrew re-smoke: (a) first play → `pre_play currentTime=0`, no immediate onEnded; (b) resize → scene stays centered, no rightward drift; (c) live-video LV-1..LV-4.** Ledger #14 reclassified defer-safe→keep-pre-master. **Resize-recenter SPEC clarified by Andrew + re-fixed @ `5aa4ce4`/`040416b`:** first fix (`b7b8d3e`) was behaviorally wrong (pinned to full-screen center) — root cause: it captured `scrollX` BEFORE the camera-fit ran (saw scrollX=0 → computed center as full-screen origin). Correct spec (Andrew's grid examples): **constant zoom, recompute pan so the scene point at viewport center stays at viewport center for the new dimensions** — `scrollNew = scrollOld + (Δsize)/(2·zoom)` per axis, same math as student-follow-tutor center-match, "following itself" through resize. Applied to **BOTH replay AND live** board (live additive-only — no page-switch/student-follow/persist/recording changes). 8 grid-vector tests. **Awaits Andrew re-smoke: (a) first play → currentTime=0; (b) resize (narrow/wide/asymmetric, live+replay, paused+playing) → centered mark stays centered, zoom unchanged; (c) live-video LV-1..LV-4; (d) replay surface has no A/V cluster.** **AUDIO SEEK CONFIRMED WORKING by Andrew (2026-06-16)** (two-cause fix: play/pause race `38dcc63` + duration-mapping collapse `8559ae9`); header-time matches scrubber. Follow-on fixes @ `f3a525e`: first-play-snaps-to-end (end-cap fired at 1ms when stored duration=0 → guarded until measured duration loads) + scrubber-drag seek-storm (onChange wrote currentTime per-move → now scene-preview/UI only, single audio seek on pointer-up). **Thumbnail dark-mode fix @ `fe2c701`:** prior fix passed `exportWithDarkMode:true` AND a dark `viewBackgroundColor` → Excalidraw's invert filter flipped the dark bg to near-white, and canonical near-black ink inverts to white → faint-white-on-white (Andrew's screenshot). Fix: export bg ALWAYS white, let invert filter produce dark-on-light/light-on-dark correctly; WB theme source = global `useTheme().resolvedTheme` (no separate WB context). **Awaits Andrew re-smoke of first-play + drag-storm + thumbnail-dark** (seek/header/volume confirmed). If clean → P1 smoke-complete → `test:wb-sync` + merge `--no-ff` to `v1-redesign`. 5th smoke (real browser) gave CONSOLE ERRORS = breakthrough: `RangeError: Maximum call stack size exceeded` in `apply→el.pause()→(sync pause event)→onPause→loop.pause()→apply` recursion + infinite `review_surface_replay_enter/hero_return` loop + `AbortError play() interrupted` — the controller was crashing before it could ever process a scrubber-drop seek (that's why audio never seeked). Fixed with `isApplyingRef` re-entrancy guard + `replaySettledRef` (suppress returnToHero until entry paint settles) + seek log `[avx] action=replay_scrub_seek`. Thumbnail now honors WB theme (dark export); resize now center-preserves (snapshot scroll math) not bbox-refit-flash; volume slider WORKS (Andrew confirmed). ⚠️ Automated real-Chromium console capture STILL not delivered (auth-gated preview blocks Playwright login — same wall as audio value) → Andrew's 6th smoke w/ console open is the gate: no RangeError, no enter/return loop, seek log fires on drop. `eventCount=0` log = benign (server raw-blob fetch unauth'd; replay/thumbnail use auth proxy). 4th smoke (real browser): canvas-fill WORKS; audio-seek STILL t=0 (4th fail), thumbnail live-Excalidraw flash-then-black + native menu, no resize-recenter. **PIVOT — ported legacy `WhiteboardReplay.tsx` audio contract** into `useReplayTimelineController` (Play = `void el.play()` not re-`loadSegmentAt`; paused-scrub `canplay` retry for BOTH autoplay states; removed destructive init re-run reset + `hasEverPlayed` gate; JSX `<audio src>`). **Thumbnail → static `exportToCanvas` `<img>`** (no live editor). **Resize → `createCameraFitter.fit()`** on debounced resize. **Volume control added.** ⚠️ **Audio real-browser verify NOT automatable** (needs authed audio session) → Andrew audible confirm is the gate; high-confidence since it's a known-good port, not a patch. Orchestrator owes `test:wb-sync` pre-merge. _(Earlier `phase1/wb-replay-in-frame` @ `52a9dae` superseded.)_ **Lesson: audio-element seek is real-browser-only — 4 jsdom-passing patches failed in browser; porting proven legacy logic beat patching the rewrite.** **DONE/MERGED:** `auth/2fa-remember-device` (unified auth branch) @ [`8ffd344`](https://github.com/Arangarx/tutoring-notes/commit/8ffd344) — thread closed; `fix/password-manager-username-field` absorbed into it (do NOT merge separately); `fix/preview-badge-copy-commit` MERGED @ [`feccae9`](https://github.com/Arangarx/tutoring-notes/commit/feccae9) (pill copies branch + commit id). **Known pre-existing test debt (carried forward):** `identity-2fa-management` "settings page links" jest assertion is stale (checks JSX `href=` on an object-literal `href:`) — unrelated, fix opportunistically. **Reset-screen generate-offer → BL-RESET-GENERATE** (backlog), Andrew wants eventually. **⚠️ Dispatch-safety near-miss (2026-06-14):** badge executor `git stash`+reset on orchestrator's uncommitted state edits instead of STOPPING — preserved+recovered; reinforce STOP-don't-stash; commit state promptly. |
