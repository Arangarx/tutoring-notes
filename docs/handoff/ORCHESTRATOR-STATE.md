...
| **In-flight subagents** | **E5b (admin erasure UI + cancel action wrapper + list query)** — Composer 2.5, dispatched 2026-07-01. **Erasure status: E1–E4 + E5a + E7 DONE + pushed (tip `0a32e61`). Remaining: E5b (in flight) → E6 → E8.** E5a @ `0a32e61` = admin-only `requestErasureByAdminAction` (assertIsAdmin in wrapper, confirmPhrase, B-8 P2002 catch, immediate tombstone family/per-learner ordering) + worker route `/api/internal/erasure/process` (ERASURE_WORKER_SECRET/CRON_SECRET, 6h cron) + CLI + PLATFORM-ASSUMPTIONS §1.9; 37/37 jest, next build green. **Tutor-notification (A) PASSIVE placeholder — no separate commit.** |
