...
## ⏩ HEAD — 2026-06-22 Wave 5 polish @ `c4fff44`; 2 hardware-smoke regressions fixed (desktop styles flyout + viewport-follow lag), wb-sync re-PASS, awaiting Andrew re-smoke
...
| **In-flight subagents** | **None.** 2026-06-22: follow-lag fix (`c4fff44`) + **styles-flyout REAL fix `6652522`** (portal flyouts out of rail; `06ce763`'s `overflow-clip-margin` was invalid CSS → still clipped → REAL-BROWSER Playwright proof now mandatory; screenshot `docs/handoff/assets/wb-props-flyout-open-2026-06-22T22-22-08.png`) + **invariant 13 continuous-gesture follow test** added (`4c643de`) then recalibrated to a robust stale-lock guard (`32ef3f5`). **Follow fix PROVEN at relay** via diagnostic run: mid-gesture student tracks tutor EXACTLY (`distToOracle=0.00`, far from stale first-frame lock) — Andrew's reported bug (mid-gesture freeze) is fixed. Invariant 13 FINAL exact-frame check was a HARNESS limitation (E2E `setViewport` bridge doesn't fire the real mouse-release `scheduleViewportPersist`→`broadcastPageViewState` trailing edge → student settles ~1 throttle-frame short; NOT a product defect); recalibrated: MID = tight live-tracking guard (passes on fix, fails on bug, survives load), FINAL = "not frozen at stale lock" (>100 from firstStep/initial). **PENDING: one CLEAN relay run @ `32ef3f5`** to confirm GREEN — deferred until machine cools (ran wb-sync ~7× today → overload: invariant 1 timed out at 45s, run took 5.8m vs ~2m; load-flake, not regression). **wb-sync was GREEN @ `6652522`** before invariant 13 added. |
...
| **Uncommitted / unmerged** | **`wb-wave5-polish` @ `32ef3f5`** (pushed; orchestrator parked here) — Wave 5 polish + 2026-06-22 regression fixes (`c4fff44` viewport-follow lag; `6652522` desktop styles flyout via portal, browser-proven) + invariant 13 follow-regression test (`32ef3f5`); **gate (a) wb-sync GREEN @ `6652522`; one clean re-run @ `32ef3f5` PENDING (machine cooldown)**; awaits Andrew re-smoke (continuous pan/zoom follow + desktop styles popout + items 9–13 + 7) → verify ~11 pre-existing jest failures on `v1-redesign` base + `merge --no-ff` into `v1-redesign`. **QUEUED (separate branch `chore/test-db-safety`, after wave5 confirm): DB-safety hardening** — host-guard `jest.global-setup.ts` `--force-reset` + `reuseExistingServer:false` for wb-regression (audit: `.env` `TEST_DATABASE_URL` UNSET so currently safe, but latent foot-guns; `DATABASE_URL`=Neon). Playwright already headless (no VM needed). Smokebook [`wb-wave5-polish-smokebook-2026-06-21.md`](wb-wave5-polish-smokebook-2026-06-21.md). **None uncommitted** on `v1-redesign` @ `a166f6c`. `wb-wave4-responsive` @ `64108cf` **merged** (branch preserved for cleanup). **`docs/phase3-consent-model` @ `4f9dbcd`** (pushed) — awaits `merge --no-ff` into `v1-redesign`; **conflict risk** on P3 handoff docs (union-merge required). **`phase1/wb-reliability-floor` @ `d63ac22`** — awaits DESKTOP smoke. `phase2/wb-student-new-shell` **superseded + absorbed** by wb-unify merge. |
