...
| 🔑 KEYSTONE — Blob-gate hermetic harness | 🟡 phase 1 built+committed [`a1cc2bd`](https://github.com/Arangarx/tutoring-notes/commit/a1cc2bd), **PENDING review gate** | Andrew chose **test-layer-first hermetic**. Phase 1 = in-memory blob store + `PLAYWRIGHT_TEST`-gated test route + `blobIntegrationEnabled()` guard; all prod touches behind `isBlobHarnessActive()` (needs `PLAYWRIGHT_TEST=1` **AND** `BLOB_HARNESS_LOCAL=1`). Proven on 3 representative specs (replay-scrub audio, VAD DB count, inv-7 image) — now RUN+PASS (were skip); jest guard 19 green. **IN FLIGHT:** [Sonnet 5-axis review](8b43c131-d9c7-4e97-aa23-ad546813f996) (auth/host-allowlist/prod-off/false-green) + [next build+jest verify](bf3c69ee-6fe1-4453-b7e5-31ab66ee7eb7). Then phase 2 = mechanical guard swap for remaining ~11 specs + `--list` verify RUN-not-skip. Executor self-flagged: more prod surface than minimal (globalThis store, `NEXT_PUBLIC_*` client sentinels, host-allowlist) → the review scrutinizes exactly these. |
