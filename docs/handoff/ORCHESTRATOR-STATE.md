...
| **In-flight subagents** | **Checkpoint verification gate** (Composer 2.5): full `npx jest` (local test-DB override) + `npx next build` + `npm run test:wb-sync` (CF-2/2.1 touched `WhiteboardWorkspaceClient` — WB relay gate applies); confirm the 2 pre-existing `src/__tests__/dom` failures (`StudentLiveWorkspaceClient` AggregateError, `SharePage.whiteboard` `Request is not defined`) are genuinely pre-existing; verify the `playwright-harness.ts` erasure-admin 2FA bypass is fully `WB_E2E_HARNESS`+`!VERCEL`-gated. **After green → regenerate SLIM smokebook (hardware/jsdom-blind residual only) then Part 3.** **Continuation bootstrapper:** [`session-experience-arc-continuation-bootstrapper.md`](session-experience-arc-continuation-bootstrapper.md). |
