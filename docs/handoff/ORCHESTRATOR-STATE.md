...
## ⏩ HEAD — 2026-06-28 **OVERNIGHT autonomous run** on `wb-wave5-waiting-polish` (sub-branch of `wb-wave5-polish`) — takeover FIXED; UI parity/polish batch + adult-learner join in flight for Andrew morning smoke
...
>
> **Sub-branch [`wb-wave5-waiting-polish`] created off `wb-wave5-polish`@`2f7bdd9`** for tonight's autonomous work (leave on sub-branch for Andrew morning smoke; merge back to `wb-wave5-polish` only if clean & no smoke needed). Backlog @ [`c893038`](https://github.com/Arangarx/tutoring-notes/commit/c893038): deferred `WB-DEVTOOLS-FIXTURE-LOGIN-AS` (post-Sarah) + `WB-AV-PER-STREAM-VOLUME`. **IN FLIGHT:** (A) waiting-room parity/polish batch (subagent [efd0e45d](efd0e45d-9c76-4f3b-b63d-3742ea19c4e9), background) — 5 findings: off-cam remote tile shows black not initials (both dirs); student mic missing volume indicator; device pickers unreachable at narrow width; top-bar more-menu spacing too loose; in-overlay theme control. (B) adult self-learner `/join` path (`WB-JOIN-ADULT-LEARNER`) — QUEUED to dispatch after (A) lands (both touch `wb-session-lifecycle.spec.ts` + Playwright is single-instance ⇒ serialize).
...
| **In-flight subagents** | **(A) DONE** — waiting-room parity/polish batch ([efd0e45d](efd0e45d-9c76-4f3b-b63d-3742ea19c4e9)) pushed @ [`25426ef`](https://github.com/Arangarx/tutoring-notes/commit/25426ef): all 5 smoke findings (off-cam initials via `AVTile` active-track check + `useLiveAV` mute listeners; student mic volume meter parity via `WbTopBarMicControlLive`; narrow-width pickers `.mynk-wtr-device-pickers`; tightened overflow-menu spacing; in-overlay `WbThemeToggle`). Gates: jest 719/719, tsc clean, 5/5 new Playwright. **Caveat:** Finding-1 REMOTE cam-off proven by jest only (hermetic relay doesn't propagate remote `track.enabled`/`mute`) → hardware smoke is final proof. **(B) RUNNING** — adult self-learner `/join` (`WB-JOIN-ADULT-LEARNER`) on Sonnet ([5cbe5c33](5cbe5c33-4013-488f-9a7f-fccdad469aa0), background): route redirect by `isSelfLearner`, join-gate accepts account-holder session via `assertOwnsLearnerProfile`, 3 downstream API routes; auth-boundary (child sessions must stay un-joinable by account-holders). Takeover fix ([832bbc19](832bbc19-f493-457c-8c35-b845dab62244)) DONE @ `2f7bdd9`. |
...
| **Uncommitted / unmerged** | **`wb-wave5-polish`** pushed @ [`2f7bdd9`](https://github.com/Arangarx/tutoring-notes/commit/2f7bdd9) (round-3+4 smoke fixes + takeover fix). **Sub-branch `wb-wave5-waiting-polish`** @ `c893038`+ (overnight UI batch + adult-join landing here) — leave for Andrew morning smoke; merge to `wb-wave5-polish` only if clean. **NOT merged** to `v1-redesign`/`master` — per Plan #1, no merge until final Sarah end-to-end gate. Integration base remains `v1-redesign`. |
