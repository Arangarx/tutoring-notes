...
| **Open Andrew-confirms** | **(d)** learner-swap · **(f)** C1–C4 before `CONSENT_ENFORCEMENT` flip · **(g)** env scoping at master cut. **Resolved 2026-06-14:** **(c)** student URL → **KEEP (hybrid)** — do NOT retire anonymous `/w/[joinToken]` paste-and-go join link; stays as fallback (esp. unclaimed students); claimed/logged-in students → authed `/join/[sessionId]` remains design intent; does not block student-WB shell migration (cutover-policy only). **(e)** camera default → **SELF-VIEW ON** — student + tutor see own camera by default so they know their face is showing. **Standing:** B2 D-1/D-2/D-5, B1 deferred TODOs, N-2. **Ratified 2026-06-11:** platform→tutor metering = wall-clock (cash + tokens). **Lawyer-needed:** VPC method/copy, retention in /privacy, FERPA/SOPIPA. **Logged for wiring phase (not blocking cut):** waiting room (Gate A2), parent consent-edit (B2 Step 6), scheduler + Google OAuth — visual-only surfaces; design Qs in [`v1-design-system-morning-status-2026-06-12.md`](v1-design-system-morning-status-2026-06-12.md). |
