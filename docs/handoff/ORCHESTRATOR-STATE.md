...
**QUEUED FIXES (dispatch serially off `v1-redesign` as the tree frees):** (1) **replay multi-segment stitch** (P0 reliability — present all segments as ONE continuous recording, no gaps, map WB `t` across boundaries; embodies the abstraction principle) — DISPATCHING NOW; (2) family-id scheme A; (3) WB page-insertion→end; (4) correct analysis-doc §2.
