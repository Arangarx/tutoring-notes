...
## ⏩ HEAD — 2026-06-08 (V1 two-tier master-cut gates ratified; A′ merged; P1.1 next)

| Field | Value |
|---|---|
| **Last action completed** | **V1 master-cut gates ratified (Andrew 2026-06-08)** — two-tier checklist pinned below (canonical operational list). Prior: **A′ theme foundation MERGED → `v1-redesign`** — `feat/theme-plumbing-a-prime` merged `--no-ff` ([`982e53f`](https://github.com/Arangarx/tutoring-notes/commit/982e53f)) after Andrew's **GREEN** both-theme smoke; push gated on real `npx next build` (exit 0, 39/39) + `theme-plumbing.test.ts` (7/7). Smoke record: [`SMOKE-RUNBOOK-A-prime-theme-2026-06-08.md`](SMOKE-RUNBOOK-A-prime-theme-2026-06-08.md). PAPER design pass COMPLETE ([`6713f32`](https://github.com/Arangarx/tutoring-notes/commit/6713f32)) — P1.2 whiteboard-chrome visual design; freedraw latency root-caused → P1.1 Sonnet-tier w/ PR-01 acceptance gate. |
| **Next action(s)** | **P1.1 dispatch (Sonnet-tier)** — tutor-desktop hybrid chrome + freedraw-latency hot-path fix (PR-01, Phase-1 acceptance gate); build from [`whiteboard-chrome-p1.2-visual-design-2026-06-08.md`](whiteboard-chrome-p1.2-visual-design-2026-06-08.md). **Legacy-surface composition build** (B2–B6 toward mock per §5 migration checklist) — **first surface folds in the `dark:`→token primitive cleanup** (button/input) so System mode is visually whole. **Do NOT cut `master` until ALL Gate A items below are done** — see pre-master gates section. |
| **Open Andrew-confirms** | (1) **IAC-13 corrected-copy smoke** — Disconnect dialog copy on claimed student. (2) Component-pass **cohesive visual review** (Gate A). (3) **Sarah XPPen G640** stylus confirm (generic tablet passed; residual). (4) When **all Gate A** items pass: green-light `v1-redesign → master` (= Sarah reveal). |
...
| # | Gate | Status |
|---|---|---|
| A1 | **Visual redesign + whiteboard chrome + theme parity** — component-pass cohesive visual review; Mynk custom chrome (PR-01 incl. freedraw latency); both-theme per-component gate | **In flight** (P1.1 next) |
