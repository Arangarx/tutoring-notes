...
## ⏩ HEAD — 2026-07-01 Erasure FEATURE COMPLETE @ 51e5bfd; CF-2/CF-4 root-caused (both SAFE-additive) → fix dispatch in flight

> **Active branch:** [`wb-wave5-polish`](https://github.com/Arangarx/tutoring-notes/tree/wb-wave5-polish) @ [`51e5bfd`](https://github.com/Arangarx/tutoring-notes/commit/51e5bfd) (+ state-doc commit on top). **Worktree:** `tutoring-notes-polishwt` (default `tutoring-notes` checkout is on `v1-redesign` — NOT current). **All remaining Sarah-gate work lands here; single `merge --no-ff` to `v1-redesign` at the FINAL full-arc gate only (no interim merge — Andrew reaffirmed 2026-07-01).** Integration base remains **`v1-redesign` @ [`7397abc`](https://github.com/Arangarx/tutoring-notes/commit/7397abc)**.
>
...
| Field | Value |
|---|---|
| **Last action completed** | **Erasure Step 5/6 shipped @ [`51e5bfd`](https://github.com/Arangarx/tutoring-notes/commit/51e5bfd)** — **ER-4** tutor-facing pending-erasure UI (roster "Pending erasure"/"Deleted" badge + detail warning banner w/ grace countdown + suspended-access states on Start CTA), derived by `deriveStudentErasureDisplayState()` off `tombstonedAt` (+ AH for full-family), purge dates batched via `lookupActiveErasurePurgeDates` — **composed from existing primitives** (`Badge`/`Alert`/`LocalDateTimeText`/`AlertDialog`, NO new primitives); **ER-6** honest operator copy in `ErasureAdminClient` (7-day grace, cancel fully restores; removed false "[Deleted] immediately" claim) — **none in privacy/terms**; **ER-7** `ers=`/`sal=` logging registry in `RECORDER-LIFECYCLE.md`. **Cancel wiring TESTED** (`cancel-erasure-action-wiring.test.ts` proves the control invokes `cancelErasureJob` — no visual-stub). 80 erasure-lib + 31 targeted jest green vs local DB; `next build` 0; no whiteboard paths. **⇒ Erasure FEATURE COMPLETE** (backend A–I + UI + copy + docs). — — — **Prior:** Step 4.5 @ [`dd37288`](https://github.com/Arangarx/tutoring-notes/commit/dd37288) (share-link HIGH closed); Step 4/ER-3 @ [`6e46e16`](https://github.com/Arangarx/tutoring-notes/commit/6e46e16); Step 3.5 @ [`6631b5e`](https://github.com/Arangarx/tutoring-notes/commit/6631b5e); Step 3 @ [`11a812c`](https://github.com/Arangarx/tutoring-notes/commit/11a812c); Steps 1+2 (@ [`d3458f9`](https://github.com/Arangarx/tutoring-notes/commit/d3458f9), @ [`ac963a3`](https://github.com/Arangarx/tutoring-notes/commit/ac963a3)). |
| **Next action(s)** | **CF-2 + CF-4 fix (in flight — 2 serialized Composer commits, disjoint regions of `WhiteboardWorkspaceClient.tsx`).** **CF-2/MB-4** root cause CONFIRMED: WB event persistence gated on `audioCapturePolicy !== "none"` via shared `recordingActive` (`audio-capture-policy.ts:28`, `WhiteboardWorkspaceClient.tsx:2333`, `useWhiteboardRecorder.ts:587`); IN_PERSON+denied→`none`→no events→empty replay (live sync independent). Fix = ADDITIVE `wbEventsActive` flag (role=tutor && phaseActive && userWantsRecording, NO audio check) → pass as `recordingActive` + `getAudioMs=useAudioMsClock(wbEventsActive)` at call sites (~1400/~2543/~2859); LEAVE FSM `tutorWantsRecording` audio gating + engine/apply/outbox UNCHANGED. **CF-4/MB-5:** no code gate blocks tutor_only notes — server ALLOWS it; failure is silent `.catch`-swallowed `triggerNotesGenerationAction` (ownership/impersonation throw). Fix = ADDITIVE: return `{ok,error?}` + surface failure in `handleEndSession` (~3816) + `[tnt]`/`[nsi]` log + tutor_only→TutorNote reduce integration test. Both **SAFE-additive, no escalation**. → **Sonnet 5-axis review of CF-2 diff** (clock/ordering/replay-correctness + no engine regression) → **CF-2 `test:wb-sync` at merge boundary** (WhiteboardWorkspaceClient is wb-surface) + re-confirm MB-1/CF-1 → **Workstream C** Playwright e2e → **checkpoint re-smoke (NO merge)** → **Part 3** spine → **full-arc both-themes hardware smoke** → single `merge --no-ff → v1-redesign`. **All DB-test runs override `DATABASE_URL`/`DIRECT_URL` to local `tutoring_notes_test` (never Neon).** |
