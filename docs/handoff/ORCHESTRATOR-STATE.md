...
| E2 dup-stroke PDF | ✅ code | [`34f650a`](https://github.com/Arangarx/tutoring-notes/commit/34f650a) — bump `tutorSwitchTokenRef`+`pageSwitchProgrammaticRef` at `commitPdfBatch` entry (reuse existing guard checks), release in `.finally()`. jest 794. Teeth spec `wb-e2-pdf-stroke-leak.spec.ts` authored, relay-blocked (webServer). |
| GATE PREREQ FIX — Playwright webServer `prisma db push` | ⬜ IN PROGRESS | fixing now so relay teeth specs can run (unblocks E2 proof + integrated gate). Relay was green @ d299a6c; Wave-0 db-execute migration likely caused drift. |
| E3 reconnect pill (fold a962171 + BUG-8) | ⬜ | fragile-serial; [human-only] Safari/iOS |
