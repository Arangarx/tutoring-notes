...
| **Uncommitted / unmerged** | **FOUR unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (merge after Andrew DESKTOP smoke). (2) `phase1/wb-replay-in-frame` @ `52a9dae` — in-frame two-state replay (awaits smoke; orchestrator owes `test:wb-sync` pre-merge). (3) `auth/2fa-remember-device` @ `5734c65` (code @ `dbe8178`) — **UNIFIED AUTH BRANCH**: 2FA remember-device + cascade/dedup + NOW ALSO the username `autocomplete` anchors, the must-differ bcrypt fix, `/reset-password` invalid-link hardening (token-doesn't-resolve → Invalid-link state so the anchor is always present when the form renders), and the **forgot-password trusted-device cascade** (`completePasswordReset` revoke-all). Consolidated from `fix/password-manager-username-field`. Carries badge via back-merge `55ba810`. `ADMIN_TFA_DEVICE_HMAC_SECRET` live in Vercel. Not whiteboard → no `test:wb-sync` gate. **Awaits Andrew's ONE comprehensive auth smoke** (smokebook items 1–13 + appended A–E) then `--no-ff`. (4) `fix/preview-badge-copy-commit` @ `75451ad` — pill click copies commit id (preview-only badge), awaits preview confirm → `--no-ff`. (5) `fix/password-manager-username-field` @ `394f6b6` — **ABSORBED into `auth/2fa-remember-device` @ `dbe8178`; redundant, do NOT merge separately** (kept as historical ref; its username-anchor + must-differ-bcrypt fixes now live on the unified auth branch). **Badge base MERGED — done.** **⚠️ Dispatch-safety near-miss (2026-06-14):** badge executor `git stash`+reset on orchestrator's uncommitted state edits instead of STOPPING — preserved+recovered; reinforce STOP-don't-stash; commit state promptly. |
