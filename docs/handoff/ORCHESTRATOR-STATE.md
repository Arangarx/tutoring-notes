...
| **In-flight subagents** | **E5a (admin-only requestErasure action + worker route/CLI + PLATFORM-ASSUMPTIONS)** — Composer 2.5, dispatched 2026-07-01. **Erasure status: E1–E4 + E7 DONE + pushed (tip `412c5bb`). Remaining: E5a (in flight) → E5b (admin UI) → E6 → E8.** **Tutor-notification decision (Andrew 2026-07-01): (A) PASSIVE — the "[Deleted learner]" placeholder from E4 scrub IS the notification; NO separate notify commit.** |
