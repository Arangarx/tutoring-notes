...
## ⏩ HEAD — 2026-06-07/08 (master cut gated on FULL redesign incl. whiteboard chrome)

| Field | Value |
|---|---|
| **Last action completed** | **Both small pre-master gates CLOSED** + **sequencing decision ratified (Andrew 2026-06-07/08).** (1) **IAC-13 disconnect-copy FIXED** @ [`05af41f`](https://github.com/Arangarx/tutoring-notes/commit/05af41f). (2) **Legacy JWT role-fallback DEFERRED** @ [`4eb355f`](https://github.com/Arangarx/tutoring-notes/commit/4eb355f) — cosmetic one-time middleware bounce, zero security impact; hardening follow-up in `BACKLOG.md`, NOT a pre-master blocker. **Exposure model verified:** `v1-redesign → master` = **Sarah reveal** — `tutoring-notes.vercel.app` and `usemynk.com` share the same Vercel production deployment on `master` (currently @ `a621a5b`); no UI-skin feature flag decouples code-on-master from what Sarah sees. **Ratified:** master cut gated on **entire site redesign cohesive end-to-end, including whiteboard chrome** — not after component visual review alone. |
| **Next action(s)** | **Pre-master gates (both required before `v1-redesign → master`):** (1) component-pass **cohesive visual review** of a full page/flow vs mocks; (2) **whiteboard chrome design pass → build** (canonical input: [`docs/handoff/whiteboard-chrome-requirements.md`](whiteboard-chrome-requirements.md)). **Immediate thread:** whiteboard chrome design pass. Keep `v1-redesign` as long-lived integration branch; smoke on preview URL; cut `master` only when whole site (incl. whiteboard chrome) is one cohesive reveal. Heavy ORCHESTRATOR-STATE restructure at master-cut milestone. `feature/sarah-forward-migration-q6` parked. |
| **Open Andrew-confirms** | (1) **IAC-13 corrected-copy smoke** — open claimed student's `/admin/students/{id}` → Parent account → Disconnect → confirm dialog shows parent email/name in first bold slot, learner name in second. (2) Component-pass **cohesive visual review** for a full page/flow (pre-master). (3) Whiteboard chrome design + build on `v1-redesign` (pre-master; path to master cut). (4) When **both** pre-master gates pass: green-light the `v1-redesign → master` cut (= Sarah reveal). **No new decision needed** — sequencing ratified. |
...
| **Uncommitted / unmerged** | **`v1-redesign` @ [`05af41f`](https://github.com/Arangarx/tutoring-notes/commit/05af41f)** (IAC-13 disconnect-copy fix on top of slice-3 + auth-role-refresh + component spine + whiteboard reqs/TU-11 + docs-policy + archive sweep). **Main tree ON `v1-redesign`, working tree CLEAN.** **Unmerged branches → v1-redesign:** `feature/sarah-forward-migration-q6` @ `a396ab5` (parked). **MERGED (retained for branch-sweep cleanup):** `feat/recording-p1-slice3-autonotes`, `harden/auth-role-refresh` + assorted stale worktrees (`tn-auth-harden`, `tn-docs-notes`, `.cursor/worktrees/*`). |
