...
## ⏩ HEAD — 2026-06-30 4 consent forks RATIFIED; CC-1/CC-2 impl plan + erasure verification in flight on `wb-wave5-polish`
...
| **Next action(s)** | **(1) CC-1/CC-2 impl plan (in flight) + erasure verification (in flight) land → orchestrator synthesizes for Andrew → optional Sonnet 5-axis review → execute consent-honesty blocker (Block B + CC-1 + CC-2) + erasure path (pre-Sarah). **(2) Part 1+2 checkpoint (NO merge):** `test:wb-sync` + surrogates + `npx next build` + hardware A/V smoke. **(3) Part 3 spine (unblocked):** `p3-clock` FIRST (incl. disconnect pause/freeze acceptance) → `p3-perspeaker-capture` → `p3-vad-chunking` + `p3-consent-recording` → `p3-incremental-map` → `p3-model-abstraction` → `p3-finalize` → `p3-replay-scrub` → `p3-video-seam` (design-only). **(4) FINAL Sarah gate:** full-experience hardware smoke both themes → single `merge --no-ff` to `v1-redesign`. |
| **Open Andrew-confirms** | **Review CC-1/CC-2 impl plan + erasure-path verification findings when they land**; **decide whether a Sonnet 5-axis adversarial pass runs before execute.** **Go to execute consent-honesty blocker** (Block B + CC-1 + CC-2 per plan + follow-up hook-point pass for session-gate + claim flow). **CC-2 warning-copy approval** (DRAFT strings). **Debounced-disconnect pause trigger** — recommended default: stable disconnect ~6s (`PEER_EVICTION_TIMEOUT_MS`), not transient mobile blips; **final confirm at `p3-clock` execution**. **Standing:** **WB-LABEL-PARENT-SIGNIN** new term; **Sarah primary device** (assumed Windows desktop Chromium — verify on next call, [`SARAH-CALL-PREP.md`](../SARAH-CALL-PREP.md)); **Ship-to-Sarah gate** (notes path, end/continue save discipline, single-segment seek); **iOS student WB/A/V** zero real-device coverage ([`BACKLOG.md`](../BACKLOG.md) **WB-STUDENT-MOBILE-VALIDATION**). |
| **In-flight subagents** | **CC-1/CC-2 impl plan** (Composer; authoring [`cc1-cc2-consent-gate-plan.md`](cc1-cc2-consent-gate-plan.md)). **Erasure-path verification** (read-only explore). ⏸️ **Full `test:wb-sync` relay DEFERRED to final Sarah merge boundary** (per merge-cadence refinement). ⬜ **Hardware A/V smoke (both themes) still owed** before/at Sarah gate. p1c CSS co-location **COMPLETE @ `045a7b4`**. |
...
| **Data erasure path (pre-Sarah)** | ✅ **RATIFIED (Andrew 2026-06-30, fork session)** — learner/family-level erasure ships **pre-Sarah**; Andrew directed verifying the deletion path **now**. **CRITICAL CONSTRAINT:** erasure must delete the learner + ALL PII but **PRESERVE financial tracking / auditability** (schema designed for this separation) — do NOT over-delete. |
