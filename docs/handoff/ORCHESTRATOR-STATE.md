...
## ⏩ HEAD — 2026-06-23 Wave 5 smoke blockers **fixed on `wb-wave5-polish`** (uncommitted → commit this turn); **re-smoke required**

> **SMOKE VERDICT (Andrew 2026-06-23, prior):** [`wb-wave5-polish-smokebook-2026-06-21.md`](wb-wave5-polish-smokebook-2026-06-21.md) — **halted / OVERALL FAIL** @ `32ef3f5`. **Fix wave landed (this turn):** graph UI persist (`excalidrawAPI` state + `syncFromBoard`), student exit→rejoin (`leaveAllPeers` + explicit sync disconnect + **Rejoin session** button + tutor `reconnectPeer` on roster 0→1), recording banner copy when sync roster present (`syncRosterHasStudent`), follow-pill CSS, loading-guard clear on connect. **Playwright:** `wb-student-exit-rejoin.spec.ts` (fresh join + exit/rejoin), invariants **12b/12c/12d** in regression spec; `playwright.config` wb-regression includes rejoin spec. **Jest:** lifecycle-machine A/V banner case green. **Next:** Andrew targeted re-smoke (graph, rejoin, banner, follow pill, reload) — merge still blocked until PASS.
...
| **Next action(s)** | **Andrew targeted re-smoke** on `wb-wave5-polish` after fix commit: graph UI plot + tutor→student sync, exit→rejoin (Rejoin button), recording banner copy (Teams/no-WebRTC), follow pill #9, fresh-session reload guard. Playwright gate green for new specs (12b/12c/12d + rejoin). Merge blocked until smoke PASS. |
...
| **In-flight subagents** | **None.** Fix wave complete; awaiting Andrew re-smoke. |
| **Wave 5 status** | Blocker fix wave on `wb-wave5-polish` (this turn). Prior smoke OVERALL FAIL @ `32ef3f5`. New Playwright specs green locally. Merge **blocked** until re-smoke PASS. |
| **Uncommitted / unmerged** | **`wb-wave5-polish`** — fix wave staged for commit this turn. |
...
**Process directive (Andrew 2026-06-07, strengthened 2026-06-23):** prefer **agent-runnable validation harnesses** over manual smoke wherever behavior is verifiable without Andrew's hardware. **Standing rule:** every behavioral/visual bug fix ships a Playwright regression test in the same branch ([`.cursor/rules/playwright-on-fix.mdc`](../../.cursor/rules/playwright-on-fix.mdc)) — Andrew should not re-smoke the same regression twice.
