...
| Field | Value |
|---|---|
| **Last action completed** | **Excalidraw audit dispositions ratified (2026-06-08)** — new standing principles (every-function-a-button **TU-14**, full touch parity **TM-10**) + new reqs **TU-13** (whiteboard-local theme toggle); P1.2 toolbar design **unblocked**. Separate **draw-latency investigation** in flight. Prior: **Phase 0 whiteboard chrome feasibility POC = GREEN** (Andrew real-device smoke 2026-06-08); verdict [`docs/handoff/POC-SMOKE-wb-chrome-2026-06-07.md`](POC-SMOKE-wb-chrome-2026-06-07.md). IAC-13 disconnect-copy FIXED @ [`05af41f`](https://github.com/Arangarx/tutoring-notes/commit/05af41f). **Exposure model:** `v1-redesign → master` = **Sarah reveal** — no UI-skin feature flag. **Operating model:** **`master` stays frozen**; whole redesign cooks on **`v1-redesign`**; isolated **cherry-pick to `master`** is the lever for any urgent Sarah fix. |
| **Next action(s)** | **Phase 1 — tutor-desktop hybrid chrome** — P1.2 toolbar/properties build now unblocked (audit ratified 2026-06-08); leading with foundational **theme plumbing slice (A′)**; remaining slices: chrome-over-workspace (`test:wb-sync`-gated), toolbar+tool-wiring (laser V1 top-level, all-styles-tiered More, z-order/delete buttons), properties popover + drawing defaults. **Draw-latency investigation** in flight (separate thread). Spike worktree `spike/wb-chrome-poc-857e6d4c` **RETAINED** — Phase 1 lifts verified CSS hide-selector list. **Pre-master gates still open:** (1) component-pass cohesive visual review; (2) whiteboard chrome **build**. Cut `master` only when whole site is one cohesive reveal. |
