...
| **Next action(s)** | (1) **⏳ AWAITING ANDREW WAVE 4 ROUND-2 RE-SMOKE.** Round-1 smoked 2026-06-20: **tutor desktop PASS** (no merge regression on desktop — the key check); rest PARTIAL (mobile styles "more styles" missing, mobile shapes selector/affordance missing, phone-landscape over-compaction + "More" off left rail, student desktop double-Connected-pill + half-width off-screen overflow, follow-toggle not inline). **Round-2 fixes landed @ [`17dc8d4`](https://github.com/Arangarx/tutoring-notes/commit/17dc8d4)** on `wb-wave4-responsive` (additive chrome only; tsc/eslint clean) — all 6 addressed. Round-2 re-smoke section **R2-1..R2-6** added to smokebook [`wb-wave4-responsive-smokebook-2026-06-19.md`](wb-wave4-responsive-smokebook-2026-06-19.md) @ `834b4f4` (on `v1-redesign`; preview = same branch alias `tutoring-notes-git-wb-wave4-responsive-arangarx-5209s-projects.vercel.app` now serving `17dc8d4`). **Smokebook canonical copy = `v1-redesign`** (Andrew's round-1 results live here) → at `wb-wave4-responsive` merge, take `v1-redesign`'s version. On round-2 PASS → `merge --no-ff` into `v1-redesign`; on tutor regression → that's priority (merge-introduced). **Do NOT stack Wave 5 on this branch until smoked.** (2) **Wave 5** polish (post-Wave-4-smoke) — coral Exit btn, smaller Match-tutor's-view control, 2 design Qs (highlight non-clickable student tab? / allow student graph entry?), student view-lock-while-synced — see [`wb-unify-stabilization-plan-2026-06-17.md`](wb-unify-stabilization-plan-2026-06-17.md) § Waves 4–5 + W1-3 backlog IDs. (3) **`docs/phase3-consent-model` @ `4f9dbcd`** → union-merge to `v1-redesign` (conflict risk — see lesson below). (4) `phase1/wb-reliability-floor` @ `d63ac22` — awaits DESKTOP smoke. (5) Gate A→A6 + Ship-to-Sarah gate burndown per table below. |
