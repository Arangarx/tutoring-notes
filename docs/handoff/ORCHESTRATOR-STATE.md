...
> **🚨 BLOCKER #2 — FALSE "STUDENT DISCONNECTED" BANNER (Andrew 2026-06-23, demo — triaged):** Guest was **student**. **Surface:** tutor **brown/warning banner at top of canvas** (`wb-recording-autopause-banner` — copy: *"Student disconnected — recording paused. We'll resume automatically when they reconnect."* from `lifecycle-machine.ts` `derivePresentation`). **Drawing:** appeared to keep working **both ways** (Andrew ~90% confident). **A/V:** both in **Teams** simultaneously — browser mic/camera likely never established WebRTC; can't assess video-tile disconnect separately. **Recovery:** did **not** self-clear by session end. **Working hypothesis:** NOT sync drop — **split-brain / misleading copy**: recording FSM keys off **WebRTC `reachableParticipants`** while WB strokes use the **relay websocket** (`bothPartiesInRoomSync` / peerCount). With Teams holding A/V, WebRTC may never go reachable → FSM pauses + "disconnected" banner even while student is actively on the board. **Fix direction (queued):** banner must not claim "student disconnected" when sync roster says present (`peerCount ≥ 1`); distinguish *"A/V not connected — recording paused"* vs *"student left"*. Andrew smoke: note FAIL if this banner appears while drawing works.
>
> **🚨 BLOCKER #1 — GRAPH REGRESSION (Andrew 2026-06-23, demo):** Equation graphing **broken on hardware** — **(a)** tutor enters equations → **do not plot locally**; **(b)** student does not see tutor graph state; **(c)** student enters equations → **plots locally** but **tutor does not receive**. Likely suspect: Wave 5 #4 (`98f80ee`). **Test harness gap:** invariant 12 uses E2E `insertGraphFixture` (pre-seeded JSON), not UI expression entry. **FAIL #8/#12.**
