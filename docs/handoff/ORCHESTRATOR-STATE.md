...
| **Next action(s)** | **(1) Andrew re-smoke on `phase1/wb-review-correct` @ `54021b7`:** LV-2 two-device (student cam on → tutor sees second tile with live video — fix pending real-browser confirm); thumbnail-pin ([`a05abdf`](https://github.com/Arangarx/tutoring-notes/commit/a05abdf)); replay centering with **NEW** recording (legacy recordings approximate only due to window-size fallback); smokebook [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — Andrew actively entering results (**do NOT edit**). **Once those pass →** `test:wb-sync` + merge `--no-ff` to `v1-redesign`. **(2) P2 student-shell** execute-ready @ [`80ac571`](https://github.com/Arangarx/tutoring-notes/commit/80ac571) — awaiting greenlight (unchanged). **(3) Prior threads:** `phase1/wb-reliability-floor` @ `d63ac22` merge after Andrew desktop smoke; Gate A→A6 unchanged; `Experience-Driven Wedge` program + consent v2 when Andrew prioritizes. **Deferred post-master (Andrew 2026-06-16, captured in BACKLOG):** DRY viewport consolidation (**WB-DRY-VIEWPORT**), review transition animation polish (**WB-REVIEW-TRANSITION**), impersonation session-recovery bypass (**WB-IMPERSONATION-SESSION**). |
| **Open Andrew-confirms** | **P1 replay (2026-06-16):** live+replay center tracking ✓; in-page delete warning (no `alert()`) ✓; notes/form across prominent↔docked ✓; LV-1 self-view both sides ✓; idempotency #13 tentative (will reconfirm); legacy standalone replay D1-D3 skipped. **Pending re-smoke (pre-master gate):** LV-2 ([`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8)); thumbnail-pin ([`a05abdf`](https://github.com/Arangarx/tutoring-notes/commit/a05abdf)); replay centering with NEW recording. **Resolved — deferred post-master:** DRY consolidation, transition animation #2, impersonation bypass → [`BACKLOG.md`](../BACKLOG.md) § P1 replay-in-frame post-master follow-ups. **Standing:** (c) student URL · (d) learner-swap · (e) student camera default · (f) C1–C4 · (g) env scoping · B2 D-1/D-2/D-5 · B1 deferred TODOs · N-2. **P2 Q1/Q7** branch strategy + `defaultShowLocalVideo`. **Ratified 2026-06-11:** wall-clock metering. **Lawyer-needed:** VPC, retention, FERPA/SOPIPA. |
...
| **P1 replay-in-frame (ACTIVE thread 2026-06-16)** | Branch `phase1/wb-review-correct` @ [`54021b7`](https://github.com/Arangarx/tutoring-notes/commit/54021b7) — unified in-frame review surface. **Session fixes (2026-06-16):** replay center-match per frame [`ed04279`](https://github.com/Arangarx/tutoring-notes/commit/ed04279); hero thumbnail pin + CTA on-screen [`a05abdf`](https://github.com/Arangarx/tutoring-notes/commit/a05abdf); live-A/V track buffer [`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8). **Andrew-confirmed:** live+replay center; delete warning; notes/form dock toggle; LV-1; audio seek/header/volume/thumbnail-dark/resize-recenter (prior fixes). **Pending re-smoke (pre-master):** LV-2; thumbnail-pin; replay center with NEW recording. **Deferred post-master (BACKLOG):** transition animation polish; DRY viewport consolidation; impersonation session-recovery bypass. Smokebook [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — Andrew actively entering results. Prior FIX 1–7 + WebM duration + seek-storm + resize-recenter chain still in place. |
...
| **Uncommitted / unmerged** | **TWO unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (merge after Andrew DESKTOP smoke). (2) **`phase1/wb-review-correct` @ [`54021b7`](https://github.com/Arangarx/tutoring-notes/commit/54021b7)** — in-frame unified review surface + full replay/A/V fix chain (audio seek, WebM duration, seek-storm, resize-recenter, center-match per frame, thumbnail pin, live-A/V track buffer). **Andrew actively smoking** via [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — do NOT edit. **Confirmed:** live+replay center; delete warning; notes/form dock; LV-1; audio seek/header/volume/thumbnail-dark/resize-recenter. **Pending re-smoke:** LV-2; thumbnail-pin; replay center NEW recording. **If clean →** `test:wb-sync` + merge `--no-ff` to `v1-redesign`. **DONE/MERGED:** `auth/2fa-remember-device` @ [`8ffd344`](https://github.com/Arangarx/tutoring-notes/commit/8ffd344); `fix/preview-badge-copy-commit` @ [`feccae9`](https://github.com/Arangarx/tutoring-notes/commit/feccae9). **Known pre-existing:** `identity-2fa-management` stale jest assertion; `test:wb-sync` broadcast-throttle half fail (pre-existing, not redesign). **Reset-screen generate-offer → BL-RESET-GENERATE** (backlog). |
