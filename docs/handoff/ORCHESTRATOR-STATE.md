...
## ⏩ HEAD — 2026-06-17 WB unify W1-3 done; relay gate FAILED (E2E bridge missing under unified routing) — fix in flight
...
| **Next action(s)** | **BLOCKER — relay gate FAILED on W3 tip `d26b922`:** all 12 `wb-regression` Playwright tests time out in `waitForWbE2eBridge` (helpers.ts:64) waiting for `window.__TN_WB_E2E__[role].getElements` (tutor + student) — bridge never appears within 90s. Jest 666/666 green; Docker/PG/relay healthy. **Root cause almost certainly the unified routing** (W1b deleted `StudentLiveWorkspaceClient` + rerouted student through unified client — the E2E bridge install likely not reached for the student role, failing every test since each opens both roles). **FIX WAVE in flight** (investigate `__TN_WB_E2E__` install site + gating, restore for both roles under the SAME test-only gate, re-run `wb-sync` to green). THEN: build cumulative smokebook (SMOKEBOOK-TEMPLATE + PARTIAL; fetched Vercel `branchAlias`; **Ignore-this-run** = Wave 4-5: chrome overflow/mobile, laser role-COLORS, polish) → Andrew hardware smoke → `merge --no-ff → v1-redesign`. Waves 4-5 follow post-merge. |
...
| **In-flight subagents** | **E2E-bridge fix wave** (Composer) — restore `__TN_WB_E2E__` under unified routing + re-run wb-sync. |
| **Uncommitted / unmerged** | **`wb-unify-stabilize` @ [`d26b922`](https://github.com/Arangarx/tutoring-notes/commit/d26b922)** — Waves 1-3 DONE, green on jest, **wb-sync relay gate FAILED (E2E bridge missing — fix in flight)**, **NOT merged** (awaiting fix → cumulative smoke); Waves 4-5 follow post-merge. **`phase2/wb-student-new-shell` SUPERSEDED** (unify branch built on it + deleted the student copy; will NOT merge independently). **ONE other unmerged feature branch:** `phase1/wb-reliability-floor` @ `d63ac22`. |
