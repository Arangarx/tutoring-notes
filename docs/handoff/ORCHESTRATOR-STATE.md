...
| **In-flight subagents** | **E6 (tutor content-route guards + `endWhiteboardSession` erasure short-circuit)** — Composer 2.5, dispatched 2026-07-01. **Erasure status: E1–E5b + E7 DONE + pushed (tip `e38071b`). Remaining: E6 (in flight) → E8 → merge-gate STOP.** E5a @ `0a32e61` = admin request action + worker route/CLI/cron. E5b @ `e38071b` = admin-gated `/admin/erasure` page (trigger + confirm phrase + irreversible copy; jobs table w/ grace countdown + cancel-during-`requested`) + `cancelErasureByAdminAction` + `listErasureJobsForAdmin`; 40/40 jest, next build green. **Tutor-notification (A) PASSIVE placeholder — no separate commit.** |
