...
> **Active branch:** [`wb-wave5-polish`](https://github.com/Arangarx/tutoring-notes/tree/wb-wave5-polish) — code @ [`3464baf`](https://github.com/Arangarx/tutoring-notes/commit/3464baf). **Worktree:** `tutoring-notes-polishwt` (default `tutoring-notes` checkout is on `v1-redesign` — NOT current). **All remaining Sarah-gate work lands here; single `merge --no-ff` to `v1-redesign` at final gate only (no interim merge, Andrew confirmed).** Integration base remains **`v1-redesign` @ [`7397abc`](https://github.com/Arangarx/tutoring-notes/commit/7397abc)**.

| Field | Value |
|---|---|
| **Last action completed** | **`useLiveAvCoordinator` extraction IN PROGRESS (p1b, Opus, stabilize-in-place → pure-move; Andrew scope: A/V coord now, recording coord DEFERRED to Part 3 design pass).** Landed + pushed: `e24c76a` stabilize whole-`liveAv` deps via `liveAvRef` (reconnect trio + mic/cam callbacks + cam auto-request); `3464baf` create `src/hooks/useLiveAvCoordinator.ts` + pure-move reconnect trio (student/tutor sync-reconnect + tutor roster-rejoin) + per-session latch. Gates each commit: dom av-mount 17/17, `test:wb-jest` 755/755, `tsc` clean. **Remaining A/V coord slices (queued):** (4) student bootstrap + device-list refresh + camera auto-request; (5) lifecycle-participant 8s debounce + `bothPartiesInRoom` gate + `reachablePeerIdsKey` (TRICKIEST — entangled refs + ordering-before-FSM-eval); (6) mount diagnostics + mic/cam callbacks + overlay-meter memo; (7) checkpoint. **Earlier this session:** p1a/p1d residual slivers closed @ `8b77506` (student sync-reconnect dom test, `WB-AV-GAP-3`, dead regex removed). **Prior — Consent/permission hardening DONE:** tutor consent modal removed; `CONSENT_ENFORCEMENT` deleted; `sessionPhase=ACTIVE` server guards; `allowLiveSession=false` blocks learner join; `allowNoteSending` ungated + UI hidden (`WB-NOTES-EMAIL-SUBSCRIPTION-REFRAME`). **Plan input decisions resolved 2026-06-30:** per-speaker ≤3–4 cap no mixdown fallback; **t=0 = FSM recording entry (RATIFIED INPUT)** + WB↔audio hardware sync oracle; eval harness post-Sarah; session-scoped consent override → BACKLOG won't-build. **Andrew ratified Part 3 design-pass gate:** no p3-* execution until design session with Andrew approves overall Part 3 architecture/sequencing. |
...
| **In-flight subagents** | **None.** (A/V+recording effect mapping explore completed; map informed the extraction plan.) |
| **Uncommitted / unmerged** | **`wb-wave5-polish` @ `3464baf`** (this state-doc head bump pending commit) — pushed through `3464baf`; **not merged** to `v1-redesign`. **`v1-redesign` @ `7397abc`** unchanged. |
