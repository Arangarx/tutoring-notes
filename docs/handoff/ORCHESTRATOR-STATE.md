...
| **In-flight subagents** | **E8 (integration tests + smokebook)** — Composer 2.5, dispatched 2026-07-01. **Erasure status: E1–E7 (ALL code) DONE + pushed (tip `bdbaac5`). Remaining: E8 (in flight) → merge-gate STOP.** E6 @ `bdbaac5` = 5 tutor content-route 404 guards on `Student.erasedAt` via `assert-student-not-erased.ts` + additive `endWhiteboardSession` short-circuit (skips segment reg + content persist when erased/active-job; still ends session + revokes tokens); 31/31 jest, next build green, no `src/lib/whiteboard/**` touched. **OPEN design nuance for merge-gate: route guards 404 only on `erasedAt` (post-purge); tutor retains read-access during 7-day grace (content still recoverable) — confirm with Andrew.** **Tutor-notification (A) PASSIVE placeholder.** |
