...
| **🟡 P1-WB-6 tab-kill WS-N teeth — 1st attempt RED → verdict (B) TEST-MODEL error (~88%); corrected-oracle fix IN FLIGHT** | First teeth [sa](4e9f2d0f-5da1-4c2e-b51f-0e911fa3e860) went RED: exact `final.count===expectedCount` (kill-time snapshot) saw 4→6/12 rows. **Root-cause ([investigation](7e81b145-a568-4ec8-9fa0-dbbd1ffc45b7)): NOT duplication** — both register paths dedupe on `blobUrl` (`actions.ts` L1809 mid-session, L1126 End), and post-resume the recorder auto-resumes (`recordingDefaultEnabled=true` + `WhiteboardWorkspaceAudioBridge`) capturing NEW VAD segments before End → `count>expectedCount` is CORRECT; the `===`-vs-kill-snapshot oracle was wrong (`>=` was right). 6-vs-12 variability = flaky post-resume capture, not stable dup. **Corrected-oracle fix [sa](47f820f0-5339-49f8-87ee-b90bace48ae8) in flight (test + additive test-route only, NO product):** extend `recording-count` to expose `blobUrls`/`distinctBlobUrlCount`; rewrite teeth = survival (`>=`) + **no-duplicate-blobUrl (`distinctBlobUrlCount===count` — definitive dup check, FAILS→escalate real (A))** + no-End-re-add (`final===beforeEnd` after drain+stop-capture) + no-ghost (`postEndOutbox===[]`, the real `6799aa4` teeth); freeze VAD before resume for determinism. **§If distinct-blobUrl passes → closes the 12%, no duplication.** | 
| **In-flight subagents** | (see P1-WB-6 row above — corrected-oracle fix [sa](47f820f0-5339-49f8-87ee-b90bace48ae8) in flight). **Tranche B progress (5/9 committed green):** P1-WB-1 ✅ `95acb3f`, P1-WB-2 ✅ `398a4c2`, P1-WB-3 ✅ `507c8ce` (stale-oracle repair), P1-WB-4 ✅ `970aa18` (fragile replay — no product seek bug), P1-WB-5 ✅ `c46662f` (notes ≤3s + zero-LLM WS-K; +2 prod-inert test routes, verified [review](cd8c6801-cd29-4305-81d0-be633000f006)). WS-T #7 (`d9a6c4d`): 4 pre-existing `wb-session-lifecycle` reds, triage owed. Queue: P1-WB-7 End-guards RW-8 → P1-WB-8 review-overlay → P1-WB-9 admin cost-dashboard auth. |
