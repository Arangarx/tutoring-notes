...
| **In-flight subagents** | **1 — 2FA "remember this device" EXECUTOR (Sonnet, background)** on `auth/2fa-remember-device`. Chain: plan [`b079a1d`](https://github.com/Arangarx/tutoring-notes/commit/b079a1d) → 5-axis [`588f732`](https://github.com/Arangarx/tutoring-notes/commit/588f732) (NEEDS-REVISION: 3 BLOCKER + 6 SHOULD-FIX, all plan-text) → fold Rev 2 [`f0bcbdf`](https://github.com/Arangarx/tutoring-notes/commit/f0bcbdf) → executor building (impl + TD-* tests + additive `AdminTrustedDevice` migration [unapplied to prod] + smokebook; gate = tsc/eslint/jest/`next build`). **Origin:** Andrew tired of TOTP on every preview/prod login (preview SSO already solved free via stable `preview.usemynk.com` subdomain — no code). Trusted-device = the login-nag fix. BLOCKER fixes: B1 impersonation-start step-up, B2 fail-closed skip mint, B3 step-up TOTP rate-limit. |
