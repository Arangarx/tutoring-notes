...
| **Next action(s)** | **PRE-MASTER gate = ONE re-smoke on `phase1/wb-review-correct` @ [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130):** LV-2 two-device on latest deploy ([`cbe6d20`](https://github.com/Arangarx/tutoring-notes/commit/cbe6d20) + [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130)) — student enables cam after joining → tutor sees student video **without** needing to resize the cluster, and resizing scales both tiles smoothly (no squish). *(Scrub re-smoke [`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08) — **CONFIRMED**.)* Smokebook [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — Andrew actively entering results (**do NOT edit**). **Once LV-2 passes →** `npm run test:wb-sync` + merge `--no-ff` to `v1-redesign`/`master`. Everything else confirmed or deferred. **(2) P2 student-shell** execute-ready @ [`80ac571`](https://github.com/Arangarx/tutoring-notes/commit/80ac571) — awaiting greenlight (unchanged). **(3) Prior threads:** `phase1/wb-reliability-floor` @ `d63ac22` merge after Andrew desktop smoke; Gate A→A6 unchanged; `Experience-Driven Wedge` program + consent v2 when Andrew prioritizes. **Deferred post-master (Andrew 2026-06-16, captured in BACKLOG):** DRY viewport consolidation (**WB-DRY-VIEWPORT**), review transition animation polish (**WB-REVIEW-TRANSITION**), impersonation session-recovery bypass (**WB-IMPERSONATION-SESSION**), eraser reliability + partial erase (**WB-ERASE-RELIABILITY**), new-session false recovery banner (**WB-NEW-SESSION-FALSE-RECOVERY**). |
| **Open Andrew-confirms** | **P1 replay (2026-06-16) — confirmed:** replay first-play starts at 0:00 ✓; live+replay center tracking ✓; thumbnail-pin + button layout ([`8380bf8`](https://github.com/Arangarx/tutoring-notes/commit/8380bf8)) ✓; #5 audio pauses when leaving replay to notes-hero ✓; hard-refresh stored-recording scrub ([`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08)) ✓; in-page delete warning (no `alert()`) ✓; notes/form across prominent↔docked ✓; LV-1 self-view both sides ✓; idempotency #13 tentative (will reconfirm); legacy standalone replay D1-D3 skipped. **Pending re-smoke (pre-master gate — 1 item):** LV-2 two-device late-add cam + cluster resize ([`cbe6d20`](https://github.com/Arangarx/tutoring-notes/commit/cbe6d20) + [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130)) — student enables cam after join → tutor sees video without manual cluster resize; resize scales both tiles smoothly (no squish). **Resolved — deferred post-master:** DRY consolidation, transition animation #2, impersonation bypass, eraser reliability, new-session false recovery → [`BACKLOG.md`](../BACKLOG.md) § P1 replay-in-frame post-master follow-ups. **Standing:** (c) student URL · (d) learner-swap · (e) student camera default · (f) C1–C4 · (g) env scoping · B2 D-1/D-2/D-5 · B1 deferred TODOs · N-2. **P2 Q1/Q7** branch strategy + `defaultShowLocalVideo`. **Ratified 2026-06-11:** wall-clock metering. **Lawyer-needed:** VPC, retention, FERPA/SOPIPA. |
...
| **P1 replay-in-frame (ACTIVE thread 2026-06-16)** | Branch `phase1/wb-review-correct` @ [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130) — unified in-frame review surface. **Session fixes (2026-06-16):** replay CTA follows thumbnail [`8380bf8`](https://github.com/Arangarx/tutoring-notes/commit/8380bf8) ✓; replay seek-on-load [`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08) ✓ (12/12 scrub tests; Andrew confirmed hard-refresh scrub); live-A/V late-add renegotiation [`cbe6d20`](https://github.com/Arangarx/tutoring-notes/commit/cbe6d20) (109 tests); live-A/V play() + cluster resize height [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130) (71/71 tests); replay center-match per frame [`ed04279`](https://github.com/Arangarx/tutoring-notes/commit/ed04279); hero thumbnail pin [`a05abdf`](https://github.com/Arangarx/tutoring-notes/commit/a05abdf); live-A/V track buffer [`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8). **Andrew-confirmed:** replay first-play at 0:00; live+replay center; thumbnail-pin + button layout; #5 audio pause on notes-hero exit; hard-refresh scrub; delete warning; notes/form dock toggle; LV-1; audio seek/header/volume/thumbnail-dark/resize-recenter (prior fixes). **Pending re-smoke (pre-master — 1 item):** LV-2 late-add cam + cluster resize ([`cbe6d20`](https://github.com/Arangarx/tutoring-notes/commit/cbe6d20) + [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130)). **Deferred post-master (BACKLOG):** transition animation polish; DRY viewport consolidation; impersonation session-recovery bypass; eraser reliability; new-session false recovery. Smokebook [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — Andrew actively entering results (**do NOT edit**). Prior FIX 1–7 + WebM duration + seek-storm + resize-recenter chain still in place. |
...
| **Live-A/V tutor video regression (FIXED 2026-06-16)** | **LV-1 CONFIRMED** (Andrew real browser): tutor self-view + student sees self + tutor. **LV-2 fixes:** (1) per-peer pending-track buffer @ [`3261da8`](https://github.com/Arangarx/tutoring-notes/commit/3261da8) — student's `ontrack` arriving before sync presence no longer drops track; kept as defense-in-depth. (2) **Late-add renegotiation @ [`cbe6d20`](https://github.com/Arangarx/tutoring-notes/commit/cbe6d20):** student enabling cam after join — `onnegotiationneeded` defers when not stable + `onsignalingstatechange` flushes pending renegotiation; 109 tests pass. (3) **play() + resize height @ [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130):** `AVTile` never called `play()` after `srcObject` (video loaded but didn't paint until resize reflow); `WbAVCluster` resize snapshotted stale height → tiles squished; 71/71 tests — independent of cbe6d20 (stream was already arriving). **Awaits Andrew two-device re-smoke:** student enables cam after joining → tutor sees student video **without** manual cluster resize; resize scales both tiles smoothly (no squish). Prior tutor cam-acquisition fix (`682fc15`/`add3b81`) + tile-body min-height still in place. Part of ship-to-Sarah gate (A2 A/V verify). Student-side A/V cluster unification in P2. `test:wb-sync` owed pre-merge. |
| **Uncommitted / unmerged** | **TWO unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (merge after Andrew DESKTOP smoke). (2) **`phase1/wb-review-correct` @ [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130)** — in-frame unified review surface + full replay/A/V fix chain (seek-on-load [`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08), late-add renegotiation [`cbe6d20`](https://github.com/Arangarx/tutoring-notes/commit/cbe6d20), play()+resize [`40b3130`](https://github.com/Arangarx/tutoring-notes/commit/40b3130), CTA layout [`8380bf8`](https://github.com/Arangarx/tutoring-notes/commit/8380bf8), center-match, thumbnail pin, track buffer, audio seek, WebM duration, seek-storm, resize-recenter). **Andrew actively smoking** via [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — do NOT edit. **Confirmed:** replay first-play at 0:00; live+replay center; thumbnail-pin + button layout; #5 audio pause on notes-hero exit; hard-refresh scrub; delete warning; notes/form dock; LV-1; audio seek/header/volume/thumbnail-dark/resize-recenter. **Pending re-smoke (1 item):** LV-2 late-add cam + cluster resize. **If passes →** `npm run test:wb-sync` + merge `--no-ff` to `v1-redesign`. **DONE/MERGED:** `auth/2fa-remember-device` @ [`8ffd344`](https://github.com/Arangarx/tutoring-notes/commit/8ffd344); `fix/preview-badge-copy-commit` @ [`feccae9`](https://github.com/Arangarx/tutoring-notes/commit/feccae9). **Known pre-existing:** `identity-2fa-management` stale jest assertion; `test:wb-sync` broadcast-throttle half fail (pre-existing, not redesign). **Reset-screen generate-offer → BL-RESET-GENERATE** (backlog). |
