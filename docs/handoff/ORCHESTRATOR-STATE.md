...
| Field | Value |
|---|---|
| **Last action completed** | **✅ WS-T #7 FULLY RESOLVED + PUSHED [`9450906`]** (Tranche E). Triage ([run](792d8f06-d30a-4872-b619-3ab928d5d2ca)) proved all 4 `wb-session-lifecycle` reds DETERMINISTIC (not flake). **#2/#3/#4 = stale oracles** orphaned by WS-M (`3947728`) → realigned to positive WS-M student-overlay oracles (caret visible, `mic-gain-slider` present, `mic-device-select` absent in popover, on-page `audio-device-select` present). **#1 = correct oracle + correct product BUT a real waiting-room-cancel reliability defect** (non-optimistic Cancel; soft `router.push` doesn't commit from `/workspace` → tutor stranded on admin-not-found ~17% relay). **Product fix (WWC `handleCancelPendingSession`):** await the fast un-started PENDING delete → `window.location.assign` hard-nav; removes delete-fetch abort race + revives `cancelError` surface; dropped dead `router.push`; confined to the one handler (FSM/sync/lifecycle untouched). **Sonnet 5-axis = SHIP-WITH-FIXES no blockers** ([review](02ff6d52-0fec-4fd5-ad7f-2861c7714110)); await-then-hard-nav is the conservative variant resolving the reviewer's abort-race + dead-error findings. Full file **49 passed / 0 failed / 1 skipped** (was 45/4); tsc clean. Prior: P2-J5 outbox drain UI test [`9747425`]; IDENTITY-E2E 4/4. Tree **CLEAN**, fully pushed. |
...
| **Next action(s)** | **Andrew chose 'root-cause both bugs now' (2026-07-06).** ✅ Stale oracle #20 FIXED + pushed (`0118359`, oracle → `/solo rehearsal|recording/i`, verified PASS — also progresses P2-WB-2). ✅ Bug B refined (L159 = test oracle; guard sound; branch-only). **ALL DIAGNOSTICS DONE.** Bug A = confirmed real (branch-only WS-I gap); Bug B = test-only; #20 fixed. **PENDING ANDREW decisions:** (1) **Bug A fix** — fragile surface (`mic-recorder-audio`/`useAudioRecorder` acquire path); sync `setTutorRecordingMute` on mute-click + workspace-bridge DOM test; Composer step-back plan → Sonnet review; green `wb-tutor-recording-mute.spec.ts` required. Needs Andrew go (it's a WS-I completeness gap before WS-I merges). (2) **Bug B apply-remote spec realignment** — test-only, Composer. (3) **infra:** relay merge-gate must run as serial SHARDS (one 20-min marathon manufactures ~18 false reds via dev-server exhaustion); jest isolation-class-2 (per-test DB cleanup — email fix done, a non-email pollution class remains). Bug A + Bug B + infra follow-ups LOGGED to BACKLOG (`## wb-wave5-polish ... relay-unblock findings (2026-07-06)`) + execution-queue WS-T #10/#11 + GATE/HARNESS (c)resolved/(e)/(f) — commit `e3542fd`. **This is a clean context-swap milestone (Andrew swapping 2026-07-06).** Harness+oracle fixes committed (`3cb9a7b`,`8a381ce`,`0118359`); findings `e3542fd`; state pushed. **DEFERRED (Tranche E remaining — Andrew chose harness first):** — recommend harness-hardening (Tranche D: per-worker DB or transactional per-test cleanup or unique fixture keys across seeding files; + auth.setup signout/csrf robustness) FIRST, because a suite that can't run green isn't trustworthy wholesale regardless of individual test quality, and it unblocks merge-gate #1. Flagged "do not rush unattended" (touches seeding across many files) → careful attended pass. Deferred alt: continue **P2-WB-2** → **P3-J1..5**, batch harness later. **Still owed pre-master (gated):** A4 WS-X relay red/green, WS-T #9 outbox self-heal (design-first), Tranche C WS-G-A poll. Docker relay :3002 + Postgres :5432 UP. | **Gated/deferred:** WS-T #9 upload-outbox self-heal (design-first), WS-T #7 relay triage (4 pre-existing `wb-session-lifecycle` reds, owed before master-cut), A4 WS-X relay red/green (deferred — fragile, no blind 3rd fix), Tranche C WS-G-A "preparing seamless replay…" poll (design-first), Tranche D jest-isolation pass. P1-WB-10 IN_PERSON already discharged via A3; Tranche B P1-WB 9/9 complete. Tranche A discharged: A1 build ✅, A2 WS-P defer-reload ✅, A3 in-person ✅ relay-proven. |
...
| **Uncommitted / unmerged** | Branch **`wb-wave5-polish`** @ code-tip [`310aa75`](https://github.com/Arangarx/tutoring-notes/commit/310aa75) (RW-B2; state-doc commit follows), worktree **`tutoring-notes-polishwt`**; tree **CLEAN**, **fully pushed (0 ahead / 0 behind origin)**. **NOT merged** to `v1-redesign`/`master` (Andrew hard stop). **Merge-gate proofs:** A1 `next build` ✅, A2 WS-P defer-reload seam ✅, A3 in-person relay ✅; **A4 WS-X relay OWED/deferred**. CRLF artifact on `EndedUnsavedSessionsList.tsx` reverted; no uncommitted files. |
...
**Andrew decisions ledger (2026-07-06):** approved Tranche E → then chose fix+baseline → then harden-now → then root-cause-both-bugs → then swap → **(post-swap) approved Tranche F infra-first:** F1 jest-isolation-2 → F2 relay-shards → F3 Bug B (test-only); **Bug A = GO** with plan-mode design → Sonnet 5-axis → fix (green `wb-tutor-recording-mute.spec.ts` = merge bar); then resume Tranche E (P2-WB-2 → P3).

**🟥 F1 SETBACK (2026-07-06, post-swap) — global per-test TRUNCATE mechanism FAILED, not committed.** Diagnosis ([b638f945](b638f945-7db5-4ac6-88f0-c3c3a93a3497)) recommended a global `afterEach` `TRUNCATE CASCADE`; implementation ([751f74b3](751f74b3-3d74-4d10-b037-b145b5741541)) produced **~40 failed suites/run × 3** (vs ~1-flake baseline) — `40P01` deadlocks + FK/"Engine not connected" races: **TRUNCATE's AccessExclusiveLock collides with in-flight fire-and-forget async DB work that outlives test bodies** (same stragglers as the `Cannot log after tests are done` + CostEvent-FK leaks). **REAL ROOT CAUSE reframed:** tests leak async DB writes → *any* between-test cleanup races them; the correct fix is to eliminate the stragglers (await/mock all fire-and-forget DB paths across ~45 integration suites) — a multi-day careful effort, NOT the truncate. Attempt preserved in `git stash@{0}` (`wb-wave5-polish`); **2 salvageable test-only fixes inside it** (upload-outbox observation-timing determinism, transcription-worker extract-chunk leak mock — both independent of the truncate, pass in isolation). **Baseline restored (tree clean @ `7d655ff`).** **PENDING ANDREW scope decision (surfaced):** (rec) salvage the 2 test-only fixes + revert truncate + DEFER heavy jest-isolation as a tracked follow-up + keep merge-gate jest at `--workers=1`+retry (already the documented KNOWN CONDITION; ~1-flake is a loud stale-red, NOT the dangerous false-green class) → move to **F2 relay-shards** (the actual merge-gate blocker) then Tranche E. Alternatives: (b) scoped per-suite `afterAll` cleanup on the aggressive-deleter suites only; (c) full straggler-elimination pass now (multi-day). **Not dispatching until Andrew steers.**
