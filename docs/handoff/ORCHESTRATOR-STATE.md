...
## ⏩ HEAD — 2026-07-01 Erasure execution started — orphan cleanup done + BLOCKERs B & I shipped @ ac963a3

> **Active branch:** [`wb-wave5-polish`](https://github.com/Arangarx/tutoring-notes/tree/wb-wave5-polish) @ [`ac963a3`](https://github.com/Arangarx/tutoring-notes/commit/ac963a3). **Worktree:** `tutoring-notes-polishwt` (default `tutoring-notes` checkout is on `v1-redesign` — NOT current). **All remaining Sarah-gate work lands here; single `merge --no-ff` to `v1-redesign` at the FINAL full-arc gate only (no interim merge — Andrew reaffirmed 2026-07-01).** Integration base remains **`v1-redesign` @ [`7397abc`](https://github.com/Arangarx/tutoring-notes/commit/7397abc)**.
>
...
| Field | Value |
|---|---|
| **Last action completed** | **Erasure Steps 1 + 2 shipped** — BLOCKER **B** (additive `LearnerCredential.disabled` + fail-closed learner login) @ [`d3458f9`](https://github.com/Arangarx/tutoring-notes/commit/d3458f9); BLOCKER **I** / MB-2 (both tombstone helpers THROW on `not_found` + `DELETE`-path existence validation) @ [`ac963a3`](https://github.com/Arangarx/tutoring-notes/commit/ac963a3). Migration `20260701120000_learner_credential_disabled` **live on preview-dev** (auto-applied + recorded in `_prisma_migrations` by Vercel `migrate deploy` on push; `disabled` column verified). Local `.env` → **preview-dev** endpoint `ep-bold-feather-amv9wmao` (NOT production `ep-still-dream-amuo3nrn`); subagent P1001 was transient compute-suspend, no prod risk. **3 orphan `ErasureJob`s canceled** on preview-dev (incl. mis-scoped MB-2 account_holder job whose scopeId was a learner-profile id "Bob Bobsley"). Scoped tests green (5 new); `npx next build` exit 0; 5 pre-existing jest failures unrelated. — — — **Prior:** 5-axis review (9 BLOCKERs + Option A); CF-1 ([`183f09b`](https://github.com/Arangarx/tutoring-notes/commit/183f09b)) + CF-3 ([`7a9514f`](https://github.com/Arangarx/tutoring-notes/commit/7a9514f)). |
| **Next action(s)** | **Erasure Step 3** (ER-2+ER-5 as ONE atomic commit: soft-disable credential / preserve AH+LP PII through grace / atomic `db.$transaction` cancel-restore [C] / `advancePhase` conditional-update `WHERE status='requested'` + affected-count [D/F] / re-request race [E] / `scrubDbContent` hard-redacts AH+LP PII at grace expiry [A]) — **ER-7 test rewrite MUST land in the SAME commit**. Grounding erasure integration-test DB wiring first (read-only explore in flight) so the rewrite is provable red/green. Then **Sonnet 5-axis review of the Step 3 diff** before building further → **Step 4** (ER-3 guards on create/startWhiteboardSession + extend `assertStudentNotErased`(+Api) to active `ErasureJob`; all content routes — G/H) → **Step 5/6** (ER-4 UI + ER-6 copy) → **CF-2** (wb-sync-gated) + **CF-4** → **Workstream C** Playwright e2e → **checkpoint re-smoke (NO merge)** → **Part 3** spine → **full-arc both-themes hardware smoke** → single `merge --no-ff → v1-redesign`. |
