...
## ⏩ HEAD — 2026-06-18 wb-unify-stabilize: W1-3 smoked, ENGINE FIX WAVE in flight

> Full restructure of this doc is OWED at the next milestone (merge of `wb-unify-stabilize` → `v1-redesign`) — dispatch Composer. Head below is current.

| Field | Value |
|---|---|
| **Last action completed (2026-06-18)** | **Engine fix wave dispatched** (Sonnet, bg [`79d9d089`](79d9d089-e1d0-4c9b-bd8b-81571ef48ca2)) on `wb-unify-stabilize` @ [`39daece`](https://github.com/Arangarx/tutoring-notes/commit/39daece). Prior: Andrew's **hardware smoke of Waves 1-3** captured ([`39daece`](https://github.com/Arangarx/tutoring-notes/commit/39daece)) — PASS: unify structure, cross-page-bleed fix, laser bidir, dark theme, exit-shrink, no-cam initials, right-click end-line, follow toggle, auto-A/V. **FAIL (merge-blockers):** tutor eraser unreliable; undo/redo erratic (student undo hits tutor strokes + resurrects other-page strokes); exit→rejoin A/V recovery broken; video-disappears-until-resize back. Housekeeping done on `v1-redesign` @ [`6aced18`](https://github.com/Arangarx/tutoring-notes/commit/6aced18) (5-state smokebook template incl. "N/A with notes" + 11 backlog IDs). |
| **Next action(s)** | (1) Engine fix wave returns → review. (2) **A/V rejoin recovery fix wave** (Sonnet) — exit→rejoin recovers without ghost/stall; paint-on-reconnect without resize. (3) **Single `wb-sync` relay gate** on final fixed tip (merge-boundary cadence — NOT per wave). (4) Update STATE; hand to Andrew for **re-smoke**. (5) On PASS → **`merge --no-ff` `wb-unify-stabilize` → `v1-redesign`**. (6) Waves 4-5 (chrome/responsive, laser colors, polish) post-merge. |
| **Open Andrew-confirms** | **Re-smoke PASS** of the fixed result (eraser/undo/rejoin) → merge to `v1-redesign`. |
| **In-flight subagents** | **Engine fix wave** — [`79d9d089`](79d9d089-e1d0-4c9b-bd8b-81571ef48ca2) (Sonnet, bg): tombstone/version reconcile so student v2 echo can't resurrect tutor deletes; per-page history isolation; full `captureUpdate:"NEVER"` coverage; undo-button intermittency. |
| **Uncommitted / unmerged** | **WORK BRANCH:** `wb-unify-stabilize` @ `39daece` (W1-3 + smoke results) — engine + A/V-rejoin fixes landing here before merge to `v1-redesign`. **Other unmerged:** `phase1/wb-reliability-floor` @ `d63ac22` (awaits DESKTOP smoke). NOTE: `phase2/wb-student-new-shell` superseded by the unify-stabilize thread. |
