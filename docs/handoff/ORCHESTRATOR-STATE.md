...
| **P1 replay-in-frame (ACTIVE thread 2026-06-15)** | Branch `phase1/wb-review-correct` @ [`14bc644`](https://github.com/Arangarx/tutoring-notes/commit/14bc644) — unified in-frame review surface (notes prominent↔docked, persist-once replay inside live WB chrome). **3rd Andrew smoke FAILED → root-caused + fixed (Sonnet).** Recurring failures (audio reset-to-0 on play/scrub, canvas-below-chrome, blank thumbnail) had survived 3 passes because prior "proof" was jsdom-only. **Real root causes:** (audio) `attachWebmDurationFix` async `currentTime=0` on `durationchange` raced the scrub + `play()` re-entered `loadSegmentAt`→`el.load()`→0 + an unstable inline `audioSegments` array ref re-fired the reset block every render — all fixed, test now red-before(`Expected 5 Received 0`)/green-after simulating the WebM lifecycle; (canvas) `.mynk-wb-replay-body` was `flex-direction:column` + inline `position:relative` beat the absolute-fill class — fixed, **Playwright-verified** canvas fills chrome 1236×800 zero-gap; (thumbnail) one-shot `paintedRef` set before fit succeeded — fixed (mark on `onFit` + ResizeObserver retry + asset registration); (**session idempotency, item 13 — introduced by P1 routing fb53b2f**) ended `/workspace` reused persist-once replay across session ids → played a PREVIOUS session — fixed via `key={whiteboardSessionId}` remount + `entryPaintDoneRef` reset. Also: pause audio on Hide-replay (Andrew clarified), delete-session in-page confirm + optimistic navigate, zenMode to kill native "scroll-back" button, thumbnail z-index. **Awaits Andrew's 4th smoke** — audible audio-scrub + visual canvas/thumbnail/resize are the real-browser-only confirmations. Diagnoses: [replay engine](b9c90bc4-5456-4c11-a329-cb6fbb2ecc0c) · [session](c96f1998-8a9b-481d-b167-437d715cf758); fix pass [21d25bd3](21d25bd3-c530-4154-ace6-478f733ed6b6). **Lesson reinforced:** jsdom/unit "proof" is theater for audio-element-timing + layout — require real-browser (Playwright currentTime/bbox) or red-before/green-after lifecycle sims. |
| **Uncommitted / unmerged** | **TWO unmerged branches off `v1-redesign`:** (1) `phase1/wb-reliability-floor` @ `d63ac22` — 1b audio-clock + 3-tier iOS-safe fallback (merge after Andrew DESKTOP smoke). (2) `phase1/wb-review-correct` @ [`14bc644`](https://github.com/Arangarx/tutoring-notes/commit/14bc644) — in-frame unified review surface (3rd smoke fixed; awaits Andrew 4th smoke; orchestrator owes `test:wb-sync` pre-merge). _(Earlier `phase1/wb-replay-in-frame` @ `52a9dae` superseded by `phase1/wb-review-correct`.)_ **DONE/MERGED:** `auth/2fa-remember-device` (unified auth branch) @ [`8ffd344`](https://github.com/Arangarx/tutoring-notes/commit/8ffd344) — thread closed; `fix/password-manager-username-field` absorbed into it (do NOT merge separately); `fix/preview-badge-copy-commit` MERGED @ [`feccae9`](https://github.com/Arangarx/tutoring-notes/commit/feccae9) (pill copies branch + commit id). **Known pre-existing test debt (carried forward):** `identity-2fa-management` "settings page links" jest assertion is stale (checks JSX `href=` on an object-literal `href:`) — unrelated, fix opportunistically. **Reset-screen generate-offer → BL-RESET-GENERATE** (backlog), Andrew wants eventually. **⚠️ Dispatch-safety near-miss (2026-06-14):** badge executor `git stash`+reset on orchestrator's uncommitted state edits instead of STOPPING — preserved+recovered; reinforce STOP-don't-stash; commit state promptly. |
