...
| **Next action(s)** | **PRE-MASTER gate = ONE re-smoke on `phase1/wb-review-correct` @ [`caaabf2`](https://github.com/Arangarx/tutoring-notes/commit/caaabf2):** LV-2 two-device — student enables cam after joining → tutor sees student video **paint on arrival without** manual cluster resize; resize still scales both tiles smoothly (no squish). Remaining failure is **purely video PAINT-on-arrival**. This has been the **sole blocker for 5 attempts**; on-device is the **only verifier** (jsdom has no compositor). **Attempt 6 [`caaabf2`](https://github.com/Arangarx/tutoring-notes/commit/caaabf2)** is a principled CSS fix grounded in the working student-side reference: the A3b CSS rule was removing `aspect-ratio: 4/3` from the tile video body, leaving it with no intrinsic height in an absolutely-positioned flex chain — Chrome's compositor doesn't wire such elements on initial mount. Removed `aspect-ratio: unset !important` → body keeps `4/3` → concrete intrinsic box at mount → matches student path exactly. Also cleaned up: removed Mechanism A (WbAVCluster auto-fires setSize+setUserResized) + Mechanism B (AVTile ResizeObserver). Kept: key-remount + double-rAF play(). 80/80 tests. *(Scrub re-smoke [`1240c08`](https://github.com/Arangarx/tutoring-notes/commit/1240c08) — **CONFIRMED**.)* Smokebook [`phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md`](phase-1-wb-floor-replay-in-frame-smokebook-2026-06-14.md) — Andrew actively entering results (**do NOT edit**). **Once LV-2 passes →** `npm run test:wb-sync` + merge `--no-ff` to `v1-redesign`/`master`. Everything else confirmed or deferred. **Decision pending:** WB-NEW-SESSION-FALSE-RECOVERY (stale IndexedDB recovery banner on new sessions) — pull forward pre-master vs leave post-master (Andrew asked). **(2) P2 student-shell** execute-ready @ [`80ac571`](https://github.com/Arangarx/tutoring-notes/commit/80ac571) — awaiting greenlight (unchanged). **(3) Prior threads:** `phase1/wb-reliability-floor` @ `d63ac22` merge after Andrew desktop smoke; Gate A→A6 unchanged; `Experience-Driven Wedge` program + consent v2 when Andrew prioritizes. **Deferred post-master (Andrew 2026-06-16, captured in BACKLOG):** DRY viewport consolidation (**WB-DRY-VIEWPORT**), review transition animation polish (**WB-REVIEW-TRANSITION**), impersonation session-recovery bypass (**WB-IMPERSONATION-SESSION**), eraser reliability + partial erase (**WB-ERASE-RELIABILITY**), new-session false recovery banner (**WB-NEW-SESSION-FALSE-RECOVERY**). |
