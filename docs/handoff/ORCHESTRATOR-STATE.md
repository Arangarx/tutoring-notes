...
| WS-D resume-from-backend | ⬜ IN PROGRESS | server hydrate on ACTIVE open (boardDocumentJson multi-page); remove IDB prompt for ACTIVE w/ server batches; clock continuity. `wb-resume-from-backend` Playwright. |
