...
## ⏩ HEAD — 2026-06-08 (latency root-caused; P1.1 Sonnet-tier)

| Field | Value |
|---|---|
| **Last action completed** | **Freedraw draw-latency investigation COMPLETE + root-caused (2026-06-08)** — ~250ms solo lag on `v1-redesign` (stroke trails cursor); Phase 0 POC instant; primary cause = `preserveImageAssetUrlsOnSceneWrite` full-scene clone on every pointer-move in `handleExcalidrawChange` (+ secondary v3 emit / recorder periodic work). **Andrew decision 2026-06-08:** fix **folded into P1.1 whiteboard-chrome build as Phase-1 acceptance gate** (Option A + Option E) — **not** standalone, **not** deferred follow-up. **P1.1 escalated to Sonnet-tier** (fragile sync hot path). Captured: [`whiteboard-chrome-design-2026-06-07.md`](whiteboard-chrome-design-2026-06-07.md) §6 P1.1 gate, **PR-01**, `BACKLOG.md` row. Prior: Excalidraw audit dispositions ratified; P1.2 toolbar design **unblocked** (design-only). Phase 0 POC = GREEN. |
| **Next action(s)** | **P1.1 — tutor-desktop hybrid chrome (Sonnet dispatch)** — chrome-over-workspace + **freedraw-latency hot-path fix** (Phase-1 acceptance gate); gates: `test:wb-sync` + wire cadence tests. **P1.2 toolbar/properties design** still unblocked — **design-only**, can run **parallel** to P1.1; **serialize** on `test:wb-sync` against P1.1 implementation. **A′ theme foundation** still awaiting Andrew's both-theme smoke. Spike worktree `spike/wb-chrome-poc-857e6d4c` **RETAINED**. **Pre-master gates still open:** (1) component-pass cohesive visual review; (2) whiteboard chrome **build** (incl. PR-01). Cut `master` only when whole site is one cohesive reveal. |
...
| **In-flight subagents** | **P1.1 tutor-desktop hybrid chrome** (Sonnet-tier; incl. freedraw-latency gate). **A′ theme foundation** awaiting Andrew both-theme smoke. |
...
| **Uncommitted / unmerged** | **`v1-redesign` @ [`1eb9266`](https://github.com/Arangarx/tutoring-notes/commit/1eb9266)** (audit dispositions + TU-13/TU-14/TM-10). **Main tree ON `v1-redesign`.** **Unmerged branches → v1-redesign:** `feature/sarah-forward-migration-q6` @ `a396ab5` (parked). **MERGED (retained for branch-sweep cleanup):** `feat/recording-p1-slice3-autonotes`, `harden/auth-role-refresh` + assorted stale worktrees (`tn-auth-harden`, `tn-docs-notes`, `.cursor/worktrees/*`). |
