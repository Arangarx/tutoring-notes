...
| **Uncommitted / unmerged** | **None uncommitted** (untracked `.jest-config-dump.json` + `.git/*.log` build/test logs are throwaway). `v1-design-system` @ tip `287aa3d` (+ pending doc-finalize commit) pushed to origin. Branch layering: **`master`** ← **`v1-redesign`** (held for Gate A + re-smoke) ← **`v1-design-system`** (all 7 groups merged). **Parked:** `feature/sarah-forward-migration-q6` @ `a396ab5`. **Per-group worktrees** (`v1ds-group-*`, `group-g-*`) remain — stale-sweep eligible after Andrew review. |
