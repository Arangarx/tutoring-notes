...
export async function tombstoneAccountHolder(
  ...
  if (ah.tombstonedAt === null) {
    ...
    if (credentialsDisabled > 0) {
      console.log(
        `[ers] action=soft_disable_credential scope=account_holder scopeId=${accountHolderId} count=${credentialsDisabled}`
      );
    }
  }
...
export async function tombstoneLearnerProfile(
  ...
  console.log(
    ...
    `[ers] action=tombstone_learner_profile scopeId=${learnerProfileId} sessions_revoked=${sessionsRevoked}`

  );

  if (disableResult.count > 0) {
    console.log(
      `[ers] action=soft_disable_credential scope=learner_profile scopeId=${learnerProfileId} lp=${learnerProfileId} count=${disableResult.count}`
    );
  }
