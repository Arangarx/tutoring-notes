/**
 * perspeaker-identity ΓÇö deterministic CORE of per-speaker capture (Phase 4 / p3-perspeaker C).
 *
 ...
 * (recorder lifecycle + outbox-drain trigger) is intentionally NOT here ΓÇö this
...
describe("resolveSpeakerId ΓÇö stable speaker identity across devices", () => {
...
describe("speakerTranscriptStreamId ΓÇö transcript lane keyed on identity, not peer", () => {
...
describe("reconcileSpeakers ΓÇö dedup by identity, newest device wins, never pre-kill incoming device", () => {
  ...
  it("does NOT drop the incoming device based on raw stream count (the 3>2 pre-kill bug)", () => {
    ...
    // 2 distinct identities (tutor + student A) ΓÇö within cap; incoming device B is the surviving one.
  ...
  it("is deterministic when joinedAt is missing (stable peerId tiebreak, no throw)", () => {
    ...
    const res = reconcileSpeakers(peers, { maxSpeakers: 4 });
    expect(res.active).toHaveLength(1);
    // tie on joinedAt ΓåÆ deterministic pick = lexicographically smallest peerId.
...
describe("dedupSpeakerSegments ΓÇö collapse the brief A+B device-switch overlap", () => {
