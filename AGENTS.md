...
- [docs/LEGAL-SYNC.md](docs/LEGAL-SYNC.md) — **read before touching
  ...
  consent flow, or any external policy reference.** The
  **`mortensenapps.com/privacy` + `mortensenapps.com/terms`** umbrella is
  the **canonical legal source** (Google's verification rounds have been
  against those URLs; verification history lives in the mortensenapps.com
  site repo). The product's `/privacy` and `/terms` are **local
  subordinate facades** that supplement the umbrella with product-
  specific sections (Vercel Blob audio, OpenAI Whisper, whiteboard data,
  minor-data tutor-consent specifics). This doc maps which sections are
  umbrella-derived vs. product-specific, the sync protocol when the
  umbrella changes (incl. Google OAuth Limited-Use re-verification
  check), and the quarterly drift review cadence. **Open question** —
  which URLs the Tutoring Notes OAuth consent screen registers in Google
  Cloud Console is not yet confirmed; resolving that is an action item
  in the doc.
...
- **Legal copy stays synced with the umbrella.** `mortensenapps.com/privacy`
  + `mortensenapps.com/terms` are the **canonical legal source** (the
  Google verification rounds have been against those URLs; the
  mortensenapps.com site repo has the verification history). The
  product's `/privacy` and `/terms` are **local subordinate facades**
  that supplement the umbrella with product-specific sections (Vercel
  Blob audio, OpenAI Whisper, whiteboard data, minor-data tutor-consent
  specifics) — they are not a parallel canonical source. Any change to
  either TSX file MUST follow the sync protocol in
  [docs/LEGAL-SYNC.md](docs/LEGAL-SYNC.md): identify whether the
  edited section is umbrella-derived (must match upstream verbatim)
  or product-specific (free to edit), update the top-of-file sync
  date and the in-UI "Last updated" string, and update the doc's
  classification tables if the section changed type. Quarterly drift
  review applies. **Open question** in the doc: which URLs the
  Tutoring Notes OAuth consent screen registers in Google Cloud
  Console — confirm before any sync that materially edits embedded
  umbrella sections.
