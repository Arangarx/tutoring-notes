...
- [docs/LEGAL-SYNC.md](docs/LEGAL-SYNC.md) — **read before touching
  ...
  consent flow, or any external policy reference.** The product's `/privacy`
  and `/terms` are intentionally local copies (not redirects) of the
  Mortensen Apps umbrella at `mortensenapps.com/privacy` + `/terms`. This
  doc maps which sections are umbrella-derived vs. product-specific, the
  sync protocol when the umbrella changes (incl. Google OAuth Limited-Use
  re-verification check), and the quarterly drift review cadence.
...
- **Legal copy stays synced with the umbrella.** `/privacy` and
  `/terms` are local copies of `mortensenapps.com/privacy` +
  `mortensenapps.com/terms`, not redirects, because Google OAuth
  verification is anchored to the tutoring-notes domain and the
  product needs sections the umbrella omits (Vercel Blob audio,
  OpenAI Whisper, whiteboard data, minor-data consent specifics).
  Any change to either TSX file MUST follow the sync protocol in
